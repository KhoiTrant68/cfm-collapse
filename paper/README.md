...
- `figures/` — 18 PNGs referenced by `main.tex`. Two kinds:
  - **copied from `results/`** (the analysis-script outputs): `fig_p1_variance`,
    ...
    `fig_exp2_coverage`, `fig_exp2_2d`, `fig_exp3_curve`, `fig_exp3_grid200`,
    `fig_exp3_grid15k`.
  - **generated by `scripts/make_paper_figures.py`** from the theory CSVs (these
    visualise results that were table-only before): `fig_p7_summary` (P7 headline),
    `fig_optgap` (T5), `fig_lipschitz` (T6), `fig_posterior_distance` (T7).
