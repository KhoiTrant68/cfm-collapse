...
$\Psi(x_0)=(1-t)x_0+tx^i$ has $\det D\Psi=(1-t)^d$ and $\Psi^{-1}(x)=\frac{x-tx^i}{1-t}$, so
$\sigma(X_t\mid I=i)=\sigma(X_0\mid I=i)$ and, as $X_0\sim\pi_0\perp I$,
\begin{equation*}
p(x\mid I=i,t)=p_{X_0}\big(\Psi^{-1}(x)\big)\,|\det D\Psi|^{-1}=(1-t)^{-d}\,\pi_0\!\Big(\frac{x-tx^i}{1-t}\Big).
\end{equation*}
$(1-t)^{-d}$ does not depend on $i$ and cancels in $\Pr(I=i\mid X_t=x,t)$; $\pi_0>0$ gives positivity.
\end{proof}
...
(b) Local existence and uniqueness on $[0,\tau)$: Picard--Lindel\"of \citep[Ch.~II]{hartman2002ode}. Since $w_i\ge0$ and $\sum_iw_i=1$,
\begin{equation}
|v(x,t)|\le\sum_iw_i\,\frac{|x^i-x|}{1-t}\le\frac{M+|x|}{1-t}.
\label{eq:vgrowth}
\end{equation}
For $g(t)=|x_t|+M$, $g'\le|v(x_t,t)|\le g/(1-t)$, so by Gr\"onwall
\begin{equation*}
g(t)\le g(0)\exp\int_0^t\frac{ds}{1-s}=\frac{g(0)}{1-t}<\infty\quad(t<1),
\end{equation*}
and $\tau=1$.

(c) For $\varphi\in C_c^\infty(\R^d)$ and $\mu_t=(\Phi_t)_\#\mu_0$,
\begin{equation*}
\frac{d}{dt}\int\varphi\,d\mu_t=\int\nabla\varphi(\Phi_t)\cdot v(\Phi_t,t)\,d\mu_0=\int\nabla\varphi\cdot v\,d\mu_t,
\end{equation*}
the weak form of $\partial_t\mu_t+\nabla\!\cdot(v\mu_t)=0$. Uniqueness: let $(\mu_t)_{t\le T}$, $T<1$, be a narrowly continuous solution and $\psi(x,s)=\varphi(\Phi_{s,T}(x))$, which is $C^1$, compactly supported in $x$, and solves
\begin{equation*}
...
\end{equation*}
so $\int\varphi\,d\mu_T=\int\varphi\circ\Phi_{0,T}\,d\mu_0$, i.e.\ $\mu_T=(\Phi_T)_\#\mu_0$ \citep[see also][Ch.~8]{ambrosio2008gradient}.
\end{proof}
...
By Bayes, Lemma~\ref{lem:cov} and $p_i^{(h)}(y)\propto K_h(y-y^i)$,
\begin{equation*}
\Pr(I=i\mid X_t=x,t)\propto p_i^{(h)}(y)\,(1-t)^{-d}\pi_0\!\Big(\frac{x-tx^i}{1-t}\Big)\propto\pi_0\!\Big(\frac{x-tx^i}{1-t}\Big)K_h(y-y^i),
\end{equation*}
the numerator of $w_i^{(h)}$ in \eqref{eq:kernel-field}; both are normalised over $i$, so $\Pr(I=i\mid X_t=x,t)=w_i^{(h)}$. As $X_1-X_0=\frac{x^i-x}{1-t}$ given $I=i$,
\begin{equation*}
\E[X_1-X_0\mid X_t=x,t]=\sum_iw_i^{(h)}\,\frac{x^i-x}{1-t}=\vstar(x,t,y).\qedhere
\end{equation*}
\end{proof}
...
\end{split}
\end{equation*}
since $\E\langle U-m,m-v\rangle=\E\langle\E[U-m\mid\mathcal G],m-v\rangle=0$. The last term vanishes iff $v=m$ a.e.
\end{proof}
...
\begin{equation}
U=x^i-X_0=\frac{x^i-x}{1-t},
\label{eq:pf:collapse:2}
\end{equation}
a function of the conditioning variables, so $v_0^\star(x,t,y^i)=\E[U\mid X_t=x,t,Y=y^i]=\frac{x^i-x}{1-t}$ by Proposition~\ref{prop:projection}: this is (a). By \eqref{eq:pf:collapse:2}, $U-v_0^\star(X_t,t,Y)=0$ a.s., so $L_0(v_0^\star)=0=\inf_vL_0$: (b).

Along $\dot x_t=\frac{x^i-x_t}{1-t}$,
\begin{equation*}
\frac{d}{dt}\,\frac{x_t-x^i}{1-t}=\frac{\dot x_t(1-t)+x_t-x^i}{(1-t)^2}=0
\ \Longrightarrow\ x_t=(1-t)x_0+tx^i\to x^i,
\end{equation*}
so $p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$ and $\Cov=0$: (c), (d).
\end{proof}
...
\begin{equation*}
q_i(x,t)=\frac{\pi_0\big(\frac{x-tx^i}{1-t}\big)}{\sum_j\pi_0\big(\frac{x-tx^j}{1-t}\big)}>0 .
\end{equation*}
Given $I=i$, $U=\frac{x^i-x}{1-t}$ by \eqref{eq:pf:collapse:2}, so by Proposition~\ref{prop:projection}
\begin{equation*}
v_{\mathrm{unc}}^\star(x,t)=\E[U\mid X_t=x,t]=\sum_iq_i(x,t)\,\frac{x^i-x}{1-t}.
\end{equation*}
This has the form of Lemma~\ref{lem:wellposed}; as in the proof of Theorem~\ref{thm:endpoint} (with the uniform law on $I$), $p_t=\mathrm{Law}(X_t)$, and $X_t\to X_1\sim\rhohat_X$ a.s., so $p_1^{\mathrm{unc}}=\rhohat_X$.

With $u_i=\frac{x^i-x}{1-t}$ and $\bar u=\sum_iq_iu_i$,
\begin{equation*}
\Var(U\mid X_t=x,t)=\sum_iq_i\,|u_i-\bar u|^2>0
\end{equation*}
...
By Proposition~\ref{prop:collapse}(d), $p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$, $\Cov=0$. By Proposition~\ref{prop:uncond},
\begin{equation*}
...
Given $I=i$, $X_t=(1-t)X_0+tx^i$ and $\widetilde Y=y^i+h\varepsilon$ with $X_0\perp\varepsilon$, so by Bayes and Lemma~\ref{lem:cov}
\begin{equation*}
\Pr(I=i\mid X_t=x,t,\widetilde Y=y)=\frac{\pi_0\big(\frac{x-tx^i}{1-t}\big)K_h(y-y^i)}{\sum_j\pi_0\big(\frac{x-tx^j}{1-t}\big)K_h(y-y^j)}=w_i^{(h)}
\end{equation*}
($\frac1N(1-t)^{-d}$ cancels). Given $I=i$ and $X_t=x$, $U=\frac{x^i-x}{1-t}$ by \eqref{eq:pf:collapse:2}, so by Proposition~\ref{prop:projection}
\begin{equation*}
\vstar(x,t,y)=\E[U\mid X_t=x,t,\widetilde Y=y]=\sum_iw_i^{(h)}(x,t,y)\,\frac{x^i-x}{1-t}.\qedhere
\end{equation*}
\end{proof}
...
By Lemma~\ref{lem:mixture-coupling}, $\vstar(x,t,y)=\E[X_1-X_0\mid X_t=x,t]$ for the coupling $I\sim p^{(h)}(y)$, $X_1=x^I$. The weights $w_i^{(h)}$ are smooth, positive and sum to one, so by Lemma~\ref{lem:wellposed} the flow $(\Phi_t)_{t<1}$ exists and the continuity equation
\begin{equation}
\partial_tp_t+\nabla\!\cdot\big(p_t\vstar\big)=0,\qquad p_0=\pi_0,
\label{eq:pf:endpoint:2}
\end{equation}
has a unique solution. $(\Phi_t)_\#\pi_0$ solves it, and so does $\mathrm{Law}(X_t)$ (superposition); hence $(\Phi_t)_\#\pi_0=\mathrm{Law}(X_t)=:p_t$. Since $X_t=(1-t)X_0+tx^I\to x^I$ a.s.,
\begin{equation*}
p_1=\lim_{t\uparrow1}p_t=\mathrm{Law}(x^I)=\sum_ip_i^{(h)}(y)\,\delta_{x^i}.\qedhere
\end{equation*}
\end{proof}
...
\end{equation*}
so $p_i^{(h)}(y)\to\mathbf 1\{i=i^\star\}$, and $p_h^{\mathrm{gen}}\Rightarrow\delta_{x^{i^\star}}$ by Theorem~\ref{thm:endpoint}.
\end{proof}
...
Uniformly in $i$, $K_h(y-y^i)=(2\pi h^2)^{-k/2}\big[1+O(h^{-2})\big]$; the prefactor cancels,
\begin{equation*}
p_i^{(h)}(y)=\frac{1+O(h^{-2})}{N+O(h^{-2})}\longrightarrow\frac1N,
\end{equation*}
so $p_1\to\frac1N\sum_i\delta_{x^i}=\rhohat_X$ by Theorem~\ref{thm:endpoint}.
\end{proof}
...
By Theorem~\ref{thm:endpoint} the endpoint law is $\sum_ip_i^{(h)}\delta_{x^i}$, so
\begin{equation*}
\bar x_h(y)=\sum_ip_i^{(h)}x^i,\qquad
\Covh(y)=\sum_ip_i^{(h)}(x^i-\bar x_h)(x^i-\bar x_h)^\top .
\end{equation*}
Non-monotonicity: take $k=d=1$, $y=0$, $(y^i,x^i)=(0,0),(1,10),(10,0)$. Then
\begin{equation*}
\begin{split}
\tr\Covh&\to0\ (h\to0),\qquad\tr\Covh=24.90\ (h=2),\\
\tr\Covh&\to\tr\Xhat=\tfrac{200}{9}=22.22\ (h\to\infty),
\end{split}
\end{equation*}
with $p^{(h)}\to(1,0,0)$, $p^{(2)}=(0.531,\,0.469,\,2{\times}10^{-6})$, $p^{(h)}\to(\frac13,\frac13,\frac13)$ (Corollaries~\ref{cor:zerobw}, \ref{cor:infbw}).
\end{proof}
...
By Theorem~\ref{thm:endpoint}, $p_h^{\mathrm{gen}}=\sum_ip_i^{(h)}\delta_{x^i}$, so $\operatorname{supp}p_h^{\mathrm{gen}}\subseteq\{x^i\}$. For any coupling $(X,X')$ of $p_h^{\mathrm{gen}}$ and $p(\cdot\mid y)$, $X\in\{x^i\}$ a.s., so
\begin{equation*}
\E|X-X'|^2\ge\E\operatorname{dist}(X',\{x^i\})^2=\int\operatorname{dist}(x,\{x^i\})^2\,p(x\mid y)\,dx=:F(y).
\end{equation*}
...
Given $I=i$, $r:=x-tx^i=(1-t)X_0+t\rho\xi\sim\N(0,s_t^2I_d)$ with $s_t^2=(1-t)^2+t^2\rho^2$, so $p(x\mid I=i,t)=\varphi_{s_t}(x-tx^i)$ and, as in Proposition~\ref{prop:kernel-field},
\begin{equation*}
w_i=\Pr(I=i\mid X_t=x,t,\widetilde Y=y)\propto\varphi_{s_t}(x-tx^i)\,K_h(y-y^i).
\end{equation*}
$(X_0,\xi,r)$ is jointly Gaussian: $\E[X_0\mid r]=\frac{1-t}{s_t^2}r$, $\E[\xi\mid r]=\frac{t\rho}{s_t^2}r$. With $U=x^i+\rho\xi-X_0$,
\begin{equation}
\E[U\mid X_t=x,t,I=i]=x^i+\frac{t\rho^2-(1-t)}{s_t^2}\,(x-tx^i),
\label{eq:pf:tgtnoise:4}
\end{equation}
so by Proposition~\ref{prop:projection}
\begin{equation}
\vstar(x,t,y)=\sum_iw_i\Big[x^i+\frac{t\rho^2-(1-t)}{s_t^2}\,(x-tx^i)\Big].
\label{eq:tgtfield}
\end{equation}
For the coupling $I\sim p^{(h)}(y)$, $X_0\sim\pi_0$, $\xi\sim\N(0,I_d)$ independent, $X_1=x^I+\rho\xi$, the index posterior is $\propto p_i^{(h)}\varphi_{s_t}(x-tx^i)\propto w_i$ and \eqref{eq:pf:tgtnoise:4} holds, so its conditional velocity is $\vstar$. As in the proof of Theorem~\ref{thm:endpoint}, and since $X_t\to X_1$ a.s.,
\begin{equation*}
p_1=\mathrm{Law}(x^I+\rho\xi)=\sum_ip_i^{(h)}(y)\,\N(x^i,\rho^2I_d),
\end{equation*}
with mean $\bar x_h(y)$ and covariance $\rho^2I_d+\Covh(y)$ (total covariance). It is absolutely continuous, so \eqref{eq:w2floor} does not apply. Since $s_1=\rho>0$, $\frac{t\rho^2-(1-t)}{s_t^2}\to1$ and $\vstar$ is bounded on bounded sets up to $t=1$.
\end{proof}
...
Given $I=i$ (pinned by $Y=y^i$), $r:=x-tx^i=(1-t)X_0+\gamma Z\sim\N(0,s_t^2I_d)$, $s_t^2=(1-t)^2+\gamma^2$; $\Cov(X_0,r)=(1-t)I_d$, $\Cov(Z,r)=\gamma I_d$, so $\E[X_0\mid r]=\frac{1-t}{s_t^2}r$, $\E[Z\mid r]=\frac{\gamma}{s_t^2}r$. With $\dot X_t=x^i-X_0+\dot\gamma Z$,
\begin{equation*}
v_\sigma^\star(x,t,y^i)=x^i+\frac{-(1-t)+\dot\gamma\gamma}{s_t^2}\,r,\qquad
\dot\gamma\gamma=\tfrac12(\gamma^2)'=\tfrac{\sigma^2}2(1-2t),
\end{equation*}
which is (a) with $c(t)=\frac{-(1-t)+\frac{\sigma^2}2(1-2t)}{s_t^2}$; and $(s_t^2)'=-2(1-t)+2\dot\gamma\gamma=2s_t^2c(t)$, so $c=\frac12(\log s_t^2)'$.

(b) $u_t=x_t-tx^i$ satisfies $\dot u_t=c(t)u_t$, so
\begin{equation*}
u_t=x_0\exp\!\big(\tfrac12[\log s_t^2-\log s_0^2]\big)=s_t\,x_0,\qquad x_t=tx^i+s_tx_0 .
\end{equation*}
(c) $s_t^2=(1-t)[(1-t)+\sigma^2t]$, so $s_1=0$ and $x_t\to x^i$ for all $x_0$, $\sigma\ge0$.
\end{proof}
...
Endpoint: Proposition~\ref{prop:prop17}(c). For $\sigma=0$, $\dot X_t=X_1-X_0$ and $\inf_vL_0=0$ (Proposition~\ref{prop:collapse}). For $\sigma>0$, $\inf_vL_\sigma=\E\Var(\dot X_t\mid X_t,t,Y)$; with $u=(-1,\dot\gamma)$, $w=(1-t,\gamma)$, $|w|^2=s_t^2$, $u\cdot w=c\,s_t^2$,
\begin{equation*}
\Var(\dot X_t\mid r,I=i)=|u|^2-\frac{(u\cdot w)^2}{|w|^2}=1+\dot\gamma^2-c(t)^2s_t^2 .
\end{equation*}
It vanishes iff $u\parallel w$, i.e.\ $\dot\gamma(1-t)=-\gamma$; with $\gamma=\sigma\sqrt{t(1-t)}$ this is $1-2t=-2t$, impossible. So $\inf_vL_\sigma>0$. Finally
\begin{equation*}
(1-t)\,c(t)=\frac{-(1-t)+\frac{\sigma^2}2(1-2t)}{(1-t)+\sigma^2t}\longrightarrow-1\ (\sigma=0),\qquad-\tfrac{\sigma^2/2}{\sigma^2}=-\tfrac12\ (\sigma>0).\qedhere
\end{equation*}
\end{proof}
...
Label factor: $p_h^{\mathrm{gen}}=\sum_ip_i^{(h)}\delta_{x^i}$ depends on $h$ through $K_h(y-y^i)$, and tends to $\delta_{x^{i^\star}}$ ($h\to0$, unique nearest label) and to $\rhohat_X$ ($h\to\infty$) by Corollaries~\ref{cor:zerobw}, \ref{cor:infbw}: distinct laws. Spatial factor: for $h=0$ and any $\sigma\ge0$,
\begin{equation*}
\N\big(tx^i,(1-t)^2I_d\big)\longrightarrow\N\big(tx^i,s_t^2I_d\big)\ \ \forall i,\qquad p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}\ \ \forall\sigma
\end{equation*}
...
\begin{equation}
\tilde q_h(u)=\frac{\varphi(u)\rho_Y(y+hu)}{\int\varphi\,\rho_Y(y+h\cdot)}=\varphi(u)\big[1+h\,s^\top u+O(h^2)\big].
\label{eq:pf:expansion:3}
\end{equation}
With $\E_\varphi[u]=0$, $\E_\varphi[uu^\top]=I_k$, $\E_\varphi[u_au_bu_c]=0$, $\E_\varphi[u_au_bu_cu_d]=\delta_{ab}\delta_{cd}+\delta_{ac}\delta_{bd}+\delta_{ad}\delta_{bc}$,
\begin{equation}
\E_{\tilde q_h}[u]=h\,s+O(h^2),\qquad
\E_{\tilde q_h}[uu^\top]=I_k+O(h^2),\qquad
...
\begin{equation*}
\int_{|u|>r/h}(1+|y+hu|^{2m})\varphi(u)\,du\le C\,e^{-r^2/4h^2}=o(h^p)\ \ \forall p,
\end{equation*}
so $\E_h[\Sigma(Y)]=\Sigma(y)+O(h^4)$. Without homoscedasticity, for $\Sigma\in C^2$, \eqref{eq:pf:expansion:6} gives
\begin{equation*}
\E_h[\Sigma(Y)]=\Sigma+h^2\big[\nabla\Sigma\cdot s+\tfrac12\tr\nabla^2\Sigma\big]+o(h^2),
\end{equation*}
of the order of $h^2JJ^\top$: e.g.\ $k=d=1$, $Y\sim\mathrm{Unif}(-2,2)$, $X\mid Y=y\sim\N(0,1+y^2)$ has $J\equiv0$ and $\Covh(y)=\Sigma(y)+h^2$.

\emph{Between-group.} Since $\mu\in C^3$,
\begin{equation*}
\begin{split}
\mu(y+hu)&=\mu(y)+hJu+\tfrac{h^2}2Q(u)+R_h(u),\\
Q(u)_a&=\sum_{b,c}\partial^2_{bc}\mu_a(y)u_bu_c,\qquad|R_h|\le\tfrac{M_3}6h^3|u|^3 .
\end{split}
\end{equation*}
The moments of $\tilde q_h$ are bounded for $h\le1$, so the covariances involving $R_h$ are $O(h^4),O(h^5),O(h^6)$ and
\begin{equation*}
\Cov_h[\mu(Y)]=h^2J\,\Cov[u]\,J^\top+h^3\big[J\,\Cov(u,Q)+\mathrm{sym}\big]+h^4\Cov[Q]+O(h^4).
\end{equation*}
By \eqref{eq:pf:expansion:3}, $\E_{\tilde q_h}[u_au_bu_c]=h\,[s_c\delta_{ab}+s_b\delta_{ac}+s_a\delta_{bc}]+O(h^2)$, so $\Cov(u,Q)=O(h)$, and with \eqref{eq:pf:expansion:6}
\begin{equation}
\Cov_h[\mu(Y)]=h^2JJ^\top+O(h^4),\qquad
\Cov_h^{\mathrm{pop}}=\Sigma(y)+h^2JJ^\top+O(h^4).
\label{eq:pf:expansion:1}
\end{equation}

\emph{Fluctuation.} With $S_N=\frac1N\sum_jK_h(y-y^j)$, $T_N=\frac1N\sum_jK_h(y-y^j)x^j$, $\bar x_h=T_N/S_N$,
\begin{equation*}
\Var\big(K_h(y-Y)\big)=\Theta(h^{-k}),\qquad\Var\big(K_h(y-Y)XX^\top\big)=\Theta(h^{-k})
\end{equation*}
(the latter uses $m_4$ bounded near $y$: the only use of (iv)), so $S_N,T_N=\E[\cdot]+O_P(\sqrt{h^{-k}/N})$ and, by Slutsky,
\begin{equation*}
\bar x_h=\E_h[X]+O_P\big((Nh^k)^{-1/2}\big),\qquad\Covh=\Cov_h^{\mathrm{pop}}+O_P\big((Nh^k)^{-1/2}\big).
\end{equation*}
...
Under Assumption~\ref{as:design}, $K$ bounded and symmetric with $\int K=1$, $h\to0$, $Nh^k\to\infty$ \citep[\S1.5--1.6]{tsybakov2009nonparametric}, \citep[Ch.~5]{gyorfi2002distribution}, with $\mu_2(K)=\int u_1^2K$, $R(K)=\int K^2$,
\begin{equation*}
\mathrm{Bias}=\tfrac{h^2}2\mu_2(K)\,b(y)+o(h^2),\qquad
...
\begin{equation*}
\tfrac{d}{dh}\mathrm{MSE}=4c_1h^3-c_2\,k\,N^{-1}h^{-k-1}=0\iff h^{k+4}=\frac{c_2k}{4c_1N},
\end{equation*}
a minimum: $h^\star\asymp N^{-1/(k+4)}$ ($c_1,c_2>0$).

Linear-Gaussian instance: $\mu(y')=By'$, and in $y'$, $K_h(y-y')\rho_Y(y')\propto\N\big(\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}y,\,\cdot\big)$, so
\begin{equation*}
\begin{split}
\bar x_h(y)&=\frac{\E[\mu(Y)K_h(y-Y)]}{\E[K_h(y-Y)]}=B\,\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}y,\\
\bar x_h(y)-By&=-B\,h^2(\Sigma_Y+h^2I_k)^{-1}y=-h^2B\Sigma_Y^{-1}y+O(h^4),
\end{split}
\end{equation*}
which is \eqref{eq:lgbias}; with $\Delta\mu=0$, $\nabla\log\rho_Y=-\Sigma_Y^{-1}y$: $b(y)=-2B\Sigma_Y^{-1}y$ (checked in \texttt{scripts/verify\_nw\_bias\_affine.py}).

Design points: at $y=y^i$ the weight of $j=i$ is $K_h(0)/\sum_jK_h(y^i-y^j)=\Theta\big((Nh^k)^{-1}\big)\to0$; if $Nh^k\not\to\infty$ it does not vanish (Corollary~\ref{cor:zerobw}: it tends to $1$).
\end{proof}
...
\begin{equation*}
\E|f_i(X)-f_i(X')|^2=\frac{\E|X-X'|^2}{(1-t)^2}=2d,\qquad
...
\begin{equation*}
\sqrt{2d}\le\sqrt{2\,\E|e(X)|^2}+\sqrt{2d}\,L(1-t)\ \Longrightarrow\ \E|e(X)|^2\ge d\,[1-L(1-t)]_+^2 .
\end{equation*}
(The identity for $\E|e(X)-e(X')|^2$, not the bound $4\E|e|^2$, keeps the constant $d$.)
\end{proof}
...
$\int_0^{1/L}(1-Ls)^2\,ds=\big[s-Ls^2+\tfrac{L^2s^3}3\big]_0^{1/L}=\frac1{3L}$. Proposition~\ref{prop:liplb} holds for every $i$, $v\in\mathcal F_L$ and $t\ge1-1/L$; averaging over $I$ and $t$,
\begin{equation*}
\inf_{v\in\mathcal F_L}L_0(v)\ge d\int_{1-1/L}^1[1-L(1-t)]^2\,dt=\frac d{3L}.\qedhere
\end{equation*}
\end{proof}
...
By Propositions~\ref{prop:kernel-field}, \ref{prop:uncond}, $\vstar=\sum_iw_i^{(h)}\frac{x^i-x}{1-t}$ and $v_{\mathrm{unc}}^\star=\sum_iq_i\frac{x^i-x}{1-t}$. In $v_w=(1+w)\vstar-w\,v_{\mathrm{unc}}^\star$ the $x$-terms give $-(1+w)x+wx=-x$, so $v_w=\frac{m_w-x}{1-t}$ with coefficients $(1+w)w_i^{(h)}-wq_i$ summing to $1$: $m_w\in\mathcal A$. For the orthogonal projection $\Pi$ onto $\mathcal A$ (affine, $\Pi m_w=m_w$),
\begin{equation*}
\dot q_t=-\frac{q_t}{1-t}\quad\Longrightarrow\quad q_t=(1-t)\,q_0 ,
\end{equation*}
so $\operatorname{dist}(x_t,\mathcal A)\to0$ and, $\mathcal A$ being closed, the endpoint law is supported in $\mathcal A$. Well-posedness: $|m_w|\le(1+2w)\max_i|x^i|$, so \eqref{eq:vgrowth} holds with $M\to(1+2w)M$.
\end{proof}
...
(a)
\begin{equation*}
\sum_{j\notin C}p_j^{(h)}\le\frac{\sum_{j\notin C}K_j}{\sum_{i\in C}K_i}\le\frac{(N-|C|)\,e^{-\Delta^2/2h^2}}{|C|\,e^{-\varepsilon^2/2h^2}}=\eta .
\end{equation*}
(b) For $i,i'\in C$,
\begin{equation*}
\frac{p_i^{(h)}}{p_{i'}^{(h)}}=\frac{K_i}{K_{i'}}=\exp\frac{|y-y^{i'}|^2-|y-y^i|^2}{2h^2}\le e^{\varepsilon^2/2h^2}=\kappa .
\end{equation*}
(c) Let $S=\sum_{i\in C}p_i^{(h)}$. By (b), $|C|\,p_i^{(h)}\le\kappa S$, so
\begin{equation*}
\sum_{i\in C}(p_i^{(h)})^2\le S\max_{i\in C}p_i^{(h)}\le\frac\kappa{|C|},\qquad\sum_{j\notin C}(p_j^{(h)})^2\le\eta^2,
\end{equation*}
and $n_{\mathrm{eff}}=\big(\sum_ip_i^{(h)2}\big)^{-1}\ge\frac{|C|}{\kappa+|C|\eta^2}$. As $\varepsilon/h\to0$, $\Delta/h\to\infty$: $\kappa\to1$, $\eta\to0$, $S\ge1-\eta\to1$ and $p_i/p_{i'}\in[\kappa^{-1},\kappa]$, so the posterior tends to the uniform law on $C$ and, by Theorem~\ref{thm:endpoint}, the endpoint law to $|C|^{-1}\sum_{i\in C}\delta_{x^i}$. Also $\sum_ip_i^{2}\ge S^2/|C|$ gives $n_{\mathrm{eff}}\le|C|/S^2\to|C|$.
\end{proof}
...
Suppose $m_w(x_t,t)\to m^*\ne x^*$; put $u=\frac{m^*-x^*}{|m^*-x^*|}$, $c=\frac{|m^*-x^*|}2$. For $t\ge t_0$, $\langle m_w-x_t,u\rangle\ge c$, so
\begin{equation*}
\frac d{dt}\langle x_t,u\rangle=\frac{\langle m_w-x_t,u\rangle}{1-t}\ge\frac c{1-t}
\ \Longrightarrow\ \langle x_t,u\rangle\ge\langle x_{t_0},u\rangle+c\log\tfrac{1-t_0}{1-t}\to+\infty,
\end{equation*}
...
\end{equation}
$w_j^{(h)}$ and $q_j$ both have the form (Lemma~\ref{lem:cov})
\begin{equation*}
\alpha_j=\frac{c_j\,e^{-|x-tx^j|^2/2(1-t)^2}}{\sum_lc_l\,e^{-|x-tx^l|^2/2(1-t)^2}},\qquad c_j=K_h(y-y^j)\ \text{resp.}\ \tfrac1N .
\end{equation*}
As $x_t\to x^*$ and the nearest atom $x^{i^*}$ is unique,
\begin{equation*}
|x_t-tx^j|^2-|x_t-tx^{i^*}|^2\to2\kappa_j>0\ \Longrightarrow\ \frac{\alpha_j}{\alpha_{i^*}}\le\frac{c_j}{c_{i^*}}\,e^{-\kappa_j/2(1-t)^2}\to0,
\end{equation*}
so $\alpha_{i^*}\to1$ and $\sum_j\alpha_jx^j\to x^{i^*}$, for both families with the same $i^*$.

(a) $h>0$: $m_w=(1+w)\bar x_h-w\,\bar x_{\mathrm{unc}}\to(1+w)x^{i^*}-w\,x^{i^*}=x^{i^*}$, so $x^*=x^{i^*}$ by \eqref{eq:pf:cfgatom:1}.
...
\label{eq:cfgfixed}
\end{equation}
Let $i^*\ne i$. If $w=0$, $x^*=x^i$ and $i^*=i$ (uniqueness), a contradiction. If $w>0$, $x^*-x^i=w(x^i-x^{i^*})$ and $x^*-x^{i^*}=(1+w)(x^i-x^{i^*})$, so
\begin{equation*}
...
$e_0=0$, and $\vstarz$ is affine in $x$ with linear part $-\frac1{1-t}I_d$, so
\begin{equation*}
\dot e_t=\Delta(x^v_t,t)+\big[\vstarz(x^v_t,t,y^i)-\vstarz(x^\star_t,t,y^i)\big]=\Delta(x^v_t,t)-\frac{e_t}{1-t}.
\end{equation*}
Hence, a.e.,
\begin{equation*}
\frac d{dt}\Big[\frac{e_t}{1-t}\Big]=\frac{\dot e_t}{1-t}+\frac{e_t}{(1-t)^2}=\frac{\Delta(x^v_t,t)}{1-t},
\end{equation*}
and integrating with $e_0=0$ gives \eqref{eq:survival-id}; the triangle inequality gives \eqref{eq:survival}.

(a) By Cauchy--Schwarz,
\begin{equation*}
|e_t|\le(1-t)\Big(\int_0^t|\Delta|^2\Big)^{1/2}\Big(\int_0^t\frac{ds}{(1-s)^2}\Big)^{1/2}\le(1-t)\,\varepsilon\sqrt{\tfrac t{1-t}}=\varepsilon\sqrt{t(1-t)}\le\varepsilon\sqrt{1-t}.
\end{equation*}
(b) $\int_0^t(1-s)^{-1-a}ds=\frac1a\big((1-t)^{-a}-1\big)$, so $|e_t|\le\frac ca\big((1-t)^{1-a}-(1-t)\big)$, which is $c\,t\le c$ for $a=1$, and tends to $0$, $c$, $+\infty$ for $a<1$, $a=1$, $a>1$.

(c) For $\Delta=c(1-t)^{-a}u$, $|u|=1$, the integrand is trajectory-free:
\begin{equation*}
e_t=\begin{cases}\frac ca\big((1-t)^{1-a}-(1-t)\big)u,&a>0,\\[2pt]-c\,(1-t)\log(1-t)\,u,&a=0,\end{cases}
\end{equation*}
which is \eqref{eq:survival-sharp}. The direction is fixed, so \eqref{eq:survival} is an equality, and with $\epsilon=1-t$, $|e_t|=\frac ca(\epsilon^{1-a}-\epsilon)\to0,\ c,\ \infty$. Also $v=\vstarz+\Delta$ is locally Lipschitz with $L_\Delta=0$, so Assumption~\ref{as:lipschitz} holds.

(d) With $I(t)=\int_0^t\frac{\Delta(x^v_s,s)}{1-s}ds$, $e_t=(1-t)I(t)$ and
\begin{equation*}
e_t-e_\star=(1-t)\Big[I(t)-\frac{e_\star}{1-t}\Big],
\end{equation*}
so $e_t\to e_\star\iff I(t)=\frac{e_\star}{1-t}+o\big((1-t)^{-1}\big)$: \eqref{eq:survival-conv}. The four cases are the behaviours of $(1-t)I(t)$.
\end{proof}
...
\begin{equation*}
\mathcal L_i(v)=\int_0^1\ell(s)^2ds,\qquad\ell(s)^2=\E_{X_0}|\Delta(x^\star_s,s)|^2 .
\end{equation*}
Taking $L^2(X_0)$ norms in \eqref{eq:survival} (Minkowski) and using Assumption~\ref{as:lipschitz},
\begin{equation*}
\mathcal E(t)\le(1-t)\int_0^t\frac{\|\Delta(x^v_s,s)\|_{L^2}}{1-s}\,ds,\qquad
\|\Delta(x^v_s,s)\|_{L^2}\le\ell(s)+L_\Delta\,\mathcal E(s).
\end{equation*}
With $F=\frac{\mathcal E}{1-t}$ and $G=\int_0^t\frac{\ell(s)}{1-s}ds$: $F(t)\le G(t)+L_\Delta\int_0^tF$, so by Gr\"onwall ($G$ nondecreasing) $F\le G\,e^{L_\Delta t}$. By Cauchy--Schwarz,
\begin{equation*}
G(t)\le\Big(\int_0^t\ell^2\Big)^{1/2}\Big(\int_0^t\frac{ds}{(1-s)^2}\Big)^{1/2}\le\sqrt{\mathcal L_i(v)}\,\sqrt{\tfrac t{1-t}},
\end{equation*}
so $\mathcal E(t)=(1-t)F\le e^{L_\Delta t}\sqrt{\mathcal L_i(v)}\sqrt{t(1-t)}$, which is \eqref{eq:lossform} ($t=1-\delta$). Finally $|x^\star_{1-\delta}-x^i|=\delta\,|X_0-x^i|$, and the triangle inequality in $L^2(X_0)$ gives the last display.
\end{proof}
...
(a) $\ell_i(\hat t)=\ell_j(\hat t)\iff z^j=z^i+\frac{1-\hat t}{\hat t}(x_0^i-x_0^j)$. With $s=\frac{1-t}t\in(0,\infty)$,
\begin{equation*}
\{\exists\,t:\ell_i(t)=\ell_j(t)\}=\{z^j\in R\},\qquad R=\{z^i+s\,(x_0^i-x_0^j):s>0\}.
\end{equation*}
...
(b) $z^j\in\big\{[(1-t_i)x_0^i+t_iz^i-(1-t_j)x_0^j]/t_j:(t_i,t_j)\in(0,1)^2\big\}$, a countable union of Lipschitz images of bounded planar sets, of Hausdorff dimension $\le2<d$, hence null.
\end{proof}
...
\end{equation*}
Then $v(X_t^{(i,j)},t^{(i,j)})=z^i-x_0^i$ for all $(i,j)$, and $L_{\mathrm{MC}}^{\mathrm{unc}}(v)=0$.
\end{proof}
...
\begin{equation*}
|f_\theta(t)|\le|f_\theta(t_0)|+K|t-t_0|\le|f_\theta(t_0)|+K\qquad(t\in[0,1)),
\end{equation*}
so $f_\theta$ is bounded on $[0,1)$, while $g(t)=\frac1{1-t}\to\infty$: $\sup_{t\in[0,1)}|f_\theta(t)-g(t)|=\infty$.
\end{proof}
