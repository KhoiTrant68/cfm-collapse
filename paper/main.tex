...
\begin{align}
\det D\Psi&=(1-t)^d\\
\Psi^{-1}(x)&=\frac{x-tx^i}{1-t}\label{eq:pf:cov:1}\\
\sigma(X_t\mid I=i)&=\sigma(X_0\mid I=i)
\end{align}
Justifications, in order: (1)~linear part is $(1-t)I_d$, invertible for $t<1$; (2)~solve for $x_0$; (3)~a bijection generates the same $\sigma$-algebra.

\textbf{Step 2: the density.}
\begin{align}
p_{X_t}(x)&=p_{X_0}\big(\Psi^{-1}(x)\big)\,|\det D\Psi|^{-1}\\
&=(1-t)^{-d}\,\pi_0\!\big(\tfrac{x-tx^i}{1-t}\big)\label{eq:pf:cov:2}
\end{align}
Justifications, in order: (1)~change of variables; (2)~$\eqref{eq:pf:cov:1};\ X_0\sim\pi_0\perp I$.
...
p(x\mid I=i,t)&>0
\end{align}
Justifications, in order: (1)~cancels in $\Pr(I=i\mid X_t=x,t)$, by \eqref{eq:pf:cov:2}; (2)~$\pi_0>0\ \text{(Gaussian)},\ \Psi^{-1}\ \text{bijective}$.
...
\end{align}
Justifications, in order: (1)~$w_i\in C^\infty,\ (x^i-x)/(1-t)\in C^\infty$; (2)~so $v$ is locally Lipschitz in $x$.
...
\end{align}
Justifications, in order: (1)~$w_i\ge0,\ \textstyle\sum_iw_i=1$; (2)~$g\ \text{Lipschitz, a.e.\ differentiable};\ \eqref{eq:vgrowth}$; (3)~Gr\"onwall.
...
&=\int\nabla\varphi\cdot v\,d\mu_t\label{eq:pf:wellposed:2}
\end{align}
Justifications, in order: (1)~chain rule along the flow; (2)~push-forward.
...
\begin{align}
\partial_s\psi+v\cdot\nabla\psi&=0,\quad\psi(\cdot,T)=\varphi\label{eq:pf:wellposed:3}\\
\frac{d}{ds}\int\psi(\cdot,s)\,d\mu_s&=\int(\partial_s\psi+v\cdot\nabla\psi)\,d\mu_s=0\label{eq:pf:wellposed:4}\\
\int\varphi\,d\mu_T&=\int\psi(\cdot,0)\,d\mu_0\\
&=\int\varphi\,d\big((\Phi_T)_\#\mu_0\big)
\end{align}
Justifications, in order: (1)~characteristics; (2)~test the equation; \eqref{eq:pf:wellposed:3}; (3)~$\eqref{eq:pf:wellposed:4}\text{ from $s=0$ to $T$}$; (4)~$\psi(\cdot,0)=\varphi\circ\Phi_{0,T}$.
...
\end{align}
Justifications, in order: (1)~Bayes; Lemma~\ref{lem:cov}; (2)~$p_i^{(h)}\propto K_h(y-y^i);\ (1-t)^{-d}\text{ free of }i$.

\textbf{Step 2: the two posteriors coincide.}
\begin{align}
\Pr(I=i\mid X_t=x,t)&=w_i^{(h)}(x,t,y)
\end{align}
Justification: $\eqref{eq:pf:mix:1}\ \text{is the numerator of \eqref{eq:kernel-field}; both normalise over $i$}$.
...
&=\vstar(x,t,y)
\end{align}
Justifications, in order: (1)~$X_1-X_0=\frac{x^i-x}{1-t}\ \text{given }I=i;\ \text{Step~2}$; (2)~$\eqref{eq:kernel-field}$.
...
\begin{align}
L_h(v)&=\E|(U-m)+(m-v)|^2\\
&=\E|U-m|^2+\E|m-v|^2+2\,\E\langle U-m,\,m-v\rangle\label{eq:pf:projection:1}
\end{align}
Justifications, in order: (1)~$U-v=(U-m)+(m-v)$; (2)~expand the square.

\textbf{Step 2: the cross term vanishes.}
\begin{align}
\E\langle U-m,\,m-v\rangle&=\E\big\langle\E[U-m\mid\mathcal G],\,m-v\big\rangle\\
&=0\label{eq:pf:projection:2}
\end{align}
Justifications, in order: (1)~tower property; $m-v$ is $\mathcal G$-measurable; (2)~$\E[U-m\mid\mathcal G]=0$.
...
\end{align}
Justifications, in order: (1)~$\eqref{eq:pf:projection:1},\ \eqref{eq:pf:projection:2}$; (2)~$\E|v-m|^2\ge0,\ =0\iff v=m\ \text{a.e.}$; (3)~unique a.e.\ minimiser.
...
\{Y=y^i\}&=\{I=i\}\\
X_1&=x^i\\
X_0&=\frac{x-tx^i}{1-t}\label{eq:pf:collapse:1}
\end{align}
Justifications, in order: (1)~Assumption~\ref{as:standing}(iii); (2)~$I=i$; (3)~Lemma~\ref{lem:cov}, given $X_t=x$.
...
&=\frac{x^i-x}{1-t}
\end{align}
Justifications, in order: (1)~$U=X_1-X_0$; (2)~$\eqref{eq:pf:collapse:1}$; (3)~common denominator; (4)~Proposition~\ref{prop:projection}; (5)~$\eqref{eq:pf:collapse:2};\ U\ \text{is measurable in the conditioning}$.
...
\inf_vL_0&=0
\end{align}
Justifications, in order: (1)~$\eqref{eq:pf:collapse:2}$; (2)~$\eqref{eq:objective}$; (3)~$L_0\ge0$.

\textbf{Step 4: parts (c) and (d).} Along $\dot x_t=(x^i-x_t)/(1-t)$,
\begin{align}
\frac{d}{dt}\frac{x_t-x^i}{1-t}&=\frac{\dot x_t(1-t)+(x_t-x^i)}{(1-t)^2}\\
&=0\label{eq:pf:collapse:3}\\
x_t&=(1-t)x_0+tx^i\\
x_t&\to x^i\quad(t\uparrow1),\ \text{for every }x_0\\
p_1^{\mathrm{cond}}(\cdot\mid y^i)&=\delta_{x^i},\quad\Cov=0
\end{align}
Justifications, in order: (1)~quotient rule; (2)~$\dot x_t(1-t)=x^i-x_t$; (3)~$\eqref{eq:pf:collapse:3},\ x_{t=0}=x_0$; (4)~coefficient of $x_0$ is $1-t\to0$; (5)~point mass.
...
\end{align}
Justifications, in order: (1)~Bayes; Lemma~\ref{lem:cov}; (2)~$\frac1N(1-t)^{-d}$ does not depend on $i$.

\textbf{Step 2: the minimiser.}
\begin{align}
U\mid(x,t,I=i)&=\frac{x^i-x}{1-t}\\
v_{\mathrm{unc}}^\star(x,t)&=\E[U\mid X_t=x,t]\\
&=\sum_iq_i(x,t)\,\frac{x^i-x}{1-t}\label{eq:pf:uncond:2}
\end{align}
Justifications, in order: (1)~Step~2 of the proof of Proposition~\ref{prop:collapse}; (2)~Proposition~\ref{prop:projection}; (3)~tower rule over \eqref{eq:pf:uncond:1}.
...
\end{align}
Justifications, in order: (1)~Lemma~\ref{lem:wellposed}: smooth weights, $\sum_iq_i=1$; (2)~Steps~2--3 of Theorem~\ref{thm:endpoint}, uniform law on $I$; (3)~$X_t\to X_1\ \text{a.s.},\ X_1\sim\rhohat_X$; (4)~weak limit.
...
\end{align}
Justifications, in order: (1)~$\pi_0>0,\ \eqref{eq:pf:uncond:1}$; (2)~$u\ \text{is injective affine in $x^i$}$; (3)~two distinct values with positive mass; (4)~Proposition~\ref{prop:projection}; \eqref{eq:pf:uncond:3} at every $(x,t)$.
...
\Cov[p_1^{\mathrm{unc}}]&=\tfrac1N\textstyle\sum_i(x^i-\bar x)(x^i-\bar x)^\top=\Xhat\label{eq:pf:distinction:1}
\end{align}
Justifications, in order: (1)~Proposition~\ref{prop:collapse}(d); (2)~Proposition~\ref{prop:uncond}; (3)~definition of $\Xhat$.
...
&=w_i^{(h)}
\end{align}
Justifications, in order: (1)~given $I=i$; (2)~$X_0\perp\varepsilon$; (3)~Lemma~\ref{lem:cov};\ density of $y^i+h\varepsilon$; (4)~$\eqref{eq:bayes-index}$; (5)~$\frac1N(1-t)^{-d}$ cancels.

\textbf{Step 3: the per-index target.}
\begin{align}
U\mid(X_t=x,I=i)&=\frac{x^i-x}{1-t}
\end{align}
Justification: Lemma~\ref{lem:cov}; Proposition~\ref{prop:collapse}(a).
...
\end{align}
Justifications, in order: (1)~Proposition~\ref{prop:projection}; (2)~Steps~2 and~3.
...
&=\sum_iw_i^{(h)}\,\frac{x^i-x}{1-t}\label{eq:pf:endpoint:1}
\end{align}
Justifications, in order: (1)~Lemma~\ref{lem:mixture-coupling}; (2)~definition of $w_i^{(h)}$.
...
\end{align}
Justifications, in order: (1)~ratio of smooth positive terms; $\pi_0>0$ (Lemma~\ref{lem:cov}); (2)~Lemma~\ref{lem:wellposed}.

\textbf{Step 3: the flow transports $\pi_0$ to $\mathrm{Law}(X_t)$.}
\begin{align}
\partial_tp_t+\nabla\!\cdot\!\big(p_t\vstar\big)&=0,\quad p_0=\pi_0\label{eq:pf:endpoint:2}\\
(\Phi_t)_\#\pi_0&=\mathrm{Law}(X_t)=:p_t
\end{align}
Justifications, in order: (1)~solved by $(\Phi_t)_\#\pi_0$ and by $\mathrm{Law}(X_t)$; (2)~uniqueness, Lemma~\ref{lem:wellposed}(c).
(The second solves \eqref{eq:pf:endpoint:2} by superposition, using Step~1.)

\textbf{Step 4: the endpoint.}
\begin{align}
X_t&=(1-t)X_0+tx^I\longrightarrow x^I\quad\text{a.s.}\label{eq:pf:endpoint:3}\\
p_t&\Rightarrow\mathrm{Law}(x^I)=\textstyle\sum_ip_i^{(h)}(y)\,\delta_{x^i}\\
p_1=\lim_{t\uparrow1}p_t&=\textstyle\sum_ip_i^{(h)}(y)\,\delta_{x^i}
\end{align}
Justifications, in order: (1)~$t\uparrow1$; (2)~$\eqref{eq:pf:endpoint:3}\ \text{a.s.\ implies in law}$; (3)~identify the weak limit, Lemma~\ref{lem:wellposed}.
...
\end{align}
Justifications, in order: (1)~Gaussian prefactors cancel; (2)~numerator of the exponent is a positive constant; (3)~divide by $K_h(y-y^{i^\star})$, use \eqref{eq:pf:zerobw:1}; (4)~Theorem~\ref{thm:endpoint}.
...
\mathtoolsset{showonlyrefs}
\begin{align}
K_h(y-y^i)&=(2\pi h^2)^{-k/2}\big[1+O(h^{-2})\big]\label{eq:pf:infbw:1}\\
p_i^{(h)}(y)&=\frac{1+O(h^{-2})}{N+O(h^{-2})}\label{eq:pf:infbw:2}\\
&\longrightarrow\frac1N\quad(h\to\infty)\\
p_1&\to\tfrac1N\textstyle\sum_i\delta_{x^i}=\rhohat_X
\end{align}
Justifications, in order: (1)~$\max_i|y-y^i|<\infty,\ \text{uniform in $i$}$; (2)~the prefactor cancels; \eqref{eq:pf:infbw:1}; (3)~$\eqref{eq:pf:infbw:2}$; (4)~Theorem~\ref{thm:endpoint}.
...
\end{align}
Justifications, in order: (1)~mean of $\sum_ip_i^{(h)}\delta_{x^i}$, Theorem~\ref{thm:endpoint}; (2)~covariance of the same mixture.
...
\end{align}
Justifications, in order: (1)~Corollary~\ref{cor:zerobw}; (2)~closed form; (3)~Corollary~\ref{cor:infbw}.
...
\end{align}
Justifications, in order: (1)~Theorem~\ref{thm:endpoint}; (2)~finite mixture of point masses.

\textbf{Step 2: a pointwise bound under any coupling $(X,X')$ of $p_h^{\mathrm{gen}}$ and $p(\cdot\mid y)$.}
\begin{align}
|X-X'|&\ge\min_i|x^i-X'|=\operatorname{dist}(X',\{x^i\})\label{eq:pf:atom:1}
\end{align}
Justification: $X\in\{x^i\}\ \text{a.s., Step~1}$.

\textbf{Step 3: the floor.}
\begin{align}
\E|X-X'|^2&\ge\E\operatorname{dist}(X',\{x^i\})^2\\
&=\int\operatorname{dist}(x,\{x^i\})^2p(x\mid y)\,dx=:F(y)\label{eq:pf:atom:2}\\
W_2^2(p_h^{\mathrm{gen}},p(\cdot\mid y))&\ge F(y)
\end{align}
Justifications, in order: (1)~square and integrate \eqref{eq:pf:atom:1}; (2)~$X'\sim p(\cdot\mid y)$; (3)~$\inf\ \text{over couplings; \eqref{eq:pf:atom:2}}$.
...
F(y)&>0
\end{align}
Justifications, in order: (1)~so it does not depend on $h$; (2)~$p(\cdot\mid y)\ \text{absolutely continuous}$; (3)~$\operatorname{dist}(\cdot,\{x^i\})>0\ \ p(\cdot\mid y)\text{-a.e.}$.
...
p(x\mid I=i,t)&=\varphi_{s_t}(x-tx^i)\label{eq:pf:tgtnoise:1}
\end{align}
Justifications, in order: (1)~$X_t=(1-t)X_0+tx^i+t\rho\xi$; (2)~independent Gaussians; (3)~density of $r$.
...
\end{align}
Justifications, in order: (1)~depend on $(X_0,\xi)$ and $\varepsilon$ only; Proposition~\ref{prop:kernel-field}; (2)~Bayes for $w_i:=\Pr(I=i\mid\cdot)$; \eqref{eq:pf:tgtnoise:1}.

\textbf{Step 3: the per-index target.}
\begin{align}
\Cov(X_0,r)=(1-t)I_d,&\quad\Cov(\xi,r)=t\rho I_d\\
\E[X_0\mid r]&=\tfrac{1-t}{s_t^2}\,r,\quad\E[\xi\mid r]=\tfrac{t\rho}{s_t^2}\,r\label{eq:pf:tgtnoise:3}\\
\E[U\mid X_t=x,t,I=i]&=x^i+\rho\,\tfrac{t\rho}{s_t^2}r-\tfrac{1-t}{s_t^2}r\\
&=x^i+c_t\,(x-tx^i)\label{eq:pf:tgtnoise:4}
\end{align}
Justifications, in order: (1)~$(X_0,\xi,r)$ jointly Gaussian; (2)~$\E[a\mid r]=\Cov(a,r)\Var(r)^{-1}r$; (3)~$U=x^i+\rho\xi-X_0;\ \eqref{eq:pf:tgtnoise:3}$; (4)~$c_t:=\tfrac{t\rho^2-(1-t)}{s_t^2}$.

\textbf{Step 4: the field.}
\begin{align}
\vstar(x,t,y)&=\E[U\mid X_t=x,t,\widetilde Y=y]\\
&=\sum_iw_i\big[x^i+c_t\,(x-tx^i)\big]\label{eq:tgtfield}
\end{align}
Justifications, in order: (1)~Proposition~\ref{prop:projection}; (2)~average \eqref{eq:pf:tgtnoise:4} over \eqref{eq:pf:tgtnoise:2}.
...
\end{align}
Justifications, in order: (1)~$\eqref{eq:pf:tgtnoise:1};\ p_i^{(h)}\propto K_h(y-y^i)$; (2)~$\eqref{eq:pf:tgtnoise:4}\ \text{holds unchanged; Lemma~\ref{lem:mixture-coupling}}$; (3)~smooth positive weights: Lemma~\ref{lem:wellposed}; Step~3 of Theorem~\ref{thm:endpoint}; (4)~$X_t\to X_1\ \text{a.s.}$; (5)~mixture over $I$.
...
\end{align}
Justifications, in order: (1)~$\eqref{eq:pf:tgtnoise:5}$; (2)~total covariance of the mixture; (3)~each component is; so \eqref{eq:w2floor} does not apply; (4)~$s_1^2=\rho^2>0$; (5)~$\eqref{eq:tgtfield}$.
...
\end{align}
Justifications, in order: (1)~independent Gaussians; (2)~$\gamma(t)Z\perp X_0$; (3)~$\E[a\mid r]=\Cov(a,r)\Var(r)^{-1}r$; (4)~same.
...
&=x^i+\frac{-(1-t)+\dot\gamma\gamma}{s_t^2}\,r\label{eq:pf:prop17:3}\\
\dot\gamma\gamma&=\tfrac12\tfrac{d}{dt}\gamma^2=\tfrac{\sigma^2}2(1-2t)\label{eq:pf:prop17:4}\\
c(t)&=\frac{-(1-t)+\frac{\sigma^2}2(1-2t)}{s_t^2}\\
\tfrac{d}{dt}s_t^2&=-2(1-t)+2\dot\gamma\gamma=2s_t^2\,c(t)\\
c(t)&=\tfrac12\tfrac{d}{dt}\log s_t^2
\end{align}
Justifications, in order: (1)~Proposition~\ref{prop:projection}; (2)~$\dot X_t=x^i-X_0+\dot\gamma Z,\ \eqref{eq:c2target}$; (3)~$\eqref{eq:pf:prop17:2}$; (4)~$\gamma^2=\sigma^2t(1-t)$; (5)~$\eqref{eq:pf:prop17:3},\ \eqref{eq:pf:prop17:4}$; (6)~$\eqref{eq:pf:prop17:4}$; (7)~previous line.

\textbf{Step 3: part (b).} Put $u_t:=x_t-tx^i$.
\begin{align}
\dot u_t&=v_\sigma^\star(x_t,t,y^i)-x^i=c(t)\,u_t\label{eq:pf:prop17:5}\\
u_t&=u_0\exp\!\int_0^tc\\
&=x_0\exp\!\big(\tfrac12[\log s_t^2-\log s_0^2]\big)=s_t\,x_0\label{eq:pf:prop17:6}\\
x_t&=tx^i+s_tx_0
\end{align}
Justifications, in order: (1)~(a) with $r=u_t$; (2)~linear ODE; (3)~$c=\tfrac12(\log s^2)';\ s_0=1,\ u_0=x_0$; (4)~definition of $u_t$.
...
\end{align}
Justifications, in order: (1)~definition; (2)~$\eqref{eq:pf:prop17:6}$; (3)~point mass; this is \eqref{eq:invariance}.
...
\inf_vL_0&=0
\end{align}
Justifications, in order: (1)~the target of Proposition~\ref{prop:collapse}; (2)~Proposition~\ref{prop:collapse}(b).
...
&=\big(1+\dot\gamma^2\big)-c(t)^2s_t^2\label{eq:pf:prop18:1}\\
\eqref{eq:pf:prop18:1}=0&\iff u\parallel w\iff\dot\gamma(1-t)=-\gamma\label{eq:pf:prop18:2}\\
\tfrac{\sigma(1-2t)}{2\sqrt{t(1-t)}}(1-t)&=-\sigma\sqrt{t(1-t)}\iff1-2t=-2t\\
1&=0
\end{align}
Justifications, in order: (1)~Gaussian conditioning; (2)~substitute; (3)~Cauchy--Schwarz equality; (4)~$\gamma=\sigma\sqrt{t(1-t)},\ \eqref{eq:pf:prop18:2}$; (5)~impossible.
So \eqref{eq:pf:prop18:1} is $>0$ for every $t\in(0,1)$, and $\inf_vL_\sigma>0$.

\textbf{Step 4: the Lipschitz rate.}
\begin{align}
(1-t)\,c(t)&=\frac{-(1-t)+\frac{\sigma^2}2(1-2t)}{(1-t)+\sigma^2t}\label{eq:pf:prop18:3}\\
&\to-1\ \ (\sigma=0),\qquad\to-\tfrac{\sigma^2/2}{\sigma^2}=-\tfrac12\ \ (\sigma>0)
\end{align}
Justifications, in order: (1)~Proposition~\ref{prop:prop17}(a); (2)~$t\uparrow1$.
...
\end{align}
Justifications, in order: (1)~Theorem~\ref{thm:endpoint}; depends on $h$ only through $K_h(y-y^i)$; (2)~Corollaries~\ref{cor:zerobw}, \ref{cor:infbw}.
...
\end{align}
Justifications, in order: (1)~interpolant noise; Lemma~\ref{lem:cov}; (2)~Proposition~\ref{prop:prop17}(c), despite \eqref{eq:pf:factors:2}.
...
\end{align}
Justifications, in order: (1)~LLN for both weighted sums, then Slutsky; (2)~total covariance; exact for every $h>0$; (3)~the claim, proved in Steps~3--4.
...
\tilde q_h(u)&=\varphi(u)\big[1+h\,s^\top u+O(h^2)\big]\label{eq:pf:expansion:3}
\end{align}
Justifications, in order: (1)~$y'=y+hu$; (2)~$C^2\ \text{Taylor}$; (3)~$\textstyle\int\varphi(u)u\,du=0$; (4)~ratio of the two lines above.
...
\end{align}
Justifications, in order: (1)~first-order term; (2)~odd moments vanish; (3)~$\eqref{eq:pf:expansion:4},\ \eqref{eq:pf:expansion:5}$.
...
&=\Sigma(y)+O(h^4)
\end{align}
Justifications, in order: (1)~$Y=y+hu$; (2)~$\Sigma\equiv\Sigma(y)\ \text{on $B(y,r)$; mass outside is a Gaussian tail (Step~4)}$; (3)~$e^{-r^2/4h^2}=o(h^p)$ for all $p$.
...
\end{align}
Justifications, in order: (1)~Taylor; (2)~$\eqref{eq:pf:expansion:4},\ \eqref{eq:pf:expansion:5}$.
...
\end{align}
Justifications, in order: (1)~by construction; (2)~$\rho_Y$ constant near $y$, up to tails.
...
&=o(h^p)\ \ \forall p
\end{align}
Justifications, in order: (1)~Gaussian tail beats the polynomial growth; (2)~work on $|u|\le r/h$, where $y+hu\in B(y,r)$.
...
\Cov[R_h]&=O(h^6)
\end{align}
Justifications, in order: (1)~Taylor, Lagrange remainder; (2)~$\|\nabla^3\mu\|\le M_3$; (3)~moments of $\tilde q_h$ bounded, $h\le1$; (4)~same; (5)~same.
...
\end{align}
Justifications, in order: (1)~drop $R_h$; covariance ignores $\mu(y)$; (2)~bilinearity $\;+O(h^4)$; (3)~$\eqref{eq:pf:expansion:6}$; (4)~$\eqref{eq:pf:expansion:3};\ \E_\varphi[u_au_bu_c]=0$; (5)~every third moment is $O(h)$; $Q$ is quadratic; (6)~previous line; trivially; (7)~$\eqref{eq:pf:expansion:7}$.
...
\end{align}
Justifications, in order: (1)~height $\Theta(h^{-k})$ on volume $\Theta(h^k)$; dominates the squared mean $\Theta(1)$; (2)~$m_4\ \text{bounded near $y$: the only use of (iv)}$; (3)~i.i.d.\ averages; relative error $O_P((Nh^k)^{-1/2})$; (4)~first-order Taylor (Slutsky) on $T_N/S_N$; (5)~the same, for the quadratic functional.
...
\end{align}
Justifications, in order: (1)~Taylor of $\mu(y^j)$ against symmetric weights, as in Step~4 of Proposition~\ref{prop:expansion}; (2)~classical.
...
\begin{align}
\mathrm{MSE}(h)&=c_1h^4+c_2(Nh^k)^{-1}+o(\cdot)\label{eq:pf:nwrate:3}\\
\tfrac{d}{dh}\mathrm{MSE}&=4c_1h^3-c_2\,k\,N^{-1}h^{-k-1}=0\\
h^{k+4}&=\tfrac{c_2k}{4c_1N},\quad h^\star\asymp N^{-1/(k+4)}\label{eq:pf:nwrate:4}\\
\tfrac{d^2}{dh^2}\mathrm{MSE}&>0\ \text{at \eqref{eq:pf:nwrate:4}}
\end{align}
Justifications, in order: (1)~$\eqref{eq:pf:nwrate:1}^2+\eqref{eq:pf:nwrate:2}$; (2)~first-order condition; (3)~needs $c_1,c_2>0$; (4)~so it is the minimiser.
...
\end{align}
Justifications, in order: (1)~population version; (2)~precision $\Sigma_Y^{-1}+h^{-2}I_k,\ \text{mean }(\Sigma_Y^{-1}+h^{-2}I_k)^{-1}h^{-2}y$; (3)~$\mu\ \text{affine: evaluate at the mean of \eqref{eq:pf:nwrate:6}}$; (4)~$\Sigma_Y(\Sigma_Y+h^2I)^{-1}-I=-h^2(\Sigma_Y+h^2I)^{-1}$; (5)~expand the inverse.
...
\end{align}
Justifications, in order: (1)~definition; (2)~in probability; (3)~$Nh^k\to\infty$.
...
\E|f_i(X,t)-f_i(X',t)|^2&=\frac{2(1-t)^2d}{(1-t)^2}=2d\label{eq:pf:liplb:1}
\end{align}
Justifications, in order: (1)~$f_i=(x^i-x)/(1-t)$; (2)~$X-X'\sim\N(0,2(1-t)^2I_d)$.

\textbf{Step 2: the model's increments.}
\begin{align}
\E|v(X,t,y^i)-v(X',t,y^i)|^2&\le L^2\,\E|X-X'|^2\\
&=2dL^2(1-t)^2\label{eq:pf:liplb:2}
\end{align}
Justifications, in order: (1)~$L$-Lipschitz; (2)~$\E|X-X'|^2=2d(1-t)^2$.

\textbf{Step 3: the residual's increments.}
\begin{align}
\E|e(X)-e(X')|^2&=2\E|e(X)|^2-2\big|\E e(X)\big|^2\\
&\le2\,\E|e(X)|^2\label{eq:pf:liplb:3}
\end{align}
Justifications, in order: (1)~$X,X'\ \text{i.i.d.}$; (2)~drop the mean term.
...
\end{align}
Justifications, in order: (1)~definition of $e$; (2)~$L^2(\Omega)\ \text{triangle};\ \eqref{eq:pf:liplb:1}\text{--}\eqref{eq:pf:liplb:3}$; (3)~rearrange \eqref{eq:pf:liplb:4}; (4)~square; the sign of $e$ is irrelevant.
...
&=\tfrac1L-\tfrac1L+\tfrac1{3L}=\tfrac1{3L}\label{eq:pf:floor:1}
\end{align}
Justifications, in order: (1)~antiderivative; (2)~evaluate.
...
&=\frac d{3L}\label{eq:pf:floor:2}
\end{align}
Justifications, in order: (1)~average over $I\sim\mathrm{Unif}$, $t\sim\mathrm{Unif}(0,1)$; (2)~$s=1-t;\ \eqref{eq:pf:floor:1}$.
...
&=\frac{m_w-x}{1-t}
\end{align}
Justifications, in order: (1)~Propositions~\ref{prop:kernel-field}, \ref{prop:uncond}; (2)~definition; (3)~$-(1+w)x+wx=-x;\ \text{this is \eqref{eq:cfgfield}}$.
...
\end{align}
Justifications, in order: (1)~$\textstyle\sum_iw_i^{(h)}=\sum_iq_i=1$; (2)~coefficients sum to $1$ (not nec.\ $\ge0$).

\textbf{Step 3: the orthogonal component.} $\Pi$ is affine with $\Pi z=z$ on $\mathcal A$.
\begin{align}
\dot q_t&=(I-\Pi_{\mathrm{lin}})\,\dot x_t\\
&=-\frac{q_t}{1-t}\label{eq:pf:cfg:2}\\
q_t&=(1-t)\,q_0
\end{align}
Justifications, in order: (1)~$q_t=x_t-\Pi x_t$; (2)~$(I-\Pi)m_w=0,\ \text{as $m_w\in\mathcal A$}$; (3)~solve \eqref{eq:pf:cfg:2}; this is \eqref{eq:cfgorth}.
...
\end{align}
Justifications, in order: (1)~$\eqref{eq:cfgorth}$; (2)~$\mathcal A\ \text{closed}$.
...
\end{align}
Justifications, in order: (1)~Step~2: bounded coefficients; (2)~\eqref{eq:vgrowth} with $M\to(1+2w)M$.
...
&\le\frac{(N-|C|)\,c\,e^{-\Delta^2/2h^2}}{|C|\,c\,e^{-\varepsilon^2/2h^2}}=\eta\label{eq:pf:nearduplicate:1}
\end{align}
Justifications, in order: (1)~drop $j\notin C$ from the denominator; (2)~$|y-y^j|\ge\Delta,\ |y-y^i|\le\varepsilon$.
...
\end{align}
Justifications, in order: (1)~definition; (2)~$|y-y^{i'}|\le\varepsilon,\ |y-y^i|\ge0$.
...
\end{align}
Justifications, in order: (1)~sum \eqref{eq:pf:nearduplicate:2} over $i'\in C$; (2)~previous line; $S\le1$; (3)~$\eqref{eq:pf:nearduplicate:1}$; (4)~add the two bounds, invert.
...
n_{\mathrm{eff}}&\to|C|
\end{align}
Justifications, in order: (1)~$\eqref{eq:pf:nearduplicate:1}$; (2)~$\eqref{eq:pf:nearduplicate:2}\ \text{is symmetric in $i,i'$}$; (3)~Theorem~\ref{thm:endpoint}; (4)~Cauchy--Schwarz on $C$; (5)~with the lower bound of Step~3.
...
\frac{d}{dt}\langle x_t,u\rangle&=\frac{\langle m_w-x_t,u\rangle}{1-t}\ge\frac c{1-t}\label{eq:pf:cfgatom:1}\\
\langle x_t,u\rangle&\ge\langle x_{t_0},u\rangle+c\log\tfrac{1-t_0}{1-t}\to+\infty
\end{align}
Justifications, in order: (1)~$m_w-x_t\to m^*-x^*$; (2)~the line above; (3)~integrate \eqref{eq:pf:cfgatom:1}.
...
\end{align}
Justifications, in order: (1)~Lemma~\ref{lem:cov}; $c_j=K_h(y-y^j)$ for $w^{(h)}_j$, $c_j=\frac1N$ for $q_j$; (2)~$x_t\to x^*,\ t\to1;\ \text{nearest atom unique}$; (3)~\eqref{eq:pf:cfgatom:2},\eqref{eq:pf:cfgatom:3}, for $t\ge t_j$; (4)~$\eqref{eq:pf:cfgatom:4};\ \textstyle\sum_j\alpha_j=1,\ \text{finitely many $j$}$; (5)~previous line.
...
&\to(1+w)x^{i^*}-w\,x^{i^*}=x^{i^*}\label{eq:pf:cfgatom:5}\\
x^*&=x^{i^*}
\end{align}
Justifications, in order: (1)~$\eqref{eq:cfgfield}$; (2)~Step~2; the guidance cancels; (3)~Step~1 with \eqref{eq:pf:cfgatom:5}.

\textbf{Step 3(b): $h=0$.}
\begin{align}
\vstar(\cdot,\cdot,y^i)&=\frac{x^i-x}{1-t}\ \Rightarrow\ \text{conditional mean}=x^i\\
m_w(x_t,t)&\to(1+w)x^i-w\,x^{i^*}\\
x^*&=(1+w)x^i-w\,x^{i^*}\label{eq:cfgfixed}
\end{align}
Justifications, in order: (1)~Proposition~\ref{prop:collapse}(a); (2)~$\bar x_{\mathrm{unc}}\to x^{i^*}\ \text{(Step~2)}$; (3)~Step~1.
...
\end{align}
Justifications, in order: (1)~$\eqref{eq:cfgfixed};\ \text{nearest atom unique: contradiction}$; (2)~rearrange \eqref{eq:cfgfixed}; (3)~previous line.
...
\end{align}
Justifications, in order: (1)~definition; (2)~add and subtract $\vstarz(x^v_t)$; (3)~$\vstarz\ \text{affine, linear part }-\tfrac1{1-t}I_d$.
...
|e_t|&\le(1-t)\int_0^t\frac{|\Delta(x^v_s,s)|}{1-s}\,ds
\end{align}
Justifications, in order: (1)~product rule; (2)~$\eqref{eq:pf:survival:1}$; (3)~the $e_t$ terms cancel; (4)~integrate \eqref{eq:pf:survival:2}, $e_0=0$; this is \eqref{eq:survival-id}; (5)~triangle inequality; this is \eqref{eq:survival}.
...
\end{align}
Justifications, in order: (1)~Cauchy--Schwarz on \eqref{eq:survival}; (2)~$\textstyle\int_0^t(1-s)^{-2}ds=\frac t{1-t}$; (3)~$t\le1$; (4)~$t=1-\delta\ \text{in \eqref{eq:pf:survival:3}}$.

\textbf{Step 4: part (b).} Assume $|\Delta|\le c(1-s)^{-a}$, $a>0$.
\begin{align}
\int_0^t(1-s)^{-1-a}ds&=\tfrac1a\big((1-t)^{-a}-1\big)\label{eq:pf:survival:4}\\
|e_t|&\le\tfrac ca\big((1-t)^{1-a}-(1-t)\big)\\
|e_t|&\le c\,t\le c\quad(a=1)
\end{align}
Justifications, in order: (1)~antiderivative; (2)~$\eqref{eq:survival},\ \eqref{eq:pf:survival:4}$; (3)~$(1-t)^0-(1-t)=t$.
...
&=\tfrac ca\big((1-t)^{1-a}-(1-t)\big)u\quad(a>0)\label{eq:pf:survival:5}\\
&=-c(1-t)\log(1-t)\,u\quad(a=0)
\end{align}
Justifications, in order: (1)~$\eqref{eq:survival-id}$; (2)~$\eqref{eq:pf:survival:4}$; (3)~$\textstyle\int_0^t(1-s)^{-1}ds=-\log(1-t)$.
...
\end{align}
Justification: $\eqref{eq:pf:survival:5}$.
...
\end{align}
Justifications, in order: (1)~$\eqref{eq:survival-id}$; (2)~subtract $e_\star$; (3)~$\eqref{eq:pf:survival:6};\ \text{this is \eqref{eq:survival-conv}}$.
...
\end{align}
Justifications, in order: (1)~$U=\vstarz(X_t,t,y^i)\ \text{a.s., Proposition~\ref{prop:collapse}(a)}$; (2)~$x^\star_s=(1-s)X_0+sx^i\sim X_s$.

\textbf{Step 2: Minkowski.}
\begin{align}
\mathcal E(t)&\le(1-t)\int_0^t\frac{\|\Delta(x^v_s,s)\|_{L^2}}{1-s}\,ds\label{eq:pf:lossform:2}
\end{align}
Justification: $L^2(X_0)$ norm of \eqref{eq:survival}; Minkowski's integral inequality.
...
\end{align}
Justifications, in order: (1)~Assumption~\ref{as:lipschitz}; (2)~triangle inequality in $L^2(X_0)$; (3)~$\eqref{eq:pf:lossform:2},\ \eqref{eq:pf:lossform:3};\ \mathcal E(s)/(1-s)=F(s)$.

\textbf{Step 4: Gr\"onwall.}
\begin{align}
F(t)&\le G(t)\,e^{L_\Delta t}
\end{align}
Justification: \eqref{eq:pf:lossform:4}; $G$ nondecreasing.
...
\mathcal E(1-\delta)&\le e^{L_\Delta}\sqrt{\mathcal L_i(v)\,\delta}
\end{align}
Justifications, in order: (1)~Cauchy--Schwarz; (2)~$\eqref{eq:pf:lossform:1}$; (3)~$(1-t)\times[\text{Step 4 with \eqref{eq:pf:lossform:5}}]$; (4)~$t=1-\delta;\ 1-\delta\le1$.
...
\end{align}
Justifications, in order: (1)~$x^\star_{1-\delta}=\delta X_0+(1-\delta)x^i$; (2)~triangle inequality in $L^2(X_0)$.
...
\begin{align}
\ell_i(\hat t)=\ell_j(\hat t)&\iff z^j=z^i+\tfrac{1-\hat t}{\hat t}(x_0^i-x_0^j)\label{eq:pf:nonintersect:1}
\end{align}
Justification: solve for $z^j$.
...
\end{align}
Justifications, in order: (1)~$\eqref{eq:pf:nonintersect:1}$; (2)~half-line in a 1-dim.\ affine subspace, $d\ge2$ (a point if $x_0^i=x_0^j$); (3)~$\rho_1\ \text{a.c.};\ z^j\perp(x_0^i,x_0^j,z^i)$; (4)~integrate; union bound over the finitely many pairs.
...
\end{align}
Justifications, in order: (1)~as in Step~2, $t_i\ne t_j$ free; (2)~countable union of Lipschitz images of bounded planar sets; (3)~$d>2;\ \text{the rest is as in Step~2}$.
...
\end{align}
Justifications, in order: (1)~Lemma~\ref{lem:nonintersect}(a)--(b); union over $\tbinom N2$ pairs; (2)~a fortiori.

\textbf{Step 2: a lookup table.}
\begin{align}
v(x,t)&:=z^{i(x,t)}-x_0^{i(x,t)}\ \text{on the sample},\qquad v:=0\ \text{elsewhere}\label{eq:pf:memorize:1}
\end{align}
Justification: $i(x,t)\ \text{well defined by Step~1}$.
...
\end{align}
Justifications, in order: (1)~$\eqref{eq:pf:memorize:1}$; (2)~every summand vanishes.
...
\end{align}
Justification: finitely many affine maps and Lipschitz activations.

\textbf{Step 2: it is bounded on $[0,1)$.} Fix $t_0\in[0,1)$.
\begin{align}
|f_\theta(t)|&\le|f_\theta(t_0)|+K|t-t_0|\le|f_\theta(t_0)|+K\label{eq:pf:norepr:1}\\
\sup_{t\in[0,1)}|f_\theta(t)|&<\infty
\end{align}
Justifications, in order: (1)~Step~1; $|t-t_0|\le1$; (2)~$\eqref{eq:pf:norepr:1}$.
...
\sup_{t\in[0,1)}|f_\theta(t)-g(t)|&=\infty
\end{align}
Justifications, in order: (1)~$g\ \text{unbounded on $[0,1)$}$; (2)~Step~2: $f_\theta$ bounded, $g$ not.
