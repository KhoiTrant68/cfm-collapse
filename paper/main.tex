...
\item \textbf{Restored variance is not posterior recovery.} The smoothed law is
  ...
  Neither is a certificate alone, since $\tr\Cov=10\,\tr\Covh$ has slope $1$ and is off
  by an order of magnitude. Nothing here is specific to flow matching, and the caution
  applies to our own headline statistic.
...
density, and every field below is locally Lipschitz on $\R^d\times[0,1)$, so endpoint laws
are weak limits $p_1=\lim_{t\uparrow1}p_t$.
...
the noise at the endpoint. Interpolant noise cannot succeed in principle
...
empty when $N>d$. Integrating the exact guided field shows more: endpoints land on the
atoms themselves, and larger $w$ changes which atom is reached
...
unbounded for $a>1$ (the bound is attained, so this is realised); for $a>1$ the bound alone
constrains nothing.
A model can therefore retain conditional variance only through an endpoint-scale
accumulated error. This connects to Remark~\ref{lem:norepr}, which observes that no finite
...
hypothesis on the error, not on the field, converts the estimate into the training loss:
Corollary~\ref{cor:lossform} bounds the root-mean-square deviation at truncation
$1-\delta$ by $e^{L_\Delta}\sqrt{\mathcal L_i(v)\,\delta}$. The retained deviation is
bounded by the loss the optimiser has not yet removed, and stopping early is the only
other lever. On EXP-1's checkpoints the slope is $0.485$ at $R^2=0.998$, against the
predicted $\tfrac12$
...
are marked empirical and are not called predictions that matched or failed. The theory
...
\begin{proof}[Proof of Proposition~\ref{prop:kernel-field}]
    ...
    Given $I=i$, $X_t=(1-t)X_0+tx^i$ and $\widetilde Y=y^i+h\eps$ with $X_0\perp\eps$. By Bayes and Lemma~\ref{lem:cov},
    \begin{align*}
...
\begin{proof}[Proof of Theorem~\ref{thm:endpoint}]
    The marginals of the coupling below solve the continuity equation of the flow, so they equal the flow's marginals, and they converge to $\law(x^I)$.
