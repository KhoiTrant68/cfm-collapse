...
$h{=}0$ and $h{=}0.4$ on the one-hot label.
...
image, and its six completions are different images of that class, not one image. Nothing here
shows whether the completions are training images: no checkpoint is kept, so the distance of
individual samples to the training set was not computed. Cut from the saved grids and metrics by
\texttt{scripts/make\_image\_extra\_figures.py}.}
