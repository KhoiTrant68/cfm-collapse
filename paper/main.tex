...
\paragraph{We do not benchmark against the remedies practitioners use.} Early
stopping, weight averaging, condition dropout, caption randomisation and guidance
scale are all deployed against this symptom and we compare against none of them; our
claim is diagnostic, not prescriptive. Caption randomisation is the one case where the
analysis makes a concrete prediction---it is label smoothing, so
Theorem~\ref{thm:endpoint} applies and the atomicity ceiling binds it---and testing
that against practice is the obvious next experiment.
