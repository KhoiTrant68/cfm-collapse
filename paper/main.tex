...
\emph{Step 1: the decomposition.} Write $\mathcal G=\sigma(X_t,t,\widetilde Y)$ and
$m:=\E[U\mid\mathcal G]$, which exists because $|U|\le|x^I|+|X_0|$ is square-integrable.
For square-integrable $v$ the residual splits as $U-v=(U-m)+(m-v)$ with $m-v$
$\mathcal G$-measurable, so
\begin{equation}
L_h(v)=\E|U-m|^2+\E|m-v|^2+2\,\E\langle U-m,\ m-v\rangle .
\label{eq:pf:projection:1}
\end{equation}

\emph{Step 2: the cross term vanishes.} By the tower property,
\begin{equation}
\E\langle U-m,\ m-v\rangle=\E\big\langle\E[U-m\mid\mathcal G],\ m-v\big\rangle=0,
\label{eq:pf:projection:2}
\end{equation}
since $\E[U-m\mid\mathcal G]=0$ by definition of $m$.

\emph{Step 3: the minimiser.} By \eqref{eq:pf:projection:1}--\eqref{eq:pf:projection:2},
$L_h(v)=\E[\Var(U\mid\mathcal G)]+\E|v-m|^2$, where $\E|U-m|^2=\E[\Var(U\mid\mathcal G)]$
is the trace of the conditional covariance. The second term is nonnegative and vanishes
exactly when $v=m$ almost everywhere under the law of $(X_t,t,\widetilde Y)$, so the
minimiser is a.e.\ unique, equals $\vstar(x,t,y)=\E[U\mid X_t=x,t,\widetilde Y=y]$, and
$\inf_vL_h=\E[\Var(U\mid X_t,t,\widetilde Y)]$. This proves
...
\emph{Step 1: the map is a bijection.} Put $\Psi(x_0)=(1-t)x_0+tx^i$. Its linear part
is $(1-t)I_d$, invertible for $t<1$, so
\begin{equation}
\det D\Psi=(1-t)^d,\qquad \Psi^{-1}(x)=\frac{x-tx^i}{1-t}.
\label{eq:pf:cov:1}
\end{equation}
A bijection generates the same $\sigma$-algebra as its argument, so
$\sigma(X_t\mid I=i)=\sigma(X_0\mid I=i)$.

\emph{Step 2: the density.} Given $I=i$, $X_0\sim\pi_0$ is independent of $I$, so the
change-of-variables formula for densities and \eqref{eq:pf:cov:1} give
\begin{equation}
p_{X_t}(x)=p_{X_0}\big(\Psi^{-1}(x)\big)\,|\det D\Psi|^{-1}
=(1-t)^{-d}\,\pi_0\!\big(\tfrac{x-tx^i}{1-t}\big),
\label{eq:pf:cov:2}
\end{equation}
which is \eqref{eq:cov}.

\emph{Step 3: cancellation and positivity.} The factor $(1-t)^{-d}$ in
\eqref{eq:pf:cov:2} involves neither $i$ nor $x$, so it appears identically in the
numerator and the denominator of $\Pr(I=i\mid X_t=x,t)$. Positivity is that of the
Gaussian density $\pi_0>0$ composed with the bijection $\Psi^{-1}$. This proves
Lemma~\ref{lem:cov}.
\end{proof}
...
\emph{Step 1: part (a), smoothness.} On $\R^d\times[0,1)$ the map
$(x,t)\mapsto(x^i-x)/(1-t)$ is $C^\infty$ and the $w_i$ are $C^\infty$ by hypothesis, so
$v$ is a finite sum of products of $C^\infty$ functions. Local Lipschitzness in $x$,
uniform for $t$ in compacts of $[0,1)$, follows from continuity of $\nabla_xv$ on the
compact set.

\emph{Step 2: part (b), global existence and the growth bound.} Local existence and
uniqueness on a maximal interval $[0,\tau)$ is Picard--Lindel\"of
\citep[Ch.~II]{hartman2002ode}, using Step~1. For the bound, $\sum_iw_i\equiv1$ and
$w_i\ge0$ give the pointwise estimate
\begin{equation}
|v(x,t)|\;\le\;\sum_iw_i(x,t)\,\frac{|x^i-x|}{1-t}\;\le\;\frac{M+|x|}{1-t}.
\label{eq:vgrowth}
\end{equation}
...
inequality gives
\begin{equation}
g(t)\;\le\;g(0)\exp\!\int_0^t\frac{ds}{1-s}\;=\;\frac{g(0)}{1-t},\qquad t\in[0,\tau),
\label{eq:pf:wellposed:1}
\end{equation}
which is the stated bound. Its right-hand side is finite for every $t<1$, so the
solution stays in a bounded set on every $[0,T]$ with $T<1$; it cannot blow up before
$t=1$, and $\tau=1$.

\emph{Step 3: part (c), the push-forward solves the equation.} For
$\varphi\in C_c^\infty(\R^d)$ and $\mu_t=(\Phi_t)_\#\mu_0$, differentiating along the
flow gives
\begin{equation}
\frac{d}{dt}\int\varphi\,d\mu_t
=\int\nabla\varphi(\Phi_t(x))\cdot v(\Phi_t(x),t)\,d\mu_0(x)
=\int\nabla\varphi\cdot v\,d\mu_t,
\label{eq:pf:wellposed:2}
\end{equation}
which is the weak form of $\partial_t\mu_t+\nabla\!\cdot(v\mu_t)=0$.

\emph{Step 4: part (c), uniqueness.} Fix $T<1$ and let $(\mu_t)_{t\in[0,T]}$ be any
narrowly continuous solution with the same initial datum. By Steps~1 and~2 the flow is a
$C^\infty$ diffeomorphism of $\R^d$ for each $t\le T$ \citep[Ch.~V]{hartman2002ode};
write $\Phi_{s,T}$ for the flow from time $s$ to time $T$. Given
$\varphi\in C_c^\infty(\R^d)$ set $\psi(x,s):=\varphi(\Phi_{s,T}(x))$. Then $\psi$ is
$C^1$ on $\R^d\times[0,T]$, has compact support in $x$ for each $s$, namely
$\Phi_{T,s}(\operatorname{supp}\varphi)$, and by the method of characteristics solves
the backward transport equation
\begin{equation}
\partial_s\psi+v\cdot\nabla\psi=0,\qquad\psi(\cdot,T)=\varphi .
\label{eq:pf:wellposed:3}
\end{equation}
Testing the continuity equation against $\psi$ (legitimate after the usual truncation in
time, since $\psi$ is $C^1$ and compactly supported in $x$ uniformly on $[0,T]$) and
using \eqref{eq:pf:wellposed:3},
\begin{equation}
\frac{d}{ds}\int\psi(\cdot,s)\,d\mu_s
=\int\big(\partial_s\psi+v\cdot\nabla\psi\big)\,d\mu_s=0 ,
\label{eq:pf:wellposed:4}
\end{equation}
...
$\mu_T=(\Phi_T)_\#\mu_0$, and $T<1$ was arbitrary. This is the classical smooth-field
case of the general uniqueness theory for continuity equations; see
...
\emph{Step 1: the conditioning event pins the index.} By
Assumption~\ref{as:standing}(iii) the labels are pairwise distinct, so $\{Y=y^i\}$
occurs exactly when $I=i$, and $X_1=x^i$ is then deterministic. By
Lemma~\ref{lem:cov}, conditioning further on $X_t=x$ determines the source point,
\begin{equation}
X_0=\frac{x-tx^i}{1-t},
\label{eq:pf:collapse:1}
\end{equation}
so $U=X_1-X_0$ is a function of $(x,t,i)$ alone.

\emph{Step 2: part (a).} Substituting \eqref{eq:pf:collapse:1} into $U=x^i-X_0$,
\begin{equation}
U=x^i-\frac{x-tx^i}{1-t}=\frac{x^i-x}{1-t}.
\label{eq:pf:collapse:2}
\end{equation}
The right-hand side is a measurable function of the conditioning variables
$(X_t,t,Y)$, and such a variable equals its own conditional expectation, so
Proposition~\ref{prop:projection} gives $v_0^\star(x,t,y^i)=(x^i-x)/(1-t)$.

\emph{Step 3: part (b).} Taking $v=v_0^\star$ in \eqref{eq:objective}, the residual
$U-v_0^\star(X_t,t,Y)$ vanishes almost surely by \eqref{eq:pf:collapse:2}, so
$L_0(v_0^\star)=0$; since $L_0\ge0$ for every $v$, $\inf_vL_0=0$.

\emph{Step 4: parts (c) and (d).} Along a solution of $\dot x_t=(x^i-x_t)/(1-t)$,
\begin{equation}
\frac{d}{dt}\Big[\frac{x_t-x^i}{1-t}\Big]
=\frac{\dot x_t(1-t)+(x_t-x^i)}{(1-t)^2}=0,
\label{eq:pf:collapse:3}
\end{equation}
so $(x_t-x^i)/(1-t)$ is constant on $[0,1)$ and equals its value $x_0-x^i$ at $t=0$.
Rearranging gives $x_t=(1-t)x_0+tx^i$, whence $x_t\to x^i$ as $t\uparrow1$ for every
$x_0$. The endpoint law is therefore the point mass $\delta_{x^i}$, whose covariance
is $0$, which is (d).
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:uncond}]
\emph{Step 1: the index posterior.} Bayes' rule with the uniform prior on $I$ and
Lemma~\ref{lem:cov}'s conditional density gives
\begin{equation}
q_i(x,t)=\frac{\tfrac1N\,(1-t)^{-d}\pi_0\big(\tfrac{x-tx^i}{1-t}\big)}
{\sum_j\tfrac1N\,(1-t)^{-d}\pi_0\big(\tfrac{x-tx^j}{1-t}\big)}
\ \propto\ \pi_0\!\big(\tfrac{x-tx^i}{1-t}\big),
\label{eq:pf:uncond:1}
\end{equation}
the prior $1/N$ and the Jacobian $(1-t)^{-d}$ cancelling because neither depends on
$i$.

\emph{Step 2: the minimiser.} Conditionally on $I=i$, Step~1 of the proof of
Proposition~\ref{prop:collapse} gives $U=(x^i-x)/(1-t)$, a constant given $(x,t,i)$.
Averaging over \eqref{eq:pf:uncond:1} by the tower rule and using
Proposition~\ref{prop:projection},
\begin{equation}
v_{\mathrm{unc}}^\star(x,t)=\E[U\mid X_t=x,t]
=\sum_iq_i(x,t)\,\frac{x^i-x}{1-t}.
\label{eq:pf:uncond:2}
\end{equation}

\emph{Step 3: the endpoint law.} Field \eqref{eq:pf:uncond:2} has the form required
by Lemma~\ref{lem:wellposed}, with smooth weights summing to one, so its flow is well
posed on $[0,1)$ and transports $p_0=\pi_0$ to $p_t=\mathrm{Law}(X_t)$, by the
argument of Steps~2--3 in the proof of Theorem~\ref{thm:endpoint} with
$p^{(h)}(y)$ replaced by the uniform law on $I$. Since $X_t\to X_1$ almost surely and
$X_1\sim\rhohat_X$, $p_t\Rightarrow\rhohat_X$, i.e.\ $p_1^{\mathrm{unc}}=\rhohat_X$:
the unconditional flow memorises the empirical measure rather than a single atom.

\emph{Step 4: the loss does not vanish.} Fix $t<1$ and $x$, and write
$u_i:=(x^i-x)/(1-t)$ and $\bar u(x,t):=\sum_iq_i(x,t)u_i$. By the positivity of
$\pi_0$ recorded after Lemma~\ref{lem:cov}, \eqref{eq:pf:uncond:1} gives
$q_i(x,t)>0$ for every $i$; and $x^i\ne x^j$ implies $u_i\ne u_j$, since $u$ is an
injective affine function of $x^i$ at fixed $(x,t)$. So whenever some $x^i\ne x^j$
the conditional law of $U$ puts positive mass on at least two distinct values, and
\begin{equation}
\Var(U\mid X_t=x,t)=\sum_iq_i(x,t)\,|u_i-\bar u(x,t)|^2>0
\label{eq:pf:uncond:3}
\end{equation}
at \emph{every} $(x,t)$ with $t<1$. Integrating \eqref{eq:pf:uncond:3} over $(x,t)$
and applying Proposition~\ref{prop:projection},
$\inf_vL_{\mathrm{unc}}(v)=\E[\Var(U\mid X_t,t)]>0$, in contrast with the zero
infimum of Proposition~\ref{prop:collapse}(b).
\end{proof}

\begin{proof}[Proof of Corollary~\ref{cor:distinction}]
\emph{Step 1: the conditional flow.} By Proposition~\ref{prop:collapse}(d),
$p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$, a point mass, whose covariance is $0$.

\emph{Step 2: the unconditional flow.} By Proposition~\ref{prop:uncond},
$p_1^{\mathrm{unc}}=\rhohat_X=\frac1N\sum_i\delta_{x^i}$, whose covariance is, by
definition of $\Xhat$,
\begin{equation}
\Cov[p_1^{\mathrm{unc}}]=\frac1N\sum_i(x^i-\bar x)(x^i-\bar x)^\top=\Xhat .
\label{eq:pf:distinction:1}
\end{equation}

\emph{Step 3: conclusion.} Both laws are finite mixtures of point masses at training
examples, hence supported on the training set, and Steps~1--2 give
\eqref{eq:distinction}. This proves Corollary~\ref{cor:distinction}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:atomicity}]
\emph{Step 1: the support.} Theorem~\ref{thm:endpoint} gives
$p_h^{\mathrm{gen}}(\cdot\mid y)=\sum_ip_i^{(h)}(y)\delta_{x^i}$, so
$\operatorname{supp}p_h^{\mathrm{gen}}\subseteq\{x^1,\dots,x^N\}$ for every $h>0$,
which is the first claim.

\emph{Step 2: a pointwise bound under any coupling.} Let $(X,X')$ be any coupling of
$p_h^{\mathrm{gen}}(\cdot\mid y)$ and $p(\cdot\mid y)$. By Step~1, $X\in\{x^i\}$
almost surely, and therefore, pointwise,
\begin{equation}
|X-X'|\ \ge\ \min_{1\le i\le N}|x^i-X'|\ =\ \operatorname{dist}(X',\{x^i\}).
\label{eq:pf:atom:1}
\end{equation}

\emph{Step 3: from the bound to the floor.} Squaring \eqref{eq:pf:atom:1} and taking
expectations, every coupling satisfies
\begin{equation}
\E\,|X-X'|^2\ \ge\ \E\,\operatorname{dist}(X',\{x^i\})^2
=\int\operatorname{dist}(x,\{x^i\})^2\,p(x\mid y)\,dx=:F(y),
\label{eq:pf:atom:2}
\end{equation}
and taking the infimum over couplings on the left gives
$W_2^2(p_h^{\mathrm{gen}},p(\cdot\mid y))\ge F(y)$, which is \eqref{eq:w2floor}.

\emph{Step 4: the floor is positive and free of $h$.} The right-hand side of
\eqref{eq:pf:atom:2} involves $p_h^{\mathrm{gen}}$ only through the set
$\{x^1,\dots,x^N\}$, which does not depend on $h$; and if $p(\cdot\mid y)$ is
absolutely continuous then the finite set $\{x^i\}$ is $p(\cdot\mid y)$-null, so the
integrand is strictly positive $p(\cdot\mid y)$-almost everywhere and $F(y)>0$. The
bound therefore holds at every bandwidth with the same strictly positive constant.
\end{proof}

\begin{proof}[Proof of Corollary~\ref{cor:infbw}]
Expanding the Gaussian kernel at large bandwidth, uniformly in $i$ as $h\to\infty$,
\begin{equation}
K_h(y-y^i)=(2\pi h^2)^{-k/2}\exp\Big(-\frac{|y-y^i|^2}{2h^2}\Big)
=(2\pi h^2)^{-k/2}\big[1+O(h^{-2})\big],
\label{eq:pf:infbw:1}
\end{equation}
the $O(h^{-2})$ being uniform because $\max_i|y-y^i|$ is a finite constant. The
prefactor is the same for every $i$ and cancels in the normalisation, so
\begin{equation}
p_i^{(h)}(y)=\frac{K_h(y-y^i)}{\sum_jK_h(y-y^j)}
=\frac{1+O(h^{-2})}{N+O(h^{-2})}\longrightarrow\frac1N .
\label{eq:pf:infbw:2}
\end{equation}
Substituting \eqref{eq:pf:infbw:2} into Theorem~\ref{thm:endpoint}'s endpoint law
gives $p_1\to\frac1N\sum_i\delta_{x^i}=\rhohat_X$, the unconditional empirical law.
\end{proof}

\begin{proof}[Proof of Corollary~\ref{cor:zerobw}]
Let $i^\star$ be the unique nearest label, so $|y-y^i|>|y-y^{i^\star}|$ for every
$i\ne i^\star$. The Gaussian prefactors cancel in the ratio of two kernel weights,
leaving
\begin{equation}
\frac{K_h(y-y^i)}{K_h(y-y^{i^\star})}
=\exp\Big[-\frac{|y-y^i|^2-|y-y^{i^\star}|^2}{2h^2}\Big]\xrightarrow[h\to0]{}0,
\label{eq:pf:zerobw:1}
\end{equation}
since the bracketed numerator is a strictly positive constant. Dividing numerator and
denominator of $p_i^{(h)}(y)$ by $K_h(y-y^{i^\star})$ and applying
\eqref{eq:pf:zerobw:1} to each of the $N-1$ terms $i\ne i^\star$,
\begin{equation}
p_{i}^{(h)}(y)\longrightarrow\mathbf 1\{i=i^\star\}\qquad (h\to0).
\label{eq:pf:zerobw:2}
\end{equation}
The endpoint law of Theorem~\ref{thm:endpoint} is a finite mixture whose weights are
exactly these, so it converges weakly to $\delta_{x^{i^\star}}$: the zero-bandwidth
limit is nearest-label collapse, as claimed.
\end{proof}
...
\emph{Step 1: Bayes' rule for the latent index.} By Bayes' rule applied to $I$,
\begin{equation}
\Pr(I=i\mid X_t=x,t,\widetilde Y=y)=
\frac{\Pr(I=i)\,p(x,y\mid I=i,t)}{\sum_j\Pr(I=j)\,p(x,y\mid I=j,t)}.
\label{eq:bayes-index}
\end{equation}
\emph{Step 2: the two observations are conditionally independent.} Given $I=i$,
$X_t=(1-t)X_0+tx^i$ is a deterministic affine function of $X_0$ alone
...
cancels identically for the same reason).

\emph{Step 3: the per-index target.} Given $I=i$, one further conditions on
$X_t=x$: by Lemma~\ref{lem:cov} this determines $X_0=(x-tx^i)/(1-t)$, so
$U=X_1-X_0=x^i-X_0=(x^i-x)/(1-t)$, exactly as in
Proposition~\ref{prop:collapse}(a).

\emph{Step 4: averaging over the posterior.} By Proposition~\ref{prop:projection}, the
...
\]
which is \eqref{eq:kernel-field}: the minimiser is a weighted average of the per-atom
collapsed fields, the label entering only through the weights.
\end{proof}
...
\emph{Step 1: the index posterior of the coupling.} The coupling differs from the one
in Proposition~\ref{prop:uncond} only in its prior on $I$, so Bayes' rule with
Lemma~\ref{lem:cov}'s density and prior $p_i^{(h)}(y)$ gives
\begin{equation}
\Pr(I=i\mid X_t=x,t)\ \propto\ p_i^{(h)}(y)\,(1-t)^{-d}
\pi_0\!\big(\tfrac{x-tx^i}{1-t}\big)
\ \propto\ \pi_0\!\big(\tfrac{x-tx^i}{1-t}\big)\,K_h(y-y^i),
\label{eq:pf:mix:1}
\end{equation}
the second proportionality because $p_i^{(h)}(y)\propto K_h(y-y^i)$ by definition and
$(1-t)^{-d}$ does not depend on $i$.

\emph{Step 2: the two posteriors coincide.} The right-hand side of
\eqref{eq:pf:mix:1} is, as a function of $i$ at fixed $(x,t,y)$, exactly the
numerator of $w_i^{(h)}$ in \eqref{eq:kernel-field}; both are then normalised to sum
to one over $i$, so $\Pr(I=i\mid X_t=x,t)=w_i^{(h)}(x,t,y)$.

\emph{Step 3: the velocity.} Given $I=i$, Step~1 of the proof of
Proposition~\ref{prop:collapse} gives $X_1-X_0=(x^i-x)/(1-t)$, so averaging over
Step~2's posterior,
\[
\E[X_1-X_0\mid X_t=x,t]=\sum_iw_i^{(h)}(x,t,y)\,\frac{x^i-x}{1-t}=\vstar(x,t,y),
\]
which is the claim: the label-smoothed minimiser is the conditional velocity of a
coupling that never leaves the atoms.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:endpoint}]
Fix $h>0$ and a query $y$, and let $(I,X_0,X_1,X_t)$ be the mixture coupling of
Lemma~\ref{lem:mixture-coupling}.

\emph{Step 1: the field is the coupling's own velocity.} By
Lemma~\ref{lem:mixture-coupling},
\begin{equation}
\vstar(x,t,y)=\E[X_1-X_0\mid X_t=x,t]
=\sum_iw_i^{(h)}(x,t,y)\,\frac{x^i-x}{1-t},\qquad t<1,
\label{eq:pf:endpoint:1}
\end{equation}
so the field whose flow we must study is exactly the conditional velocity of that
coupling, and not merely a field with the same formula.

\emph{Step 2: the flow is well posed on $[0,1)$.} The right-hand side of
\eqref{eq:pf:endpoint:1} has the form $v(x,t)=\sum_iw_i(x,t)(x^i-x)/(1-t)$ required by
Lemma~\ref{lem:wellposed}, and its weights are smooth and sum to one: each
$w_i^{(h)}$ is a ratio whose numerator and denominator are smooth and strictly
positive in $(x,t)$, by the positivity of $\pi_0$ recorded after Lemma~\ref{lem:cov}.
Lemma~\ref{lem:wellposed} therefore applies, and the ODE $\dot x_t=\vstar(x_t,t,y)$
has a unique solution from every initial condition, with flow maps
$(\Phi_t)_{t<1}$.

\emph{Step 3: the flow transports $\pi_0$ to the coupling's marginals.} Both
$(\Phi_t)_\#\pi_0$ and $\mathrm{Law}(X_t)$ solve the continuity equation
\begin{equation}
\partial_t p_t+\nabla\!\cdot\!\big(p_t\,\vstar(\cdot,t,y)\big)=0,
\qquad p_0=\pi_0,
\label{eq:pf:endpoint:2}
\end{equation}
the first because it is the push-forward of a well-posed flow and the second by the
superposition principle, $\vstar$ being the coupling's conditional velocity by
Step~1. Lemma~\ref{lem:wellposed} gives uniqueness of solutions of
\eqref{eq:pf:endpoint:2}, so $(\Phi_t)_\#\pi_0=\mathrm{Law}(X_t)=:p_t$ for every
$t<1$.

\emph{Step 4: the endpoint.} Under the coupling, $X_t=(1-t)X_0+tx^I$ with
$X_0\sim\pi_0$ independent of $I$, so for every realisation of $I$,
\begin{equation}
X_t\longrightarrow x^I\quad\text{almost surely as }t\uparrow1 .
\label{eq:pf:endpoint:3}
\end{equation}
Almost-sure convergence implies convergence in law, so by \eqref{eq:pf:endpoint:3}
and Step~3,
\[
p_t=\mathrm{Law}(X_t)\ \Longrightarrow\ \mathrm{Law}(x^I)
=\sum_ip_i^{(h)}(y)\,\delta_{x^i},
\]
and Lemma~\ref{lem:wellposed} identifies that weak limit with
$p_1=\lim_{t\uparrow1}p_t$. This is \eqref{eq:endpoint}: the endpoint law is the
Nadaraya--Watson mixture over the training atoms, with weights $p^{(h)}(y)$.
\end{proof}


\begin{proof}[Proof of Proposition~\ref{prop:moments}]
\emph{Step 1: the moments of a finite mixture.} By Theorem~\ref{thm:endpoint} the endpoint
law is $\sum_ip_i^{(h)}(y)\delta_{x^i}$, so its mean and covariance are the weighted sums
\begin{equation}
\bar x_h(y)=\sum_ip_i^{(h)}(y)\,x^i,\qquad
\Covh(y)=\sum_ip_i^{(h)}(y)\,(x^i-\bar x_h(y))(x^i-\bar x_h(y))^\top,
\label{eq:pf:moments:1}
\end{equation}
which involve only the training set and the kernel weights, hence are computable exactly
for every $h$.

\emph{Step 2: $\tr\Covh(y)$ need not be monotone.} It suffices to exhibit one instance.
Take $k=d=1$, $y=0$ and the three training pairs $(y^i,x^i)=(0,0),(1,10),(10,0)$. By
\eqref{eq:pf:moments:1}, $\tr\Covh=\sum_ip_i^{(h)}(x^i-\bar x_h)^2$. As $h\to0$ the
weights tend to $(1,0,0)$ (Corollary~\ref{cor:zerobw}), so $\tr\Covh\to0$. At $h=2$ the
weights are $(0.531,\,0.469,\,2\times10^{-6})$ and $\tr\Covh=24.90$. As $h\to\infty$ the
weights tend to $(\tfrac13,\tfrac13,\tfrac13)$ (Corollary~\ref{cor:infbw}) and
$\tr\Covh\to\tr\Xhat=200/9=22.22$. A function that rises from $0$ to $24.90$ and then
falls to $22.22$ is not monotone. This proves Proposition~\ref{prop:moments}.
\end{proof}
...
\emph{Step 1: an exact identity via a tilted population measure.} Define the
...
$O_P((Nh^k)^{-1/2})$ term addressed in Step~5. It therefore suffices to show
\begin{equation}
...
\emph{Step 2: the tilted law of $Y$, rescaled.} Substitute $y'=y+hu$
...
+\delta_{ac}\delta_{bd}+\delta_{ad}\delta_{bc}$, \eqref{eq:pf:expansion:3} gives the moments
\begin{align}
\E_{\tilde q_h}[u]&=\E_\varphi[u]+h\,\E_\varphi\!\big[u\,(s^\top u)\big]+O(h^2)
=h\,\E_\varphi[uu^\top]s+O(h^2)=h\,s(y)+O(h^2),
\label{eq:pf:expansion:4}\\
\E_{\tilde q_h}[uu^\top]&=\E_\varphi[uu^\top]+h\,\E_\varphi\!\big[uu^\top(s^\top u)\big]+O(h^2)
=I_k+0+O(h^2),
\label{eq:pf:expansion:5}
\end{align}
the vanishing middle term in \eqref{eq:pf:expansion:5} because $\E_\varphi[u_au_bu_c]=0$ for
...
\emph{Step 3: within-group term.} Here the homoscedasticity hypothesis is used, and
...
Gaussian-tail estimate as in Step~4 below, hence $o(h^p)$ for every $p$ and absorbed into
...
substitution together with \eqref{eq:pf:expansion:4} and \eqref{eq:pf:expansion:5},
\begin{align*}
\E_h[\Sigma(Y)]
...
Step~4, so the two cannot be separated and $\|J\|_F^2$ is not identifiable from
...
\emph{Step 4: between-group term.} First the tail. Split
...
are absorbed into $O(h^4)$ and we may drop $R_h$ from what follows. Since covariance is invariant to the additive constant $\mu(y)$,
\begin{equation}
\begin{aligned}
...
\end{aligned}
\label{eq:pf:expansion:7}
\end{equation}
The leading term is $h^2J(y)\big[I_k+O(h^2)\big]J(y)^\top=h^2J(y)J(y)^\top+O(h^4)$ by
\eqref{eq:pf:expansion:6}. It remains to show the middle, apparently $O(h^3)$, cross term is
...
\eqref{eq:pf:expansion:3} to the relevant order,
\begin{align*}
...
term in \eqref{eq:pf:expansion:7} is $h^3\cdot O(h)=O(h^4)$, absorbed into the
...
Substituting Steps~3--4 into \eqref{eq:pf:expansion:2} proves
\eqref{eq:pf:expansion:1}, i.e.\ the deterministic part of \eqref{eq:expansion}.

\emph{Step 5: Monte-Carlo fluctuation.} Write $S_N:=\frac1N\sum_jK_h(y-y^j)$ and
...
remaining term of \eqref{eq:expansion}. Together with \eqref{eq:pf:expansion:1},
established in Steps~3--4, this proves Proposition~\ref{prop:expansion}.
\end{proof}
...
\emph{Step 1: the law of the interpolant given the index.} Fix $(x,t,y)$ with $t<1$ and
condition on $I=i$. Then $X_t=(1-t)X_0+tx^i+t\rho\xi$, so
$r:=x-tx^i=(1-t)X_0+t\rho\xi$ is a linear combination of the independent standard
Gaussians $X_0,\xi$, and
\begin{equation}
r\sim\N(0,s_t^2I_d),\qquad s_t^2=(1-t)^2+t^2\rho^2,\qquad
p(x\mid I=i,t)=s_t^{-d}\varphi_1(r/s_t)=\varphi_{s_t}(x-tx^i).
\label{eq:pf:tgtnoise:1}
\end{equation}

\emph{Step 2: the index posterior.} As in Step~2 of the proof of
...
$X_t\perp\widetilde Y\mid I=i$. Bayes' rule with \eqref{eq:pf:tgtnoise:1} and the
uniform prior on $I$ gives
\begin{equation}
\Pr(I=i\mid X_t=x,t,\widetilde Y=y)=w_i\propto\varphi_{s_t}(x-tx^i)\,K_h(y-y^i),
\label{eq:pf:tgtnoise:2}
\end{equation}
the $i$-independent factor $1/N$ cancelling as before.

\emph{Step 3: the per-index target.} The vector $(X_0,\xi,r)$ is jointly Gaussian with
$\Cov(X_0,r)=(1-t)I_d$ and $\Cov(\xi,r)=t\rho I_d$, so the Gaussian conditional-mean
formula $\E[a\mid r]=\Cov(a,r)\Var(r)^{-1}r$ gives
\begin{equation}
\E[X_0\mid r]=\tfrac{1-t}{s_t^2}\,r,\qquad
\E[\xi\mid r]=\tfrac{t\rho}{s_t^2}\,r.
\label{eq:pf:tgtnoise:3}
\end{equation}
Since $U=X_1-X_0=x^i+\rho\xi-X_0$, \eqref{eq:pf:tgtnoise:3} yields
\begin{equation}
...
\emph{Step 4: the field.} By Proposition~\ref{prop:projection},
$\vstar=\E[U\mid X_t=x,t,\widetilde Y=y]$, which is \eqref{eq:pf:tgtnoise:4} averaged
over the posterior \eqref{eq:pf:tgtnoise:2}:
\begin{equation}
...
\emph{Step 5: the endpoint law.} Argue exactly as in Lemma~\ref{lem:mixture-coupling}
and Theorem~\ref{thm:endpoint}. Consider the coupling $I\sim p^{(h)}(y)$,
$X_0\sim\pi_0$, $\xi\sim\N(0,I_d)$ mutually independent, $X_1:=x^I+\rho\xi$, with the
same linear interpolant. By \eqref{eq:pf:tgtnoise:1} its index posterior at $(x,t)$ is
$\propto p_i^{(h)}(y)\,\varphi_{s_t}(x-tx^i)$, which has the same proportionality in $i$
as $w_i$ because $p_i^{(h)}(y)\propto K_h(y-y^i)$; and \eqref{eq:pf:tgtnoise:4} holds
for it unchanged. So its conditional velocity is $\vstar$. Its weights are smooth,
positive and sum to one, so Lemma~\ref{lem:wellposed} applies and the flow transports
$\pi_0$ to $\mathrm{Law}(X_t)$ for every $t<1$. Since $X_t=(1-t)X_0+tX_1\to X_1$ almost
surely as $t\uparrow1$,
\begin{equation}
\mathrm{Law}(X_t)\Rightarrow\mathrm{Law}(x^I+\rho\xi)
=\sum_ip_i^{(h)}(y)\,\N(x^i,\rho^2I_d),
\label{eq:pf:tgtnoise:5}
\end{equation}
which is \eqref{eq:tgtendpoint}.

\emph{Step 6: moments, continuity and regularity at $t=1$.} The mean of
\eqref{eq:pf:tgtnoise:5} is $\sum_ip_i^{(h)}x^i=\bar x_h(y)$ and, by the law of total
covariance applied to the mixture,
\begin{equation}
\Cov=\underbrace{\rho^2I_d}_{\text{within components}}
+\underbrace{\Covh(y)}_{\text{between components}}.
\label{eq:pf:tgtnoise:6}
\end{equation}
Each component is absolutely continuous, hence so is the mixture, and
\eqref{eq:w2floor}, which was derived from $\operatorname{supp}\subseteq\{x^i\}$, does
not apply. Finally $s_1^2=\rho^2>0$, so $t\mapsto(t\rho^2-(1-t))/s_t^2$ is continuous at
$t=1$ with value $1$, and by \eqref{eq:tgtfield} $\vstar$ is bounded on bounded sets up
to and including $t=1$. This proves Proposition~\ref{prop:tgtnoise}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:nwrate}]
\emph{Step 1: the classical bias and variance.} The hypotheses under which the pointwise
Nadaraya--Watson bias and variance are usually stated are exactly those already imposed
in Assumption~\ref{as:design}: $(x^j,y^j)$ i.i.d.; $y$ interior to
$\operatorname{supp}(\rho_Y)$ with $\rho_Y(y)>0$ and $\rho_Y\in C^2$ near $y$;
$\mu\in C^2$ near $y$; together with $K$ a bounded, symmetric, compactly supported (or
sufficiently rapidly decaying) kernel with $\int K=1$ and finite second moment, and the
regime $h\to0$, $Nh^k\to\infty$. Under precisely these, with $\mu_2(K)=\int u_1^2K(u)\,du$
and $R(K)=\int K^2$,
\begin{align}
\mathrm{Bias}&=\tfrac{h^2}{2}\,\mu_2(K)\,b(y)+o(h^2),
\label{eq:pf:nwrate:1}\\
\Var&=\tfrac{R(K)}{Nh^k}\cdot\tfrac{\Sigma(y)}{\rho_Y(y)}+o\big((Nh^k)^{-1}\big),
\label{eq:pf:nwrate:2}
\end{align}
with $b(y)$ as in \eqref{eq:nwbiascoef}; see
\citet[\S1.5 and \S1.6]{tsybakov2009nonparametric} or
\citet[Ch.~5]{gyorfi2002distribution}. We claim no novelty for either display. The
constant in \eqref{eq:pf:nwrate:1} is the one obtained by Taylor-expanding $\mu(y^j)$
about $y$ against the symmetric kernel weights, which is the same computation as the
$O(h^2)$ term in Step~4 of the proof of Proposition~\ref{prop:expansion}.

\emph{Step 2: the optimal bandwidth.} Adding the square of \eqref{eq:pf:nwrate:1} to
\eqref{eq:pf:nwrate:2}, with $c_1=\tfrac14\mu_2(K)^2|b(y)|^2$ and $c_2$ the constant of
\eqref{eq:pf:nwrate:2},
\begin{equation}
\mathrm{MSE}(h)=c_1h^4+c_2(Nh^k)^{-1}+o(\cdot),\qquad
\frac{d}{dh}\mathrm{MSE}=4c_1h^3-c_2\,k\,N^{-1}h^{-k-1}.
\label{eq:pf:nwrate:3}
\end{equation}
The derivative in \eqref{eq:pf:nwrate:3} vanishes exactly when
\begin{equation}
h^{k+4}=\frac{c_2\,k}{4c_1N},\qquad\text{i.e.}\qquad h^\star\asymp N^{-1/(k+4)},
\label{eq:pf:nwrate:4}
\end{equation}
which needs $c_1,c_2>0$; the second derivative is positive at
\eqref{eq:pf:nwrate:4}, so it is the minimiser. This is the proviso in the statement.

\emph{Step 3: the linear-Gaussian instance.} The population version of $\bar x_h$ is the
$\rho_Y$-average of $\mu$ against the kernel,
\begin{equation}
\bar x_h(y)=\frac{\E[\mu(Y)\,K_h(y-Y)]}{\E[K_h(y-Y)]}.
\label{eq:pf:nwrate:5}
\end{equation}
With $K_h=\N(0,h^2I_k)$ and $\rho_Y=\N(0,\Sigma_Y)$ the product of the two densities is,
in $y'$, an unnormalised Gaussian with precision $\Sigma_Y^{-1}+h^{-2}I_k$ and mean
\begin{equation}
(\Sigma_Y^{-1}+h^{-2}I_k)^{-1}h^{-2}y=\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}y.
\label{eq:pf:nwrate:6}
\end{equation}
Since $\mu(y')=By'$ is affine, averaging it against that Gaussian just evaluates $B$ at
the mean \eqref{eq:pf:nwrate:6}, so \eqref{eq:pf:nwrate:5} gives
$\bar x_h(y)=B\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}y$ and
\begin{equation}
\bar x_h(y)-By=B\big[\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}-I_k\big]y
=-B\,h^2(\Sigma_Y+h^2I_k)^{-1}y,
\label{eq:pf:nwrate:7}
\end{equation}
which is \eqref{eq:lgbias}. Expanding $(\Sigma_Y+h^2I_k)^{-1}=\Sigma_Y^{-1}+O(h^2)$
gives the stated $-h^2B\Sigma_Y^{-1}y+O(h^4)$, and $b(y)=-2B\Sigma_Y^{-1}y$ follows from
\eqref{eq:nwbiascoef} with $\Delta\mu=0$ and $\nabla\log\rho_Y(y)=-\Sigma_Y^{-1}y$.
Note that \eqref{eq:pf:nwrate:7} is an exact statement about the population kernel
average at every $h$, not only asymptotically. Both the closed form and its agreement
with \eqref{eq:nwbiascoef} are checked numerically in
\texttt{scripts/verify\_nw\_bias\_affine.py}, which also exhibits the U-shaped
finite-$N$ MSE at $y\neq0$ and its monotone counterpart at $y=0$.

\emph{Step 4: design points.} At $y=y^i$ the summand $j=i$ carries the weight
\begin{equation}
\frac{K_h(0)}{\sum_jK_h(y^i-y^j)},\qquad \sum_jK_h(y^i-y^j)\asymp Nh^k
\ \text{in probability},
\label{eq:pf:nwrate:8}
\end{equation}
so under $Nh^k\to\infty$ that weight is $\Theta\big((Nh^k)^{-1}\big)\to0$. When
$Nh^k\not\to\infty$ it does not vanish and the expansion above does not apply---which is
exactly the fixed-$N$, $h\to0$ regime of Corollary~\ref{cor:zerobw}, where the weight
tends to $1$ instead. This proves Proposition~\ref{prop:nwrate}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:prop17}]
\emph{Step 1: the conditional means.} Fix $i$ and write $r:=x-tx^i$. Given $I=i$,
$X_t=(1-t)X_0+tx^i+\gamma(t)Z$, so $r=(1-t)X_0+\gamma(t)Z$ is a linear combination of
the independent standard Gaussians $X_0,Z\sim\N(0,I_d)$, and
\begin{equation}
r\sim\N(0,s_t^2I_d),\qquad s_t^2=(1-t)^2+\gamma(t)^2 .
\label{eq:pf:prop17:1}
\end{equation}
The pairs $(X_0,r)$ and $(Z,r)$ are jointly Gaussian with $\Cov(X_0,r)=(1-t)I_d$ (the
term $\gamma(t)Z$ is independent of $X_0$) and $\Cov(Z,r)=\gamma(t)I_d$, so the Gaussian
conditional-mean formula $\E[a\mid r]=\Cov(a,r)\Var(r)^{-1}r$ gives
\begin{equation}
\E[X_0\mid r]=\tfrac{1-t}{s_t^2}\,r,\qquad
\E[Z\mid r]=\tfrac{\gamma(t)}{s_t^2}\,r .
\label{eq:pf:prop17:2}
\end{equation}

\emph{Step 2: part (a), the field.} By Assumption~\ref{as:standing}(iii), $Y=y^i$ pins
$I=i$, so Proposition~\ref{prop:projection} applied to the target \eqref{eq:c2target},
$\dot X_t=x^i-X_0+\dot\gamma(t)Z$, and then \eqref{eq:pf:prop17:2} give
\begin{equation}
\begin{aligned}
...
\end{aligned}
\label{eq:pf:prop17:3}
\end{equation}
Since $\gamma(t)^2=\sigma^2t(1-t)$, differentiating gives
\begin{equation}
2\dot\gamma\gamma=\sigma^2(1-2t),\qquad\text{i.e.}\qquad
\dot\gamma\gamma=\tfrac{\sigma^2}2(1-2t),
\label{eq:pf:prop17:4}
\end{equation}
and substituting \eqref{eq:pf:prop17:4} into the coefficient of \eqref{eq:pf:prop17:3}
gives the expression for $c(t)$ in (a). For the logarithmic-derivative form,
$\tfrac{d}{dt}s_t^2=-2(1-t)+2\dot\gamma\gamma=2s_t^2c(t)$ by \eqref{eq:pf:prop17:4}, so
$c(t)=\tfrac12\tfrac{d}{dt}\log s_t^2$.

\emph{Step 3: part (b), the flow.} Along a trajectory put $u_t:=x_t-tx^i$. By (a) with
$r=u_t$,
\begin{equation}
\dot u_t=\dot x_t-x^i=v_\sigma^\star(x_t,t,y^i)-x^i=c(t)\,u_t,
\label{eq:pf:prop17:5}
\end{equation}
a linear ODE, whose solution is
\begin{equation}
u_t=u_0\exp\!\Big(\int_0^tc(\tau)\,d\tau\Big)
=x_0\exp\!\big(\tfrac12[\log s_t^2-\log s_0^2]\big)=s_t\,x_0,
\label{eq:pf:prop17:6}
\end{equation}
using the logarithmic-derivative form of $c$ from Step~2, $s_0=1$ and
$u_0=x_0-0\cdot x^i=x_0$. Hence $x_t=tx^i+s_tx_0$.

\emph{Step 4: part (c), the endpoint.} Since $s_t^2=(1-t)[(1-t)+\sigma^2t]$ we have
$s_1=0$, so \eqref{eq:pf:prop17:6} gives $x_t\to x^i$ as $t\uparrow1$ for every
$x_0\sim\pi_0$ and every $\sigma\ge0$. Hence
$p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$, which is \eqref{eq:invariance}.
\end{proof}
...
\emph{Step 1: endpoint invariance.} This is Proposition~\ref{prop:prop17}(c).

\emph{Step 2: the irreducible error, $\sigma=0$.} Here $\gamma\equiv0$, so
$\dot X_t=X_1-X_0$ is exactly the target of Proposition~\ref{prop:collapse}, whose
part~(b) gives $\inf_vL_0=0$.

\emph{Step 3: the irreducible error, $\sigma>0$.} By the projection argument of
Proposition~\ref{prop:projection}, applied to $\dot X_t$ and $(X_t,t,Y)$,
$\inf_vL_\sigma(v)=\E[\Var(\dot X_t\mid X_t,t,Y)]$, so it suffices to show
$\Var(\dot X_t\mid X_t=x,t,I=i)>0$ on a positive-measure set of $t$. Write
$\dot X_t-x^i=u\cdot(X_0,Z)$ and $r=w\cdot(X_0,Z)$ with $u=(-1,\dot\gamma(t))$ and
$w=(1-t,\gamma(t))$, as in Step~1 of the proof of Proposition~\ref{prop:prop17}. The
standard Gaussian conditioning formula, with $|w|^2=s_t^2$ and
$u\cdot w=-(1-t)+\dot\gamma\gamma=c(t)s_t^2$ (Step~2 of that proof), gives
\begin{equation}
\Var(\dot X_t\mid r,I=i)=|u|^2-\frac{(u\cdot w)^2}{|w|^2}
=\big(1+\dot\gamma(t)^2\big)-c(t)^2s_t^2 .
\label{eq:pf:prop18:1}
\end{equation}
This vanishes only if $u\parallel w$, i.e.\ only if
\begin{equation}
\dot\gamma(t)(1-t)=-\gamma(t).
\label{eq:pf:prop18:2}
\end{equation}
Substituting $\gamma(t)=\sigma\sqrt{t(1-t)}$ and
$\dot\gamma(t)=\sigma(1-2t)/(2\sqrt{t(1-t)})$ reduces \eqref{eq:pf:prop18:2} to
$1-2t=-2t$, i.e.\ $1=0$, which is impossible. So for $\sigma>0$ the residual variance
\eqref{eq:pf:prop18:1} is strictly positive for \emph{every} $t\in(0,1)$, and
$\inf_vL_\sigma(v)=\E[\Var(\dot X_t\mid X_t,t,Y)]>0$.

\emph{Step 4: the Lipschitz rate.} From part~(a) of Proposition~\ref{prop:prop17},
\begin{equation}
(1-t)\,c(t)=\frac{-(1-t)+\frac{\sigma^2}2(1-2t)}{(1-t)+\sigma^2t},
\label{eq:pf:prop18:3}
\end{equation}
which as $t\uparrow1$ tends to $-1$ if $\sigma=0$, and to
$-\tfrac{\sigma^2}{2}\big/\sigma^2=-\tfrac12$ if $\sigma>0$. This gives the two limits of
$(1-t)|c(t)|$, and completes the proof of Corollary~\ref{cor:prop18}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:factors}]
\emph{Step 1: the label factor moves the endpoint.} By Theorem~\ref{thm:endpoint}, for
$h>0$ and $\sigma=0$ the endpoint law
\begin{equation}
p_h^{\mathrm{gen}}(\cdot\mid y)=\sum_ip_i^{(h)}(y)\,\delta_{x^i}
\label{eq:pf:factors:1}
\end{equation}
depends on $h$ only through the label factor $K_h(y-y^i)$ of \eqref{eq:kernel-field}.
By Corollaries~\ref{cor:zerobw} and~\ref{cor:infbw}, \eqref{eq:pf:factors:1}
interpolates between the two distinct laws $\delta_{x^{i^\star}}$ (as $h\to0$, at a $y$ with a
unique nearest label $i^\star$) and
$\rhohat_X$ (as $h\to\infty$), so perturbing the label factor changes $p_1$.

\emph{Step 2: the spatial factor does not.} Take $h=0$ and any $\sigma\ge0$. By
Assumption~\ref{as:standing}(iii), $Y=y^i$ already pins $I=i$, so no index-posterior
argument is needed. Interpolant noise replaces the spatial factor of
Lemma~\ref{lem:cov},
\begin{equation}
\N\big(tx^i,(1-t)^2I_d\big)\ \longrightarrow\ \N\big(tx^i,s_t^2I_d\big),
\label{eq:pf:factors:2}
\end{equation}
identically for every $i$, and Proposition~\ref{prop:prop17}(c) shows that despite
\eqref{eq:pf:factors:2} the endpoint is exactly invariant,
$p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$ for every $\sigma$.

\emph{Step 3: conclusion.} By Steps~1 and~2, perturbing the label factor changes $p_1$
while perturbing the spatial factor identically across $i$ does not, so the two
mechanisms act through different, non-interchangeable channels. This proves
Proposition~\ref{prop:factors}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:liplb}]
Let $X,X'$ be i.i.d.\ copies of $X_t\mid I=i\sim\N(tx^i,(1-t)^2I_d)$ and put
$e(\cdot):=f_i(\cdot,t)-v(\cdot,t,y^i)$.

\emph{Step 1: the target's increments.} Since $f_i(X,t)-f_i(X',t)=-(X-X')/(1-t)$ and
$X-X'\sim\N(0,2(1-t)^2I_d)$,
\begin{equation}
\E|f_i(X,t)-f_i(X',t)|^2=\frac{2(1-t)^2d}{(1-t)^2}=2d .
\label{eq:pf:liplb:1}
\end{equation}

\emph{Step 2: the model's increments.} By $L$-Lipschitzness of $v(\cdot,t,y^i)$ and
$\E|X-X'|^2=2d(1-t)^2$,
\begin{equation}
\E|v(X,t,y^i)-v(X',t,y^i)|^2\le L^2\,\E|X-X'|^2=2dL^2(1-t)^2 .
\label{eq:pf:liplb:2}
\end{equation}

\emph{Step 3: the residual's increments.} Since $X,X'$ are i.i.d.,
\begin{equation}
\E|e(X)-e(X')|^2=2\E|e(X)|^2-2\big|\E e(X)\big|^2\le2\,\E|e(X)|^2 .
\label{eq:pf:liplb:3}
\end{equation}
Using this sharp identity, rather than the cruder $\E|e(X)-e(X')|^2\le4\E|e(X)|^2$, is
what preserves the constant $d$ in the bound; a weaker step would give only a smaller
floor.

\emph{Step 4: the triangle inequality.} Write
$f_i(X,t)-f_i(X',t)=[e(X)-e(X')]+[v(X,t,y^i)-v(X',t,y^i)]$ and apply the
$L^2(\Omega)$ triangle inequality. By \eqref{eq:pf:liplb:1}--\eqref{eq:pf:liplb:3},
\begin{equation}
\sqrt{2d}\ \le\ \sqrt{2\,\E|e(X)|^2}\ +\ \sqrt{2d}\,L(1-t),
\label{eq:pf:liplb:4}
\end{equation}
hence $\sqrt{\E|e(X)|^2}\ge\sqrt d\,[1-L(1-t)]_+$, i.e.\
$\E|e(X)|^2\ge d\,[1-L(1-t)]_+^2$. Since $e(X)=f_i(X_t,t)-v(X_t,t,y^i)$ is the negative
of the residual in the statement, and the square does not see the sign, this proves
Proposition~\ref{prop:liplb}.
\end{proof}

\begin{proof}[Proof of Corollary~\ref{cor:floor}]
\emph{Step 1: the integral.} Substituting $s=1-t$,
\begin{equation}
...
\emph{Step 2: averaging the pointwise bound.} Proposition~\ref{prop:liplb}'s bound
holds for every training index $i$, hence for every $v\in\mathcal F_L$ and every
$t\in[1-1/L,1)$, where $[1-L(1-t)]_+=1-L(1-t)$; it therefore survives averaging over
$I\sim\mathrm{Unif}\{1,\dots,N\}$ and $t\sim\mathrm{Unif}(0,1)$:
\begin{equation}
\inf_{v\in\mathcal F_L}L_0(v)\ \ge\ d\int_{1-1/L}^1[1-L(1-t)]^2\,dt\ =\ \frac d{3L},
\label{eq:pf:floor:2}
\end{equation}
the last equality being \eqref{eq:pf:floor:1} with $s=1-t$. This is \eqref{eq:floor}.
\end{proof}
...
\emph{Step 1: the guided field.} Proposition~\ref{prop:kernel-field} and
Proposition~\ref{prop:uncond} give
\begin{equation}
\vstar=\sum_iw_i^{(h)}\,\frac{x^i-x}{1-t},\qquad
v_{\mathrm{unc}}^\star=\sum_iq_i\,\frac{x^i-x}{1-t},
\label{eq:pf:cfg:1}
\end{equation}
both with weights summing to one. In $v_w=(1+w)\vstar-w\,v_{\mathrm{unc}}^\star$ the $-x$
terms combine as $-(1+w)x+wx=-x$, which is \eqref{eq:cfgfield}.

\emph{Step 2: $m_w$ is an affine combination of the atoms.} The coefficients in
\eqref{eq:cfgfield} sum to $(1+w)\cdot1-w\cdot1=1$, so $m_w(x,t)\in\mathcal A$ for
every $(x,t)$ (though not, in general, in the convex hull, since the coefficients need
not be nonnegative).

\emph{Step 3: the orthogonal component.} $\Pi$ is affine with $\Pi z=z$ on $\mathcal A$,
so applying $I-\Pi$ to $\dot x_t=(m_w(x_t,t)-x_t)/(1-t)$ annihilates $m_w\in\mathcal A$
and leaves
\begin{equation}
\dot q_t=-\frac{q_t}{1-t},\qquad\text{whose solution is}\qquad q_t=(1-t)\,q_0,
\label{eq:pf:cfg:2}
\end{equation}
which is \eqref{eq:cfgorth}.

\emph{Step 4: the endpoint.} By \eqref{eq:pf:cfg:2}, $\operatorname{dist}(x_t,\mathcal A)
=|q_t|=(1-t)|q_0|\to0$. Since $\mathcal A$ is closed, the endpoint of every convergent
trajectory lies in $\mathcal A$, and so does the support of the endpoint law.

\emph{Step 5: well-posedness.} It is Lemma~\ref{lem:wellposed} with the nonnegativity of
the weights relaxed to boundedness. Here $|m_w|\le(1+2w)\max_i|x^i|$, so the growth
estimate \eqref{eq:vgrowth} holds with $M$ replaced by $(1+2w)M$ and the rest of that
argument is unchanged. This proves Proposition~\ref{prop:cfg}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:nearduplicate}]
Write $K_i=K_h(y-y^i)=c\,e^{-|y-y^i|^2/2h^2}$ with $c$ the common normalising constant,
so $p_i^{(h)}=K_i/\sum_jK_j$.

\emph{Step 1: part (a), mass outside $C$.} Bounding the denominator below by its $C$-part,
\begin{equation}
\sum_{j\notin C}p_j^{(h)}\le\frac{\sum_{j\notin C}K_j}{\sum_{i\in C}K_i}
\le\frac{(N-|C|)\,c\,e^{-\Delta^2/2h^2}}{|C|\,c\,e^{-\varepsilon^2/2h^2}}=\eta,
\label{eq:pf:nearduplicate:1}
\end{equation}
using $|y-y^j|\ge\Delta$ for the numerator and $|y-y^i|\le\varepsilon$ for the
denominator.

\emph{Step 2: part (b), ratios inside $C$.} For $i,i'\in C$,
\begin{equation}
\frac{K_i}{K_{i'}}=\exp\!\Big(\frac{|y-y^{i'}|^2-|y-y^i|^2}{2h^2}\Big)
\le\exp\!\Big(\frac{\varepsilon^2}{2h^2}\Big)=\kappa,
\label{eq:pf:nearduplicate:2}
\end{equation}
since $|y-y^{i'}|\le\varepsilon$ and $|y-y^i|\ge0$; and $p_i^{(h)}/p_{i'}^{(h)}=K_i/K_{i'}$.

\emph{Step 3: part (c), the effective sample size.} Let $S=\sum_{i\in C}p_i^{(h)}\le1$.
Summing \eqref{eq:pf:nearduplicate:2} over $i'\in C$ gives $|C|\,p_i^{(h)}\le\kappa S$ for
each $i\in C$, so
\begin{equation}
...
\label{eq:pf:nearduplicate:3}
\end{equation}
the second by \eqref{eq:pf:nearduplicate:1}. Adding the two and inverting,
$n_{\mathrm{eff}}=1/\sum_ip_i^{(h)2}\ge1/(\kappa/|C|+\eta^2)=|C|/(\kappa+|C|\eta^2)$.

\emph{Step 4: the limits.} As $\varepsilon/h\to0$ and $\Delta/h\to\infty$ we have
$\kappa\to1$ and $\eta\to0$. By \eqref{eq:pf:nearduplicate:1}, $S\ge1-\eta\to1$; by
\eqref{eq:pf:nearduplicate:2} the ratios inside $C$ lie in $[\kappa^{-1},\kappa]\to\{1\}$,
so the index posterior tends to the uniform law on $C$, and by
Theorem~\ref{thm:endpoint} the endpoint law to $|C|^{-1}\sum_{i\in C}\delta_{x^i}$. For
$n_{\mathrm{eff}}$, Step~3 gives the lower bound $|C|/(\kappa+|C|\eta^2)\to|C|$, and
Cauchy--Schwarz gives $\sum_ip_i^{(h)2}\ge S^2/|C|$, i.e.\ the upper bound
$n_{\mathrm{eff}}\le|C|/S^2\to|C|$. This proves Proposition~\ref{prop:nearduplicate}.
\end{proof}
...
\end{equation}
and integrating \eqref{eq:pf:cfgatom:1}, $\langle x_t,u\rangle\ge\langle x_{t_0},u\rangle+c\log\frac{1-t_0}{1-t}
...
\label{eq:pf:cfgatom:5}
\end{equation}
the guidance term cancelling exactly in the limit, and Step~1 applied to \eqref{eq:pf:cfgatom:5} gives $x^*=x^{i^*}$.
...
definition of $i^*$. Hence $i^*=i$ and $x^*=x^i$. This proves both parts of
Proposition~\ref{prop:cfgatom}.
\end{proof}
...
\emph{Step 1: the error obeys a contracting ODE.} Both trajectories start at $x_0$, so
$e_0=0$. By Proposition~\ref{prop:collapse}(a), $\vstarz(x,t,y^i)=(x^i-x)/(1-t)$ is
affine in $x$ with linear part $-\tfrac1{1-t}I_d$, so
\begin{equation}
...
\label{eq:pf:survival:1}
\end{equation}
The bracketed term is a \emph{contraction}, which is what makes the estimate go the way
it does.

\emph{Step 2: the identity and the bound.} Rather than estimate $|e_t|$ directly we
solve \eqref{eq:pf:survival:1}. $(1-t)^{-1}$ is an integrating factor: on $[0,T]$ for any
$T<1$ the map $t\mapsto e_t/(1-t)$ is absolutely continuous and, a.e.,
\begin{equation}
...
\label{eq:pf:survival:2}
\end{equation}
the two $e_t$ terms cancelling exactly by \eqref{eq:pf:survival:1}. Integrating
\eqref{eq:pf:survival:2} from $0$ to $t$ against $e_0=0$ and multiplying by $(1-t)$ gives
the identity \eqref{eq:survival-id}, and \eqref{eq:survival} follows by the triangle
inequality for the integral. This is an equality before norms are taken, which is what
parts (c) and (d) use.

\emph{Step 3: part (a), bounded errors do not survive.} By Cauchy--Schwarz applied to
\eqref{eq:survival}, and $\int_0^t(1-s)^{-2}ds=\frac1{1-t}-1=\frac t{1-t}$,
\begin{equation}
...
\label{eq:pf:survival:3}
\end{equation}
Setting $t=1-\delta$ in \eqref{eq:pf:survival:3} gives
$|e_{1-\delta}|\le\varepsilon\sqrt{\delta(1-\delta)}\le\varepsilon\sqrt\delta$.

\emph{Step 4: part (b), what the bound gives.} For $a>0$,
\begin{equation}
\int_0^t(1-s)^{-1-a}ds=\frac1a\big((1-t)^{-a}-1\big),
\label{eq:pf:survival:4}
\end{equation}
so \eqref{eq:survival} with $|\Delta|\le c(1-s)^{-a}$ gives
$|e_t|\le\frac ca\big((1-t)^{1-a}-(1-t)\big)$; at $a=1$ this is
$c\big(1-(1-t)\big)=c\,t\le c$. As $t\uparrow1$ the right-hand side tends to $0$ when
$a<1$, to $c$ when $a=1$, and to $+\infty$ when $a>1$; only the first two are statements
about $e_t$.

\emph{Step 5: part (c), the bound is attained.} With $\Delta(x,t)=c(1-t)^{-a}u$ the
integrand of \eqref{eq:survival-id} does not depend on the trajectory, so the identity
evaluates directly. For $a>0$, by \eqref{eq:pf:survival:4},
\begin{equation}
e_t=(1-t)\,c\,u\int_0^t(1-s)^{-1-a}ds
=\frac ca\big((1-t)^{1-a}-(1-t)\big)u ,
\label{eq:pf:survival:5}
\end{equation}
...
direction. Writing $\epsilon=1-t$, $|e_t|=\frac ca(\epsilon^{1-a}-\epsilon)$ has limit $0$
for $a<1$ (both powers vanish), limit $c$ for $a=1$ (the expression is $c(1-\epsilon)$
exactly), and limit $+\infty$ for $a>1$ (since $\epsilon^{1-a}\to\infty$). Finally
$v=\vstarz+\Delta$ is locally Lipschitz in $x$ on $\{t<1\}$ because $\vstarz$ is and
$\Delta$ is constant in $x$; the same constancy gives $L_\Delta=0$, so this family also
satisfies Assumption~\ref{as:lipschitz}.

\emph{Step 6: part (d), what a surviving deviation requires.} With
$I(t)=\int_0^t\Delta(x^v_s,s)(1-s)^{-1}ds$, \eqref{eq:survival-id} reads $e_t=(1-t)I(t)$
for every $t<1$, hence
\begin{equation}
e_t-e_\star=(1-t)\Big[I(t)-\frac{e_\star}{1-t}\Big].
\label{eq:pf:survival:6}
\end{equation}
The left-hand side of \eqref{eq:pf:survival:6} tends to $0$ exactly when the bracket is
$o\big((1-t)^{-1}\big)$, which is \eqref{eq:survival-conv}. The four cases listed in (d)
are the four possible behaviours of $(1-t)I(t)=e_t$ itself (tends to $0$, tends to
$e_\star\ne0$, has norm tending to $\infty$, or stays bounded without converging), so
they are exhaustive. This proves Proposition~\ref{prop:survival}.
\end{proof}

\begin{proof}[Proof of Corollary~\ref{cor:lossform}]
\emph{Step 1: the loss is the error's own norm.} By Proposition~\ref{prop:collapse}(a),
$U=\vstarz(X_t,t,y^i)$ almost surely, so $|v(X_t,t,y^i)-U|=|\Delta(X_t,t)|$ and
\begin{equation}
\mathcal L_i(v)=\int_0^1\ell(s)^2ds,\qquad
\ell(s)^2:=\E_{X_0}\big|\Delta(x^\star_s,s)\big|^2,
\label{eq:pf:lossform:1}
\end{equation}
where $x^\star_s=(1-s)X_0+sx^i$ has the law of $X_s$ by construction. Both
trajectories are pushforwards of the same $X_0$, which is what lets the two
expectations be compared at all.

\emph{Step 2: Minkowski.} Take $L^2(X_0)$ norms in \eqref{eq:survival}. By Minkowski's
integral inequality the norm passes inside the $ds$ integral, so
\begin{equation}
\mathcal E(t)\;\le\;(1-t)\int_0^t\frac{\big\|\Delta(x^v_s,s)\big\|_{L^2}}{1-s}\,ds .
\label{eq:pf:lossform:2}
\end{equation}

\emph{Step 3: closing the inequality with Assumption~\ref{as:lipschitz}.} Pointwise,
$|\Delta(x^v_s,s)|\le|\Delta(x^\star_s,s)|+L_\Delta|x^v_s-x^\star_s|$, hence by the
triangle inequality in $L^2(X_0)$,
\begin{equation}
\|\Delta(x^v_s,s)\|_{L^2}\le\ell(s)+L_\Delta\,\mathcal E(s).
\label{eq:pf:lossform:3}
\end{equation}
Write $F(t):=\mathcal E(t)/(1-t)$ and $G(t):=\int_0^t\ell(s)(1-s)^{-1}ds$. Inserting
\eqref{eq:pf:lossform:3} into \eqref{eq:pf:lossform:2},
\begin{equation}
F(t)\;\le\;G(t)+L_\Delta\int_0^t\frac{\mathcal E(s)}{1-s}\,ds
\;=\;G(t)+L_\Delta\int_0^tF(s)\,ds .
\label{eq:pf:lossform:4}
\end{equation}

\emph{Step 4: Gr\"onwall.} $G$ is nondecreasing, so Gr\"onwall's inequality in integral
form applied to \eqref{eq:pf:lossform:4} gives $F(t)\le G(t)\,e^{L_\Delta t}$.

\emph{Step 5: the bound on $G$.} By Cauchy--Schwarz and \eqref{eq:pf:lossform:1},
\begin{equation}
G(t)\le\Big(\int_0^t\ell^2\Big)^{1/2}\Big(\int_0^t(1-s)^{-2}ds\Big)^{1/2}
\le\sqrt{\mathcal L_i(v)}\,\sqrt{t/(1-t)} .
\label{eq:pf:lossform:5}
\end{equation}
Multiplying $F(t)\le G(t)e^{L_\Delta t}$ by $(1-t)$ and using \eqref{eq:pf:lossform:5}
yields the first bound in \eqref{eq:lossform}; setting $t=1-\delta$ and bounding
$1-\delta\le1$ gives the truncated form.

\emph{Step 6: the second moment about the atom.} Since
$x^\star_{1-\delta}=\delta X_0+(1-\delta)x^i$, we have
$|x^\star_{1-\delta}-x^i|=\delta|X_0-x^i|$. The triangle inequality in $L^2(X_0)$ adds
this to $\mathcal E(1-\delta)$, which is the last display of the statement. This proves
...
\emph{Step 1: part (a), why a fixed time is not enough.} For a \emph{fixed}
$\hat t\in(0,1)$, $\ell_i(\hat t)=\ell_j(\hat t)$ solves to
\begin{equation}
z^j=z^i+\tfrac{1-\hat t}{\hat t}\,(x_0^i-x_0^j),
\label{eq:pf:nonintersect:1}
\end{equation}
which pins $z^j$ to a single \emph{point}, not to a subspace; and that point moves with
$\hat t$, so a fixed-$\hat t$ argument does not by itself control the event
``$\exists t$''. We therefore take the union over $t$ directly.

\emph{Step 2: part (a), the union is a null half-line.} Conditionally on
$(x_0^i,x_0^j,z^i)$, and since $s=(1-t)/t$ is a bijection from $(0,1)$ onto
$(0,\infty)$, \eqref{eq:pf:nonintersect:1} gives
\begin{equation}
\{\exists\,t\in(0,1):\ell_i(t)=\ell_j(t)\}=\{z^j\in R\},\qquad
R:=\big\{z^i+s\,(x_0^i-x_0^j):s\in(0,\infty)\big\}.
\label{eq:pf:nonintersect:2}
\end{equation}
$R$ is a half-line, hence contained in a one-dimensional affine subspace of $\R^d$ and
Lebesgue-null for $d\ge2$ (if $x_0^i=x_0^j$, which has probability $0$, $R$ is the single
point $z^i$, still null). As $\rho_1$ is absolutely continuous and $z^j$ is independent of
$(x_0^i,x_0^j,z^i)$, the conditional probability of \eqref{eq:pf:nonintersect:2} is $0$,
and integrating preserves it. A union bound over the finitely many pairs $(i,j)$
finishes (a).

\emph{Step 3: part (b).} With $t_i\ne t_j$ both free, the same union argument places
$z^j$ in
\begin{equation}
\Big\{\big[(1-t_i)x_0^i+t_iz^i-(1-t_j)x_0^j\big]/t_j\ :\ (t_i,t_j)\in(0,1)^2\Big\},
\label{eq:pf:nonintersect:3}
\end{equation}
the image of a two-dimensional parameter set under a map that is smooth on compact
subsets, hence a countable union of Lipschitz images of bounded planar sets. Each has
Hausdorff dimension at most $2$ and so is Lebesgue-null when $d>2$; the rest is identical
to Step~2. This proves Lemma~\ref{lem:nonintersect}.
\end{proof}
...
\emph{Step 1: no collisions.} By Lemma~\ref{lem:nonintersect}(a)--(b), union-bounded
over the $\binom N2$ pairs, almost surely no two of the $N$ full segments
$\{\ell_i(t):t\in(0,1)\}$ intersect at all. In particular the $Nm$ sampled points
$(X_t^{(i,j)},t^{(i,j)})$ never collide across different $i$.

\emph{Step 2: a lookup table.} On this full-measure event $(x,t)\mapsto i$ is well
defined on the sample, say $i(x,t)$, so we may set
\begin{equation}
v(x,t):=z^{i(x,t)}-x_0^{i(x,t)}\ \text{on the sample},\qquad v:=0\ \text{elsewhere}.
\label{eq:pf:memorize:1}
\end{equation}

\emph{Step 3: zero empirical loss.} By \eqref{eq:pf:memorize:1},
$v(X_t^{(i,j)},t^{(i,j)})=z^i-x_0^i$ exactly for every sampled $(i,j)$, so every summand
of $L_{\mathrm{MC}}^{\mathrm{unc}}$ vanishes and
$L_{\mathrm{MC}}^{\mathrm{unc}}(v)=0$. This proves Proposition~\ref{prop:memorize}.
\end{proof}
...
\emph{Step 1: the network is globally Lipschitz.} A finite composition of affine maps
(finite weight matrices, hence finite operator norm) and globally Lipschitz activations
is globally Lipschitz on $\R$, with constant $K$ equal to the product of the finitely
many layer constants.

\emph{Step 2: it is bounded on $[0,1)$.} Fixing $t_0\in[0,1)$, Step~1 gives
\begin{equation}
|f_\theta(t)|\le|f_\theta(t_0)|+K|t-t_0|\le|f_\theta(t_0)|+K\qquad\text{for all }t\in[0,1),
\label{eq:pf:norepr:1}
\end{equation}
so $\sup_{t\in[0,1)}|f_\theta(t)|<\infty$.

\emph{Step 3: conclusion.} But $g(t)=1/(1-t)\to\infty$ as $t\uparrow1$, so $g$ is
unbounded on $[0,1)$, and by \eqref{eq:pf:norepr:1} so is $f_\theta-g$. Hence
$\sup_{t\in[0,1)}|f_\theta(t)-g(t)|=\infty$, which is the remark.
\end{proof}
