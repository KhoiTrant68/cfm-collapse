...
per-example collapsed fields, and under hard conditioning with identifying labels it
degenerates to a single atom: the exact minimiser has \emph{zero} conditional variance
and zero loss (Proposition~\ref{prop:collapse}). The standard remedy---label smoothing,
...
computable from the training set (Proposition~\ref{prop:moments}). It can at best match
selected moments---the second, at a suitable bandwidth---without matching the posterior
law, since the generated law stays atomic at every bandwidth $h$, leaving an
$h$-independent Wasserstein floor (Proposition~\ref{prop:atomicity}). That obstruction
names its own cure---move the support---and smoothing the \emph{endpoint} does so
(Proposition~\ref{prop:tgtnoise}), though measuring the floor shows it small at our
default configuration and removing it buys no accuracy there. Label noise moves the
population endpoint; interpolant noise provably does not, at any noise level
(Proposition~\ref{prop:prop17}). Empirically, a trained MLP tracks the kernel reference to
within seed noise, and the same collapse appears on a Gaussian-mixture posterior, on
MNIST and CIFAR-10 inpainting, and under adversarially shuffled labels. At realistic
capacity---a $35.75$M-parameter DDPM U-Net on CIFAR-10, $d{=}3072$---hard-conditioning
collapse is unambiguous ($415\times$ in $\tr\Cov$, the generated mean identifying the
correct training image at every condition), while the smoothed prediction holds only in
direction: the across-condition log--log slope of measured on predicted variance is
$0.481$ where the theory predicts $1$, and the atomicity exact for the population
minimiser is not inherited by the trained model, $38\%$ of whose samples are off-atom at
the widest bandwidth tested.
\end{abstract}
...
\citet{scarvelis2025closedform} identify the closed-form \emph{unconditional} denoiser as
a kernel-weighted average of training points degenerating to a mixture of atoms,
\citet{chen2026scoresmoothing} analyse smoothing the empirical score as the escape, and
---nearest to us---\citet{smola2026support} read the \emph{spatial} factor
$\pi_0(\tfrac{x-tx^i}{1-t})$ of the index posterior as a Nadaraya--Watson smoother of
bandwidth $1-t$. That is the other factor of the same product \eqref{eq:kernel-field}:
our subject is the \emph{label} factor $K_h(y-y^i)$, whose bandwidth is a modelling
...
augmentation of \citet{ho2022cascaded}, and \citet{sadat2024cads} anneal exactly such
noise at inference \emph{to restore conditional diversity}.
...
subsumes the other, and Table~\ref{tab:related} states the boundary work by work. In
summary:
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\textwidth]{fig_mechanism_schematic.png}
\caption{The mechanism. Shaded: the continuous true posterior. Black: the training
atoms. Orange: where the \emph{exact population minimiser}'s flow sends its samples.
Under hard conditioning the label identifies one atom and every initial condition lands
on it, so the endpoint law is a point mass for any parameterisation
(Proposition~\ref{prop:collapse}). Smoothing the label spreads the mass over the
\emph{same} atoms with Nadaraya--Watson weights (Theorem~\ref{thm:endpoint}): the second
moment can match the posterior's exactly while the law stays a finite sum of point
masses, with a Wasserstein floor no $h$ removes (Proposition~\ref{prop:atomicity}). Only
moving the support---smoothing the endpoint rather than the label---removes that
obstruction (Proposition~\ref{prop:tgtnoise}). The middle panel is this paper's title;
Figure~\ref{fig:mechanismreal} is the same scenes computed on a real instance.}
\label{fig:mechanism}
\end{figure}
...
limit without ever leaving the training atoms (Figure~\ref{fig:mechanism}). We are plain
about which half carries the weight. The population analysis is elementary---each result
is a short Bayes computation on a Gaussian interpolant---and its value is that it supplies
an exact reference, not that it is difficult. What the reference buys is the part we
...
interventions practitioners reach for cannot help, and a protocol that catches an
agreement statistic certifying nothing. The paper is best read as a negative and
diagnostic result with an exact derivation behind it rather than as a theoretical
advance. Section~\ref{sec:scope} marks the scope line: the theory characterises the exact
...
The training set is deterministic; randomness comes only from
$I\sim\mathrm{Unif}\{1,\dots,N\}$, $X_0\sim\pi_0=\N(0,I_d)$ and
$t\sim\mathrm{Unif}(0,1)$, drawn independently, together with label noise
$\varepsilon\sim\N(0,I_k)$ when $h>0$. Set $X_1=x^I$ and, for the deterministic linear
interpolant, $X_t=(1-t)X_0+tX_1$ with $U:=\dot X_t=X_1-X_0$. The smoothed label is
...
\label{eq:objective}
\end{equation}
written $L_0$ when $h=0$ and $\widetilde Y=Y=y^I$. Let $\bar x=\frac1N\sum_i x^i$ and
...
\emph{Scope marker.} This section characterises the exact population minimiser over
$L^2$-measurable vector fields for the empirical data law, with $\D$ \emph{fixed};
Propositions~\ref{prop:expansion} and~\ref{prop:nwrate} are the deliberate exception,
treating $\D$ as an i.i.d.\ sample and asymptotic in $N$. All proofs, and three
...
The base case is a raw, unsmoothed label ($h=0$) identifying its training example
uniquely: what does the conditional expectation of Proposition~\ref{prop:projection}
collapse to once the conditioning event pins down a single index?
...
generated moments must be compared. $\tr\Covh(y)$ need not be monotone in $h$.
\end{proposition}
...
removed. Unconditional is the second knob: $h$ sets $\bar x_h$ and $\Covh$ while $\rho$
shifts the covariance, so together they match the posterior's first \emph{and} second
moments with a continuous law, where $h$ alone hits at most one.
...
covered by Proposition~\ref{prop:prop17}; since we have both exact fields in closed
form, its endpoint law can be read off. For the guided field
...
vacuous when $N>d$; integrating the exact guided field shows something stronger, that
endpoints land on the atoms themselves and larger $w$ changes \emph{which} atom is
reached (Proposition~\ref{prop:cfgatom}).
...
Two further results are kept in the appendices and \emph{not} claimed here: a loss
floor $d/(3L)$ from a uniform Lipschitz bound, measured and found some $50\times$ below
the observed plateau (Corollary~\ref{cor:floor}); and two representability results for a
\emph{randomly drawn} finite \emph{unconditional} batch (Appendix~\ref{sec:theory-d2}),
which the conditional statements above do not need.
...
monotonicity of $\tr\Covh$ in $h$; or finite-sample error bars. We never call a quantity
in that second list a theoretical prediction that ``matched'' or ``failed''; it is
reported as empirical and marked so. The compact way to say this is that \emph{the theory
predicts the destination, not the speed}---a real gap rather than a framing device,
because the phenomenon practitioners report is temporal and the question they ask is when
to stop. A characterisation of the collapse time in terms of $N$, $d$ and capacity
remains the main thing missing from this account.
...
training and 8 held-out conditions; full settings and sanity checks are in
Appendix~\ref{app:exp}. We do \emph{not} early-stop: overtraining is the object of study.
Sweeps use five seeds, and the seed controls \emph{both} the problem instance and all
training randomness, so every error bar (a population standard deviation) mixes the two;
Appendix~\ref{sec:seedsplit} separates them and finds the instance carries $78\%$.
...
descending with the loss. Three further measurements sharpen this
(Appendix~\ref{app:exp4}): the memorisation ratio of \citet{yoon2023memorization} reaches
$0.650\pm0.033$ conditionally against $0.389\pm0.032$ unconditionally from a common
$\approx0.11$ baseline, so \emph{both} flows memorise as
Corollary~\ref{cor:distinction} says and the conditional one more strongly; at held-out
conditions the variance collapses as it does at training labels while the distance to
$\mu_{\mathrm{post}}(y)$ grows from $0.12$ to $0.87$, which is nearest-atom collapse and
not generalisation (Corollary~\ref{cor:zerobw}); and permuting $Y$ against $X$ before
training leaves the collapse intact while the distance to the true posterior mean
\emph{grows}, so the law tracks the arbitrary label-to-example assignment rather than any
...
recovering the posterior. The floor \eqref{eq:w2floor} is computable and here small,
$F=0.0353$, i.e.\ $3.5\%$ of $\tr\Sigmapost$, while the \emph{population-optimal} atomic
law already scores below the trained model in MMD---so the MMD curves are not measuring
atomicity at all. And endpoint smoothing removes the obstruction without improving the
fit: the best cell of the whole $(h,\rho)$ sweep is $h{=}0.5,\rho{=}0$ in both metrics.
It helps only where the atoms are sparse enough for the floor to dominate, and the gain
grows with the floor at fixed $d$ and \emph{not} across $d$ (Table~\ref{tab:floor}), so
endpoint smoothing is defeated by dimension as kernel density estimation is. Interpolant
noise changes nothing, the generated variance being $0.367\pm0.146$ against
$0.372\pm0.140$ for the mis-specified target, exactly as \eqref{eq:invariance}
predicts.
...
($N{=}500$), over three seeds the inpainted-region pixel variance drops $\sim100\times$
by iteration $15000$ ($0.137\pm0.021\to(1.17\pm0.12)\times10^{-3}$) and the
nearest-training-image distance falls to $(0.77\pm0.26)\times10^{-3}$: completions
diverse at iteration 200 become identical by 15000 and coincide with that image
(Fig.~\ref{fig:exp3prog}). CIFAR-10 gives the same collapse, $0.142\to0.0087$.
...
standard DDPM U-Net ($35.75$M parameters) on CIFAR-10 bottom-half inpainting,
$N{=}2000$, $d{=}3072$, $k{=}1536$, for $60000$ iterations at batch $128$, evaluating
$48$ conditions at $M{=}256$ samples each (Appendix~\ref{app:exp4}).

\paragraph{Hard conditioning ($h{=}0$).} $\tr\Cov$ falls from $179.1$ at iteration
$500$ to $0.431$ at $60000$, a factor of $415$; every one of the $48$ conditions has
$n_{\mathrm{eff}}=1.00$, the generated mean identifies the correct training image at
$48/48$ conditions, and the memorisation ratio is exactly $1.000$. At this scale, on real
images, the trained model does what Proposition~\ref{prop:collapse} says the population
minimiser does.
...
Theorem~\ref{thm:endpoint} predicts to be $1$; $\beta\approx0$ licenses only the narrow
reading that tracking is absent in this protocol.

The two statistics disagree, and the disagreement is the finding. At $h{=}6$ the median
ratio is $1.15$---near-perfect agreement with Theorem~\ref{thm:endpoint}, by that
reading---while $\beta=-0.156$ says the model does not track the reference there at all.
The explanation is in the last two columns: the reference is nearly constant across
conditions ($0.74$ decades), so a ratio near $1$ is equally consistent with a model that
ignores $y$ altogether. \emph{A ratio is only as informative as the variation in its
...
orders the conditions almost as the theory says it should while compressing the range by
half. That is a partial confirmation and not a static one---the same slope measured at
each checkpoint (Figure~\ref{fig:betatraj}) is near zero for $8000$ iterations, then
rises monotonically and is still rising when the budget ends, so tracking the reference
is something the model \emph{learns} and $0.481$ is where $60000$ iterations reached
rather than a ceiling of the model class. It does \emph{not} establish that $\beta\to1$,
and three rival explanations for a fixed exponent---an additive variance floor, a hard
floor, an affine map---all lose to the power law in log space. A $35.75$M-parameter
network at $60000$ iterations is not the population minimiser: $\beta=0.481$ says how far
the optimisation has come, not that the theorem fails.
...
DDPM run ($48$ conditions, $M{=}256$). Dashed: the identity, Theorem~\ref{thm:endpoint}'s
prediction of $\beta=1$. Solid: the fitted slope, printed in each panel with the range
the regression had. All three panels share one axis range, so the collapse of the
predictor's spread---$4.57$ decades at $h{=}4$ to $0.74$ at $h{=}6$---is visible
directly, and the right-hand cloud sits on the identity line while having no slope. That is the finding: the median ratio $\tr\Cov/\tr\Covh$ is $5.30$,
$1.38$, $1.15$ at $h=4,5,6$, so by the level the agreement improves with $h$, while
$\beta=0.481\,(0.037)$, $0.349\,(0.049)$, $-0.156\,(0.058)$ says tracking disappears.
Table~\ref{tab:cifarddpm} adds the bootstrap intervals, $R^2$, $n_{\mathrm{eff}}$ and the
reference's coefficient of variation.}
\label{fig:cifartrack}
\end{figure}

\paragraph{Three supporting measurements} are reported in
Appendix~\ref{app:exp4}: atomicity, exact for the population minimiser, does not survive
training ($1.000$, $0.829$, $0.717$, $0.620$ of samples on an atom at $h=0,4,5,6$); the
discrepancy is governed by $n_{\mathrm{eff}}$ rather than $h$; and across three fresh
problem instances the aggregate ratio is stable only where the reference has range.
...
clearest instance here of the population/finite-network distinction biting, since
elsewhere the two agree in direction while here they disagree about a statement that is
either true or false. Collapse is moreover \emph{optimisation-paced}---a finite network
reaches the population minimiser only up to an optimisation gap, which we bound away from
one explanation rather than explain, Corollary~\ref{cor:floor}'s Lipschitz floor sitting
$\sim\!50\times$ below the $h{=}0$ plateau (Appendix~\ref{sec:aux}).

\paragraph{Scale, and a remedy of limited reach.} EXP-1/EXP-2 are $d\le10$ synthetic
problems and EXP-3's MNIST runs use one small U-Net; Section~\ref{sec:cifarddpm} reaches
$35.75$M parameters on CIFAR-10, where hard-conditioning collapse is unambiguous but the
$h>0$ agreement is only partial, so Theorem~\ref{thm:endpoint} is confirmed in direction
and not in magnitude there. Beyond it, and for text-conditional models whose label space
is not Euclidean with a natural kernel, persistence is a claim we do not establish;
endpoint smoothing's benefit is likewise bounded by the obstruction it removes---small in
exactly the regime we study most---and degrades with dimension.
...
over the training atoms---which neither it nor interpolant noise ever leaves. Endpoint
smoothing does, but only as far as the floor it removes is large. Four of the five
remedies practitioners reach for are settled by that account, without new experiments
...
$|\mu(y')|+\|\Sigma(y')\|\le C(1+|y'|^m)$ for some $C,m$. The stochastic term carries one
further hypothesis: the fourth conditional moment $m_4(y'):=\E[\|X\|^4\mid Y=y']$ is
bounded on $B(y,r)$ and satisfies the same polynomial growth globally. It is not implied
by the hypotheses above---they bound the conditional mean and covariance, whereas
$\Covh$ is a quadratic functional of $X$ whose \emph{variance} is what Step~4 must
control---and it is what the standard kernel-smoothing rate assumes. Then, pointwise at this $y$,
...
\]
The remainder is the mass $\tilde q_h$ puts outside $B(y,r)$, where the local hypothesis
says nothing and only the global growth bound on $\Sigma$ applies; it is the same
Gaussian-tail estimate as in Step~3 below, hence $o(h^p)$ for every $p$ and absorbed into
the $O(h^4)$ of \eqref{eq:expansion}. Constancy of $\Sigma$ on the whole of $\R^k$ would
make the step exact, but is not needed. This is the within-group term of
\eqref{eq:expansion}.
...
squared mean $\Theta(1)$), and likewise for $K_h(y-Y)X$. The quadratic functional
$\Covh$ needs the same computation for $K_h(y-Y)XX^\top$, whose second moment is
$\Theta(h^{-k})$ only because $m_4$ is bounded near $y$: this is the one place the
fourth-moment hypothesis of the statement is used, and without it the $O_P$ rate is not
available even though the deterministic expansion is unaffected. Hence
