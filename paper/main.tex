...
\item \emph{(what a surviving deviation requires)} for any $\Delta$ and any $L\ge0$,
  \[
  \lim_{t\uparrow1}|e_t|=L
  \iff
  \lim_{t\uparrow1}\,(1-t)\Big|\int_0^t\frac{\Delta(x^v_s,s)}{1-s}\,ds\Big|=L ,
  \]
  immediately from \eqref{eq:survival-id}. So a nonzero retained deviation
  \emph{requires} the weighted error integral $\int_0^t\Delta/(1-s)\,ds$ to diverge at
  exactly the rate $(1-t)^{-1}$; if it converges, or diverges more slowly, nothing
...
in the precise averaged sense the identity \eqref{eq:survival-id} supports---the
weighted error integral must diverge at exactly rate $(1-t)^{-1}$, and part~(c)
exhibits errors that do. We are deliberate about the form of that converse. It is a
statement about $\int_0^t\Delta/(1-s)\,ds$, not about $\Delta$ pointwise: an error can
oscillate, or be large only on a thin set of times, and what survives depends on the
integral alone. Read that way, the observed plateau is the size of the weighted
...
\]
the $o(h^2)$ being the Peano remainder, which is all $C^2$ gives and all this remark
needs,
...
$c_2(Nh^k)^{-1}$ and needs $c_1,c_2>0$, whereas $c_1$ is built from
$\nabla^2\mu(y)$ and the score of $\rho_Y$ and vanishes identically when $\mu$ is
affine---which is exactly the linear-Gaussian instance we test, where the bias is $0$
at every $h$ and the MSE is monotone in $h$ rather than U-shaped. The rate is the
generic one; the case we measure is the degenerate one, and we say so rather than
quoting a rate our own setting does not exhibit. Queries at $y=y^i$ are \emph{design
...
the same rate we measure rather than assume
(\texttt{scripts/verify\_atom\_floor\_scaling.py}, standard Gaussian target, atoms
i.i.d.\ from the same law). For $d\ge3$ they do: fitting $\log F_N$ on $\log N$ over
$N\in[200,3200]$ gives slopes $-0.612$, $-0.379$ and $-0.249$ at $d=3,5,8$ against the
predicted $-2/d=-0.667$, $-0.400$, $-0.250$. At $d{=}2$ they do \emph{not}: the fitted
slope is $-0.846$, and the reason is a logarithmic correction that the optimal
quantiser does not pay---$F_N\,N/\log N$ is flat to within $1.05\times$ across that
