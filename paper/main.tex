...
All proofs, and three conventions they rely on, are in Appendix~\ref{app:proofs}. The
...
appendix), a single atom exactly when the label is an identifier. The unconditional
...
memorisation \citep{gu2023memorization}. Proposition~\ref{prop:collapse} explains both:
informativeness is irrelevant, and what matters is that the label identifies the example
(Appendix~\ref{app:adv} tests this directly).
...
Proposition~\ref{prop:nearduplicate} shows that a usable bandwidth exists exactly when
...
needs $h\to0$ and $N\to\infty$ together (Proposition~\ref{prop:nwrate}), not $h$ alone.
Whether the floor matters depends on its size, governed by the rate factor $N^{-1/d}$:
$0.071$ at our $(d,N)=(2,200)$ default but $0.9975$ at $(3072,2000)$. Zador's theorem
\citep[Ch.~6]{graf2000quantization} gives that rate for the optimal $N$-point quantiser
and so bounds the floor from below. Whether i.i.d.\ atoms attain it is measured in
...
label smoothing is more than a reweighting. Under a pointwise homoscedastic regularity
hypothesis, Proposition~\ref{prop:expansion} (appendix) gives
...
the noise at the endpoint. For every schedule with $\gamma(1)=0$, interpolant noise cannot change the endpoint law
(Remark~\ref{rem:notsampler} and Figure~\ref{fig:interpolant}, appendix).
...
atom, and larger $w$ changes which atom is reached (Proposition~\ref{prop:cfgatom}).
...
$\sigma_{\mathrm{obs}}$ (the theory implies no monotonicity), whether $\tr\Covh$ is
monotone in $h$, or what the finite-sample error bars are. Results in that second list
...
not generalisation (Corollary~\ref{cor:zerobw}). And permuting $Y$ against $X$ leaves the
...
\begin{proof}[Proof of Corollary~\ref{cor:prop18}]
    ...
    \end{align*}
    It vanishes iff $u\parallel w$, i.e.\ $\dot\gamma(1-t)=-\gamma$. With $\gamma=\sigma\sqrt{t(1-t)}$ this reads $1-2t=-2t$, which is impossible; hence $\inf_vL_\sigma>0$. Finally
    \begin{align*}
...
\begin{proof}[Proof of Proposition~\ref{prop:expansion}]
    ...
    \begin{align*}
        ...
        \Var\big(K_h(y-Y)XX^\top\big) &= \Theta(h^{-k}),
    \end{align*}
    the latter using $m_4$ bounded near $y$ (the only use of (iv)). Hence $S_N,T_N=\E[\cdot]+O_P(\sqrt{h^{-k}/N})$, with $\E[K_h(y-Y)]\to\rho_Y(y)>0$, and by Slutsky
    \begin{align*}
...
\begin{proof}[Proof of Proposition~\ref{prop:nwrate}]
    ...
    Design points: at $y=y^i$ the weight of $j=i$ is
    \begin{align*}
        \frac{K_h(0)}{\sum_jK_h(y^i-y^j)} &= \Theta\big((Nh^k)^{-1}\big) \\
        &\to 0 .
    \end{align*}
    If $Nh^k\not\to\infty$ it does not vanish (Corollary~\ref{cor:zerobw}: it tends to $1$).
\end{proof}
...
moderate constant; the constant is loose by a factor of $25$ and we do not use it. One limit remains: the bound
