...
Table~\ref{tab:p7kernel})---agreement with the \emph{endpoint-law reference} in this
setting, with the field agreeing too. That $h\approx0.1$ ``perfectly
...
at iteration 200 are identical by 15000 (Fig.~\ref{fig:exp3prog}). A single small-U-Net
CIFAR-10 trajectory shows the same qualitative decrease, $0.142\to0.0087$; we treat it
as corroboration rather than a replication estimate.
...
that the mechanism is an artefact of small capacity. We train a standard DDPM U-Net
...
$n_{\mathrm{eff}}=1.00$. Thus hard-conditioning collapse is robust at this scale, while
the experiment does not separate instance variation from optimisation variation.

\paragraph{Label smoothing ($h>0$): a deliberately partial test.}
Table~\ref{tab:cifarddpm} reports both the aggregate ratio $\tr\Cov/\tr\Covh$ and the
more stringent OLS slope $\beta$ of $\log\tr\Cov$ on $\log\tr\Covh$ \emph{across
conditions}; the population endpoint law predicts $\beta=1$. This tests whether the
finite model preserves the conditional variation in the reference, not merely its level.
...
The two statistics disagree, and the disagreement is informative. At $h{=}6$ the median
ratio is $1.15$, while $\beta=-0.156$: the reference spans only $0.74$ decades across
conditions, so level agreement is compatible with ignoring conditional variation. Where
the reference has range ($4.57$ decades at $h{=}4$), $\beta=0.481\pm0.037$ with
$R^2=0.785$. The model preserves the predicted ordering but compresses its dynamic range,
recovering roughly half of the population exponent. The checkpoint trajectory rises over
the available budget, but it does \emph{not} establish $\beta\to1$. We therefore report
the DDPM smoothing result as partial, and use the exact endpoint law as an audit target
rather than as a fitted-model claim.
...
those runs confound instance and optimisation randomness; the $h>0$ agreement is partial,
not a quantitative confirmation of Theorem~\ref{thm:endpoint}. Beyond this scale, and
for text-conditional label spaces carrying no natural kernel, persistence is a claim we
do not establish. Endpoint smoothing's benefit is likewise bounded by the obstruction it
removes, small in exactly the regime we study most.
