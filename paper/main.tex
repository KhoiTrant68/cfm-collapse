...
overtraining, but the population mechanism and the effect of label smoothing are
unclear. We analyse the exact $L^2$ population minimiser of the conditional
flow-matching objective with a fixed empirical dataset and a deterministic linear
interpolant. If a label identifies one training example, the minimiser transports every
...
\eqref{eq:kernel-field}. Our contribution is narrower and endpoint-level: we make the
\emph{label} factor $K_h(y-y^i)$ explicit, its bandwidth is a modelling choice rather
than a consequence of the clock, and we derive the conditional endpoint law, moment
reference, and their audit consequences. Thus we do not claim a new generic kernel
identity; we identify what the conditional factor changes and what it cannot change.
