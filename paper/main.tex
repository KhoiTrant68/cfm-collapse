...
$n_{\mathrm{eff}}$ counts the cluster and not the dataset: near-duplicate labels buy
...
population endpoint, at any noise level. Replace the interpolant by $X_t=(1-t)X_0+tX_1+\gamma(t)Z$ with
...
defeated by dimension. Interpolant noise changes nothing,
...
\begin{proof}[Proof of Proposition~\ref{prop:cfg}]
    ...
    \end{align*}
    so $\dist(x_t,\mathcal A)\to0$ and, $\mathcal A$ being closed, the endpoint law is supported in $\mathcal A$. Well-posedness: $|m_w|\le(1+2w)\max_i|x^i|$, so \eqref{eq:vgrowth} holds with $M\to(1+2w)M$.
\end{proof}
