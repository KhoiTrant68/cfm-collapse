...
empirical dataset, deterministic linear interpolant, all square-integrable fields) and
use it to audit label smoothing. If a label identifies one training example, the minimiser transports every
source draw to that atom, giving zero conditional variance and zero loss. Adding
Gaussian noise to the label instead makes the endpoint law an exact
Nadaraya--Watson mixture over training atoms. Its moments give a parameter-free
reference for auditing a trained model, but its support remains atomic: matching a
conditional variance need not recover the posterior. This also separates
interventions: label noise changes the mixture weights, interpolant noise leaves the
endpoint law unchanged, and endpoint noise moves the support. On five-seed synthetic
experiments, an MLP follows the kernel endpoint reference; collapse also appears for a
Gaussian-mixture posterior, under shuffled labels, and in MNIST inpainting. On
CIFAR-10 with a $35.75$M-parameter DDPM U-Net, hard-conditioning variance falls by
$415\times$ and all evaluated conditions select one effective atom. Label smoothing at
that scale is only a partial match: where the reference has useful dynamic range, the
across-condition log--log slope is $0.481$ rather than the population value $1$, and
$38\%$ of samples are off-atom at the widest bandwidth. Thus the population result is
an exact audit target, not a claim that a finite network has already attained it.
\end{abstract}
...
Two established lines bracket this analysis. That the empirical minimiser memorises is well
established
...
bandwidth $1-t$---the spatial factor of the same posterior that we factor in
\eqref{eq:kernel-field}. Our contribution is endpoint-level: the \emph{label} factor $K_h(y-y^i)$, whose
bandwidth is a modelling choice rather than a consequence of the clock, survives to
$t=1$ and fixes the exact conditional endpoint law, its moment reference, an obstruction
that no $h$ removes, and a separation of label, interpolant, guidance and endpoint
noise. We do not claim a new generic kernel identity.
On the other side, Gaussian noise on a conditioning
...
The endpoint view turns conditioning noise from a heuristic into an exact
Nadaraya--Watson mixture over the training atoms (Theorem~\ref{thm:endpoint}). It yields
a reference curve computable from the training set alone
(Proposition~\ref{prop:moments}) and a limitation that moment-level evaluation misses:
the law remains supported on training atoms at every bandwidth, so it can match selected
moments without recovering the posterior (Proposition~\ref{prop:atomicity}). Closest in spirit is
...
subsumes the other. Table~\ref{tab:relatedshort} states the scope boundary; the full
version, with four further lines, is Table~\ref{tab:related}. In summary:
...
\item \textbf{Conditional endpoint under label smoothing.} Conditioning noise is an
...
\item \textbf{What reaches a finite model.} The collapsed field is self-enforcing
...
kernel-weighted empirical conditional flow}, and label smoothing moves along that limit
without leaving the training atoms (Figure~\ref{fig:mechanism}). The payoff is an exact
conditional reference law: it distinguishes recovering a posterior moment from
recovering a posterior and catches agreement statistics whose denominator has no useful
variation. Section~\ref{sec:scope} marks the boundary throughout: the theory
characterises an empirical population minimiser, not the finite network that SGD reaches.
...
generated moments must be compared. $\tr\Covh(y)$ need not be monotone in $h$. \proofin{apx:pf-moments}
\end{proposition}
...
\label{eq:w2floor}
\end{equation}
\emph{and this lower bound does not depend on $h$.} \proofin{apx:pf-atomicity}
\end{proposition}
...
linear part $-\tfrac1{1-t}I_d$ contracts at a diverging rate, so the constant
