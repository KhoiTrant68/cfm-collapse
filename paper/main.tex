...
% --- shorthands: paper/math_commands.tex ------------------------------------
\input{math_commands}

\title{Conditional Flow Matching:\\
Kernel Smoothing over Atoms\\
Does Not Restore the Posterior}
...
\begin{abstract}
Finite-data conditional flow matching can become nearly deterministic after
overtraining. We derive the exact endpoint law of its population minimiser (fixed
empirical dataset, linear interpolant, all square-integrable fields) and use it to audit
label smoothing. When a label identifies one training example, the minimiser sends every
source draw to that example, so the conditional variance and the loss are zero. Adding
Gaussian noise to the label turns the endpoint law into a Nadaraya--Watson mixture over
the training atoms. Its moments give a reference curve for a trained model, but its
support stays atomic, so matching a conditional variance does not recover the posterior.
The same law separates three interventions: label noise reweights the mixture,
interpolant noise leaves the endpoint law unchanged, and endpoint noise moves the
support. In five-seed synthetic experiments an MLP follows the reference; collapse also
appears with a Gaussian-mixture posterior, with shuffled labels, and in MNIST inpainting.
On CIFAR-10 with a $35.75$M-parameter DDPM U-Net, hard conditioning reduces the
conditional variance $415$-fold and every evaluated condition selects one effective
atom. Label smoothing matches the reference only partly at that scale: where the
reference has useful range, the across-condition log--log slope is $0.481$ instead of
$1$, and $38\%$ of samples are off-atom at the widest bandwidth. The population result is
an audit target, not a claim about what a finite network reaches.
\end{abstract}
...
are usually trained by regressing a velocity field on a linear path between a noise
sample and a data sample \citep{tong2024improving}. On a finite dataset, minimising
against the \emph{empirical} data law is known to induce memorisation
\citep{carlini2023extracting,somepalli2023diffusion}, and recent work reports that
conditional flow-matching models trained past the point of good posterior calibration
give increasingly deterministic samples that coincide with training examples
\citep{physicscfm2026,gradvar2025}. We give an exact population-level account of this.
With the training set $\D=\{(x^i,y^i)\}_{i=1}^N$ fixed and only the interpolation random,
we characterise the population minimiser of the flow-matching objective over all
square-integrable vector fields, and the endpoint law of its flow.

Two lines of work are close to this. That the empirical minimiser memorises is well
established
\citep{pidstrigach2022manifolds,gu2023memorization,kadkhodaie2024generalization,biroli2023diffusion,kamb2024creativity,buchanan2025edge,gao2024flow,bertrand2025closed},
and reading a kernel out of a finite-support flow is not new either
\citep{scarvelis2025closedform,chen2026scoresmoothing}. \citet{smola2026support} read the
\emph{spatial} factor $\pi_0(\tfrac{x-tx^i}{1-t})$ of the index posterior as a
Nadaraya--Watson smoother of bandwidth $1-t$, in the same posterior that we factor in
\eqref{eq:kernel-field}. We work with the other factor, $K_h(y-y^i)$. Its bandwidth is a
modelling choice and not a consequence of the clock, and it survives to $t=1$, where it
fixes the conditional endpoint law. We do not claim a new generic kernel identity.
Gaussian noise on a conditioning signal is standard practice: it is the conditioning
augmentation of \citet{ho2022cascaded}, which \citet{sadat2024cads} anneal at inference
to restore conditional diversity. \citet{pfrommer2025conditional} find the same smoothness in the conditioning variable as
an inductive bias of trained models; here it is imposed and its endpoint law is exact.

The endpoint law turns conditioning noise from a heuristic into a Nadaraya--Watson
mixture over the training atoms (Theorem~\ref{thm:endpoint}). It gives a reference curve
computable from the training set alone (Proposition~\ref{prop:moments}), and it shows
what moment-level evaluation misses: the law stays supported on the training atoms at
every bandwidth, so it can match selected moments without recovering the posterior
(Proposition~\ref{prop:atomicity}). Closest in spirit is \citet{buchanan2025edge}, whose
capacity-driven transition in \emph{unconditional} diffusion is the capacity analogue of
our $h\colon0\to\infty$ interpolation; neither subsumes the other.
Table~\ref{tab:relatedshort} gives the scope boundary and Table~\ref{tab:related} adds
four further lines. Our contributions are:
...
\bottomrule
\end{tabular}
\caption{Scope boundary against the three nearest lines. Reading a kernel from a
finite-support flow is not new. This paper studies the conditional label factor and
uses its endpoint law to audit variance recovery. Table~\ref{tab:related} adds four
lines.}
\label{tab:relatedshort}
\end{table}

\begin{enumerate}\itemsep1pt
\item \textbf{Hard conditioning.} A label that identifies its example turns memorisation
  of the whole empirical measure into memorisation of a single atom, with zero
  conditional variance (Corollary~\ref{cor:distinction}). The contrast with the
  unconditional model is which empirical measure is memorised, not whether memorisation
  happens. It is also not overfitting of a finite batch: $y^i$ identifies the atom
  deterministically, so the collapse is already present in the population minimiser.
\item \textbf{Label smoothing.} Conditioning noise is an existing technique
  \citep{ho2022cascaded,sadat2024cads}. We show that it makes the population endpoint a
  Nadaraya--Watson mixture \citep{nadaraya1964estimating,watson1964smooth} over the
  training atoms (Theorem~\ref{thm:endpoint}), which gives a parameter-free reference
  curve $\Covh(y)$ for auditing a trained model (Proposition~\ref{prop:moments}).
\item \textbf{Restored variance is not posterior recovery.} The smoothed law is
  supported on the training atoms at every bandwidth, so conditioning noise can match
  the posterior's second moment while remaining a finite sum of point masses, with a
  Wasserstein floor independent of $h$ (Proposition~\ref{prop:atomicity}). A report of
  ``variance restored'' says nothing about posterior recovery. The statistic can also be
  uninformative on its own terms: at large $h$ the reference $\tr\Covh$ is almost
  constant across conditions, so agreement with it carries little information. We
  therefore report the across-condition slope $\beta$ of $\log\tr\Cov$ on
  $\log\tr\Covh$, for which theory predicts $\beta=1$. On CIFAR-10 at $35.75$M
  parameters the two disagree: aggregate agreement is $1.15$ while $\beta=-0.156$.
  Neither is a certificate alone: $\tr\Cov=10\,\tr\Covh$ has slope $1$ and is off by an
  order of magnitude.
\item \textbf{Two remedies.} Label noise changes the population endpoint; interpolant
  noise does not, at any noise level (Propositions~\ref{prop:prop17}
  and~\ref{prop:factors}). The obstruction in item~3 says what a remedy must do, which
  is to move the support, and endpoint smoothing does so (Proposition~\ref{prop:tgtnoise})
  at the cost of $+\rho^2I_d$ in variance.
\item \textbf{What reaches a finite model.} The collapsed field contracts errors
  (Proposition~\ref{prop:survival}): a deviation survives to the endpoint only if the
  velocity error accumulates at rate $(1-t)^{-1}$, so a model retains conditional
  variance only through an error of that scale, which is bounded by the loss not yet
  removed (Corollary~\ref{cor:lossform}). Representability is not the obstacle;
  accumulated error is. This makes $\beta$ a measure of how far optimisation has come and
  not a verdict on the theorem.
\end{enumerate}

\begin{figure}[t]
\centering
\includegraphics[width=0.88\textwidth]{fig_flow_portrait.png}
\caption{The mechanism in trajectory space, on one condition of the EXP-1 instance
($d{=}2$, $N{=}200$): $160$ draws of $x_0$, their paths and endpoints. (a,~b) One
\emph{trained} network at two checkpoints. At $1000$ iterations it is a posterior
sampler; at $200000$, $96\%$ of its paths reach the atom identified by the label
(Proposition~\ref{prop:collapse}). (c) The population optimum at the $h^\star$ with
$\tr\Covh=\tr\Sigmapost$; marker area is the weight $p_i^{(h)}$ of
Theorem~\ref{thm:endpoint}. The variance is right but the law is still atomic, with a
floor no $h$ removes (Proposition~\ref{prop:atomicity}). (d) Only moving the support
leaves the atoms (Proposition~\ref{prop:tgtnoise}). Figures~\ref{fig:traintraj}
and~\ref{fig:interventions} give the checkpoints in between and the interventions that
do not help.}
\label{fig:mechanism}
\end{figure}

In one line, conditional collapse is the zero-bandwidth limit of a kernel-weighted
empirical conditional flow, and label smoothing moves along that limit without leaving
the training atoms (Figure~\ref{fig:mechanism}). This gives an exact conditional
reference law. It separates recovering a posterior moment from recovering a posterior,
and it exposes agreement statistics whose denominator barely varies.
Section~\ref{sec:scope} marks the boundary: the theory describes an empirical population
minimiser, not the network that SGD reaches.
...
Everything in this section holds under Assumption~\ref{as:standing} and concerns the
population minimiser over $L^2$-measurable vector fields for the empirical data law, with
$\D$ fixed. Propositions~\ref{prop:expansion} and~\ref{prop:nwrate} are the exception:
they relax (i) to a random design under Assumption~\ref{as:design} and let $N\to\infty$.
All proofs, and three conventions they rely on, are in Appendix~\ref{app:deferred}. The
projection identity below is the standard $L^2$ decomposition behind flow-matching
objectives \citep{lipman2023flow,albergo2023building,liu2023rectified}.
...
$\inf_v L_h=\E[\Var(U\mid X_t,t,\widetilde Y)]$. \proofin{apx:pf-projection}
\end{proposition}

Lemmas~\ref{lem:cov} and~\ref{lem:wellposed} (appendix) justify the conditional
expectations and show the flows are well posed with no further assumption: for $t<1$ the
map $X_0\mapsto X_t$ is an invertible affine change of variables with positive conditional
density, and every field below is locally Lipschitz on $\R^d\times[0,1)$, so the flows are well
posed there. Each endpoint law $p_1$ is the weak limit of the marginals at fixed $y$,
shown to exist case by case (appendix, ``Three conventions'').

\subsection{Hard conditioning: exact collapse}
\label{sec:theory-hard}

If the conditioning event pins down a single index, the conditional expectation of
Proposition~\ref{prop:projection} collapses:

\begin{proposition}[Exact conditional collapse; $h=0$]
\label{prop:collapse}
Let $h=0$. Then
(a) $v_0^\star(x,t,y^i)=(x^i-x)/(1-t)$ for $t<1$ $(\star)$;
(b) $\inf_v L_0(v)=0$;
...
$\Cov[p_1^{\mathrm{cond}}(\cdot\mid y^i)]=0$. \proofin{apx:pf-collapse}
\end{proposition}
If labels are not distinct, collapse is onto the label's empirical conditional
support $|\mathcal I_y|^{-1}\sum_{i\in\mathcal I_y}\delta_{x^i}$ (Corollary~\ref{cor:repeated},
appendix), a single atom when the label is an identifier ($|\mathcal I_y|=1$). The unconditional
minimiser, by contrast, mixes all $N$ atoms and reaches $\rhohat_X$
(Proposition~\ref{prop:uncond}, appendix).
...
$p_1^{\mathrm{unc}}=\frac1N\sum_i\delta_{x^i}$. The difference is between
single-example and full-empirical-measure memorisation, not between memorising and not,
and it shows in the conditional second moment:
\begin{equation}
\Cov[p_1^{\mathrm{cond}}(\cdot\mid y^i)]=0
\qquad\text{vs.}\qquad
\Cov[p_1^{\mathrm{unc}}]=\Xhat.
\label{eq:distinction}
\end{equation} \proofin{apx:pf-distinction}
\end{corollary}

The often-cited observation $\tr\Cov\approx\tr\Xhat$ for the unconditional baseline is
\eqref{eq:distinction}: it is evidence of full-empirical-measure memorisation, not of its
absence. Two empirical findings read the same way. Verbatim replication is common with
text conditioning, rare without it, and reduced by randomising captions
\citep{somepalli2023copying}; and uninformative random labels sharply increase
memorisation \citep{gu2023memorization}. Proposition~\ref{prop:collapse} accounts for the
label's part in both: informativeness is irrelevant, and what matters is that the label
identifies the example (Appendix~\ref{app:adv} tests this directly).

\subsection{Label smoothing is kernel regression}
...
the velocity field of a Nadaraya--Watson regression over the atoms.
...
\end{equation} \proofin{apx:pf-kernel-field}
\end{proposition}
...
\label{eq:endpoint}
\end{equation} \proofin{apx:pf-endpoint}
\end{theorem}
\emph{Proof sketch.} By Lemma~\ref{lem:mixture-coupling}, $\vstar$ is the conditional velocity of the
coupling $I\sim p^{(h)}(y)$, $X_1=x^I$. Its marginals solve the flow's continuity equation, so they
coincide with the flow's (Lemma~\ref{lem:wellposed}), and they tend to $\mathrm{Law}(x^I)$ since
$X_t\to x^I$ almost surely.
...
recovers Proposition~\ref{prop:collapse}. \proofin{apx:pf-zerobw}
\end{corollary}
\begin{corollary}[Infinite bandwidth at fixed $N$]
\label{cor:infbw}
$p_i^{(h)}(y)\to1/N$ for every $i$ as $h\to\infty$, so
$p_h^{\mathrm{gen}}(\cdot\mid y)\Rightarrow p_1^{\mathrm{unc}}$. \proofin{apx:pf-infbw}
\end{corollary}
...
$\Covh(y)=\sum_i p_i^{(h)}(y)\,(x^i-\bar x_h(y))(x^i-\bar x_h(y))^\top$, both computable
from the training set for any $h$; they are the reference for generated moments.
$\tr\Covh(y)$ need not be monotone in $h$. \proofin{apx:pf-moments}
\end{proposition}

These are the moments of the law in Theorem~\ref{thm:endpoint}, and every experiment below
is measured against them. For labels that are close but not equal,
Proposition~\ref{prop:nearduplicate} shows that a usable bandwidth exists when
clusters are tight relative to their separation, and that inside that window
$n_{\mathrm{eff}}=1/\sum_i(p_i^{(h)})^2$ counts the cluster and not the dataset: near-duplicate labels buy
effective sample size, not continuity.

\begin{proposition}[Label smoothing never leaves the training set]
\label{prop:atomicity}
For every $h>0$, and for $h=0$ at $y=y^i$, $\operatorname{supp}(p_h^{\mathrm{gen}}(\cdot\mid y))
\subseteq\{x^1,\dots,x^N\}$. Consequently, if the true posterior $p(\cdot\mid y)$ puts no
mass on $\{x^i\}$,
\begin{equation}
...
\label{eq:w2floor}
\end{equation}
and this lower bound does not depend on $h$. \proofin{apx:pf-atomicity}
\end{proposition}
\emph{Proof sketch.} Any coupling of an atomic law with $p(\cdot\mid y)$ moves each posterior draw at
least to its nearest atom, and that bound involves the atoms alone, not $h$.
...
samples, so matching $\tr\Covh$ matches a second moment and not the posterior. A remedy
needs $h\to0$ and $N\to\infty$ together (Propositions~\ref{prop:expansion}
and~\ref{prop:nwrate}, for the first two moments), not $h$ alone.
Whether the floor matters depends on its size, governed by the rate factor $N^{-1/d}$ of $W_2$:
$0.071$ at our $(d,N)=(2,200)$ default but $0.9975$ at $(3072,2000)$. Zador's theorem
\citep[Ch.~6]{graf2000quantization} gives that rate, asymptotically in $N$, for the optimal $N$-point quantiser
and so points to a lower bound on the floor. Whether i.i.d.\ atoms attain it is measured in
Appendix~\ref{app:deferred}: the obstruction is negligible where it is easy to measure
and stays large relative to the posterior scale at image dimensions.
...
Section~\ref{sec:theory-noise}, since $\gamma(1)=0$. A remedy must move the support. Of
the three places to inject noise (label, path, endpoint), only the endpoint does.

\begin{proposition}[Endpoint smoothing]
\label{prop:tgtnoise}
Replace the endpoint by $X_1=x^I+\rho\,\xi$ with $\xi\sim\N(0,I_d)$ independent of
$(I,X_0,\varepsilon)$ and redrawn every step, keeping label bandwidth $h>0$. Write
...
$1/(1-t)$ blow-up of $(\star)$ disappears. \proofin{apx:pf-tgtnoise}
\end{proposition}

Two caveats, both quantified in Section~\ref{sec:exp1}. Removing a lower bound does not
reduce the distance: \eqref{eq:tgtendpoint} is a kernel density estimate that pays
$+\rho^2I_d$ of variance for its continuity, and the gain is bounded by the obstruction
removed. And $h$ sets $\bar x_h$ and $\Covh$ while $\rho$ shifts the covariance, so
together they can match the posterior's first and second moments with a continuous law,
where $h$ alone matches at most one.

One place in this paper leaves the fixed-$\D$ setting. It asks what $\Covh$ converges to
when the training set is an i.i.d.\ sample and $N\to\infty$, the only regime in which
label smoothing is more than a reweighting. Under local homoscedasticity ($\Sigma$ constant near $y$), $h\to0$ and $Nh^{k+4}\to\infty$,
Proposition~\ref{prop:expansion} (appendix) gives
$\Covh(y)=\Sigma(y)+h^2J(y)J(y)^\top+O(h^4)+O_P((Nh^k)^{-1/2})$. The between-group
...
$\bar x_h(y)$ and records that queries at $y=y^i$ are design points, so the $h\to0$ limit
there differs from the fixed-$N$ collapse of Corollary~\ref{cor:zerobw}: same symbol,
opposite conclusions.

\subsection{Stochastic interpolant noise does not help}
\label{sec:theory-noise}

Another place to inject randomness is the interpolant path itself. This never changes the
population endpoint at $h=0$, at any noise level. Replace the interpolant by $X_t=(1-t)X_0+tX_1+\gamma(t)Z$ with
$Z\sim\N(0,I_d)$ independent and $\gamma(t)=\sigma\sqrt{t(1-t)}$, so
$\gamma(0)=\gamma(1)=0$. A single $Z$ defines the whole trajectory, and the pathwise-derivative target is
\begin{equation}
\dot X_t=X_1-X_0+\dot\gamma(t)Z,\qquad
\dot\gamma(t)=\frac{\sigma(1-2t)}{2\sqrt{t(1-t)}};
\label{eq:c2target}
\end{equation}
regressing on $X_1-X_0$ alone would be a different objective. Write
...
\eqref{eq:c2target}. Then for $t\in(0,1)$:
...
\label{eq:invariance}
\end{equation} \proofin{apx:pf-prop17}
\end{proposition}

Part~(b) gives the flow in closed form and (c) follows because $s_1=0$, so no
integrator and no early stopping enter the argument: the endpoint is fixed by the
interpolant itself. The reason is structural. Any admissible schedule has $\gamma(1)=0$,
since otherwise the path never reaches the data, and that is the condition that removes
the noise at the endpoint. For $\gamma=\sigma\sqrt{t(1-t)}$ interpolant noise cannot change the endpoint law, and
any schedule with $\gamma(1)=0$ has $X_t\to x^I$ (Remark~\ref{rem:notsampler} and
Figure~\ref{fig:interpolant}, appendix).
Corollary~\ref{cor:prop18} records what it does change: it raises the irreducible error
and halves the Lipschitz blow-up rate, and neither touches $p_1$.
Proposition~\ref{prop:factors} locates the difference: the two noises multiply different
factors of the same index posterior, and only the label factor survives to $t=1$. This
separates the mechanism from the unconditional ``resample-$x_0$'' remedy of
\citet{gradvar2025}, which works through attainability and not through the population
optimum.

\subsection{Classifier-free guidance does not escape either}
\label{sec:theory-cfg}

Guidance is the remedy a practitioner is most likely to try. It is not covered by
Proposition~\ref{prop:prop17}, but with both exact fields in closed form its endpoint law
...
$m_w$ an affine combination of the atoms, and that the trajectory's component
orthogonal to $\mathcal A:=\operatorname{aff}\{x^1,\dots,x^N\}$ obeys $q_t=(1-t)q_0$.
Any guided endpoint law is therefore supported in $\mathcal A$, for every $w$ and every
initial law, so when $N\le d$ that support is Lebesgue-null and no guidance weight makes
the conditional law absolutely continuous. That is a statement about dimension and is
empty when $N>d$. If the guided trajectory converges and the nearest atom is unique, the endpoint is that
atom (Proposition~\ref{prop:cfgatom}); larger $w$ is observed to change which one.

The four interventions thus fall into an order: inert (interpolant noise, at every noise
level), atom-preserving (guidance), reweighting (label smoothing, so the atomicity ceiling
still binds), and support-moving (endpoint smoothing, at $+\rho^2I_d$ of variance). This
ordering is the practical content of the theory.
...
close to it. Appendix~\ref{app:survivaltheory} gives a contraction estimate. Write
$\Delta$ for a model's velocity error against $(\star)$ and $e_t$ for its trajectory's
deviation from the exact one. Proposition~\ref{prop:survival} is the identity
$e_t=(1-t)I(t)$ with $I(t):=\int_0^t\Delta(x^v_s,s)(1-s)^{-1}ds$, so
$\lim_{t\uparrow1}e_t=e_\star$ iff $I(t)=e_\star/(1-t)+o((1-t)^{-1})$. The linear part
$-\tfrac1{1-t}I_d$ of the collapsed field contracts at a diverging rate, so the constant
error $c(1-t)^{-a}u$ leaves a deviation that vanishes for $a<1$, equals $c$ at $a=1$ and is
unbounded for $a>1$ (the bound is attained, so this is realised, with infinite loss for $a\ge\frac12$); for
$a>1$ the bound alone constrains nothing.
A trajectory can therefore retain a deviation only through an endpoint-scale accumulated
error. This connects to Remark~\ref{lem:norepr}, which observes that no finite
network with globally Lipschitz activations represents $1/(1-t)$ pointwise. The converse
constrains $I$ and not $\Delta$ pointwise, and we claim no pointwise version. A Lipschitz
hypothesis on the error converts the estimate into the training loss:
Corollary~\ref{cor:lossform} bounds the root-mean-square deviation at truncation
$1-\delta$ by $e^{L_\Delta}\sqrt{\mathcal L_i(v)\,\delta}$. The hypothesis is demanding:
$\nabla_xv^\star=-I_d/(1-t)$, so a model with $\operatorname{Lip}_xv\le K$ has
$L_\Delta\ge\delta^{-1}-K$, and the bound is informative only for fields that reproduce
the linear part of $(\star)$. On EXP-1's checkpoints the empirical slope is $0.485$ at
$R^2=0.998$, against $\tfrac12$
(Appendix~\ref{app:survival}; the fit implies a prefactor $e^{L_\Delta}\approx25$, which we do not use).

\subsection{Finite capacity, finite samples, and scope}
\label{sec:scope}

A loss floor $d/(3L)$ from a uniform Lipschitz bound, about $50\times$ below the observed
plateau (Corollary~\ref{cor:floor}), and two representability results for an unconditional
batch (Appendix~\ref{sec:theory-d2}) stay in the appendices and are not claimed here.

The population theory proves consequences for the empirical minimiser
(Propositions~\ref{prop:collapse}--\ref{prop:tgtnoise},
Corollaries~\ref{cor:zerobw}--\ref{cor:infbw}). It does not determine the number of SGD
steps needed to approach the optimum. Nor does it settle how observed collapse depends on
$N$ at fixed architecture (the population optimum collapses for every finite $N$, and a
non-collapsing $N{=}5000$ run probes representation and optimisation), how it depends on
$\sigma_{\mathrm{obs}}$ (the theory implies no monotonicity), how $\tr\Covh$ varies
with $h$ on a given instance, or what the finite-sample error bars are. Results in that second list
are marked empirical. The theory
predicts the destination, not the speed (Figure~\ref{fig:collapsetime}, appendix, computes
when the index posterior commits). The phenomenon practitioners report is temporal
and their question is when to stop, so a characterisation of the collapse time in $N$, $d$
and capacity is the main thing missing here.
...
not early-stop, since overtraining is what we study. Sweeps use five seeds, and the seed
controls both the problem instance and the training randomness, so every error bar (a
population standard deviation) mixes the two; Appendix~\ref{sec:seedsplit}
...
sampler, which is why early stopping works \citep{physicscfm2026}. It then collapses,
stalling near $0.40$ at $2\times10^5$. That residual is an optimisation gap and not a
failure of Proposition~\ref{prop:collapse}, since all four observables keep descending
with the loss. Three measurements sharpen this (Appendix~\ref{app:exp4}). The
memorisation ratio of \citet{yoon2023memorization} reaches $0.650\pm0.033$ conditionally
against $0.389\pm0.032$ unconditionally from a common $\approx0.11$ baseline, so both
flows memorise, as Corollary~\ref{cor:distinction} says, and the conditional one more
strongly. At held-out conditions the variance collapses as at training labels while the
distance to $\mu_{\mathrm{post}}(y)$ grows from $0.12$ to $0.87$: nearest-atom collapse,
not generalisation (cf.\ Corollary~\ref{cor:zerobw}). And permuting $Y$ against $X$ leaves the
collapse intact while that distance grows, so the law follows the arbitrary
label-to-example assignment and not any relation between $x$ and $y$.

\label{sec:p7}%
\paragraph{On synthetic MLPs, label smoothing tracks the kernel reference (P7).} This is our strongest
empirical confirmation of the theory, and it depends on using the right reference,
$\Covh$ and not $\Sigmapost$. Over 20 conditions ($M{=}1000$, 5 seeds) the generated
variance tracks $\Covh$ at every bandwidth: ratio-to-kernel $=1.002\pm0.050$,
$0.994\pm0.039$ and $1.011\pm0.022$ at $h=0.05,0.1,0.5$ (Figure~\ref{fig:p7summary},
Table~\ref{tab:p7kernel}), with the field agreeing too. That $h\approx0.1$ ``perfectly
restores'' the posterior variance is a misreading: $\tr\Cov$ crosses $\Sigmapost$ there
only because $\Sigmapost\approx\Covh|_{h=0.1}$ for this problem. Three consequences
...
$3.5\%$ of $\tr\Sigmapost$, while the population-optimal atomic law already scores below
the trained model in MMD, so the MMD curves do not measure atomicity. And endpoint
smoothing removes the obstruction without improving the fit: the best cell of the
$(h,\rho)$ sweep is $h{=}0.5,\rho{=}0$ in both metrics. It helps only where the atoms are
sparse enough for the floor to dominate, and the gain grows with the floor at fixed $d$
but not across $d$ (Table~\ref{tab:floor}): like kernel density estimation, it is
defeated by dimension. Interpolant noise changes nothing: the generated variance is $0.367\pm0.146$ with
the corrected target, as \eqref{eq:invariance} predicts (Table~\ref{tab:p7i}).

\label{sec:exp23}%
\paragraph{Beyond the Gaussian case.} \emph{EXP-2 (Gaussian mixture).} A 4-mode GMM prior with a rank-deflating linear
observation gives a bimodal analytic posterior. Up to $\sim\!10^4$ iterations mode
coverage is $1.0$ and the MMD near zero, a calibrated sampler. After that coverage falls
to $0.72$ and the MMD grows to $\sim0.25$, while the share of late samples nearest the
$x^i$-containing mode rises from $\approx0.5$ to as much as $1.000$: selective
memorisation (Figs.~\ref{fig:exp2curves} and~\ref{fig:exp23qual}).
\emph{EXP-3 (image inpainting).} Masking the lower half of $32\times32$ MNIST ($N{=}500$),
over three seeds the inpainted-region pixel variance drops $\sim100\times$ by iteration
$15000$ ($0.137\pm0.021\to(1.17\pm0.12)\times10^{-3}$) and the nearest-training-image
distance falls to $(0.77\pm0.26)\times10^{-3}$: completions that are diverse at iteration
200 are identical by 15000 (Fig.~\ref{fig:exp3prog}). A single small-U-Net CIFAR-10
trajectory shows the same qualitative decrease, $0.142\to0.0087$; we treat it as
corroboration, not a replication estimate.
...
Everything above is $d\le10$ synthetic or a $0.5$M U-Net, so one objection is that the
mechanism is an artefact of small capacity. We train a standard DDPM U-Net
($35.75$M parameters) on CIFAR-10 bottom-half inpainting, $N{=}2000$, $d{=}3072$,
$k{=}1536$, for $60000$ iterations at batch $128$, evaluating $48$ conditions at
$M{=}256$ samples each. Each bandwidth is repeated on three independently redrawn
instances, which vary the mask, training subset, and training randomness together
(Appendix~\ref{app:exp4}).

\paragraph{Hard conditioning ($h{=}0$).} On the displayed instance, $\tr\Cov$ falls
from $179.1$ at iteration $500$ to $0.431$ at $60000$, a factor of $415$; all $48$
conditions have $n_{\mathrm{eff}}=1.00$, the generated mean identifies the correct
training image at $48/48$, and the memorisation ratio is $1.000$. Across the three
redrawn instances, final $\tr\Cov$ is $0.409$--$0.467$ and every evaluated condition has
$n_{\mathrm{eff}}=1.00$. Hard-conditioning collapse is therefore robust at this scale, although the experiment
does not separate instance variation from optimisation variation.

\paragraph{Label smoothing ($h>0$): a partial test.}
Table~\ref{tab:cifarddpm} reports the aggregate ratio $\tr\Cov/\tr\Covh$ and the stricter
OLS slope $\beta$ of $\log\tr\Cov$ on $\log\tr\Covh$ across conditions; the population
endpoint law predicts $\beta=1$. The slope tests whether the finite model preserves the
conditional variation in the reference, and not only its level.
...
one axis range, so the collapse of the predictor's spread, from $4.57$ decades at $h{=}4$
to $0.74$ at $h{=}6$, is visible: the right-hand cloud sits on the identity line but has
no slope. By the level, agreement improves with $h$ (median ratio $5.30$, $1.38$,
...
The two statistics disagree. At $h{=}6$ the median ratio is $1.15$ while $\beta=-0.156$:
the reference spans only $0.74$ decades across conditions, so agreement in level is
compatible with ignoring conditional variation. Where the reference has range ($4.57$
decades at $h{=}4$), $\beta=0.481\pm0.037$ with $R^2=0.785$. The model preserves the
predicted ordering but compresses its dynamic range, recovering roughly half of the
population exponent. The checkpoint trajectory rises over the available budget, but it
does not establish $\beta\to1$. We therefore report the DDPM smoothing result as partial
and use the exact endpoint law as an audit target, not as a claim about the fitted model.
...
approximate for the object practitioners train, and the gap widens with $h$. Elsewhere the
population and the trained network agree in direction; here they disagree about a
statement that is either true or false. Collapse is also paced by optimisation, and we
rule out one explanation of that pace without supplying another (Appendix~\ref{sec:aux}).

\paragraph{Scale, replication, and a remedy of limited reach.} EXP-1/EXP-2 are $d\le10$
synthetic problems and EXP-3's MNIST runs use one small U-Net. At $35.75$M parameters on
CIFAR-10, hard-conditioning collapse replicates across three redrawn instances, but
those runs confound instance and optimisation randomness, and the $h>0$ agreement is
partial and not a quantitative confirmation of Theorem~\ref{thm:endpoint}. We do not
establish persistence beyond this scale, or for text-conditional label spaces with no
natural kernel. Endpoint smoothing's benefit is bounded by the obstruction it removes,
which is small in the regime we study most.
...
conditional flow. Label smoothing makes that flow kernel regression over the training
atoms, and neither it nor interpolant noise leaves them. Only endpoint smoothing does,
and it helps to the extent that the floor it removes is large
(Appendix~\ref{app:remedies}).
...
\texttt{train\_exp3}, each followed by the matching analysis script. The original $p7y$
neural checkpoints were not retained. We release the per-run outputs used for
Table~\ref{tab:p7kernel}, Table~\ref{tab:dagger}, and Figure~\ref{fig:kernel}, so those
reported artefacts are reproducible from the supplement; recomputing them from weights
requires rerunning \texttt{run\_sweeps.py}. Proofs of all
...
is a theoretical and empirical account of an existing memorisation phenomenon in
conditional generative models, which bears on the data-privacy and copyright concerns
around deployed diffusion and flow models raised in Section~\ref{sec:intro}. By
characterising the mechanism, we aim to help downstream work design mitigations, not to
facilitate memorisation. We are not aware of any other ethical concerns raised by this
submission.
...
Every result of Section~\ref{sec:theory} is proved in its own subsection below, with its statement repeated for reference; results whose statement already lives in the appendix are proved where they are stated. The technical lemmas come first.
...
\begin{proof}[Proof of Lemma~\ref{lem:cov}]
    The map $\Psi(x_0)=(1-t)x_0+tx^i$ satisfies
    \begin{align*}
        \det D\Psi &= (1-t)^d, \\
        \Psi^{-1}(x) &= \frac{x-tx^i}{1-t},
    \end{align*}
    so $\sigma(X_t\mid I=i)=\sigma(X_0\mid I=i)$. Since $X_0\sim\pi_0$ is independent of $I$,
    \begin{align*}
        p(x\mid I=i,t) &= p_{X_0}\big(\Psi^{-1}(x)\big)\,|\det D\Psi|^{-1} \\
        &= (1-t)^{-d}\,\src{i}.
    \end{align*}
    The factor $(1-t)^{-d}$ does not depend on $i$ and cancels in $\Pr(I=i\mid X_t=x,t)$; positivity follows from $\pi_0>0$.
\end{proof}
...
\begin{proof}[Proof of Lemma~\ref{lem:wellposed}]
    (a) $v$ is a finite sum of products of $C^\infty$ functions, hence locally Lipschitz in $x$, uniformly on compacts of $[0,1)$.

    (b) Existence and uniqueness on $[0,\tau)$ follow from Picard--Lindel\"of \citep[Ch.~II]{hartman2002ode}. Since $w_i\ge0$ and $\sum_iw_i=1$,
    \begin{equation}
    \begin{split}
        |v(x,t)| &\le \sum_iw_i\,\frac{|x^i-x|}{1-t} \\
        &\le \frac{M+|x|}{1-t}.
    \end{split}
    \label{eq:vgrowth}
    \end{equation}
    For $g(t)=|x_t|+M$ this gives $g'\le|v(x_t,t)|\le g/(1-t)$, so by Gr\"onwall
    \begin{align*}
        g(t) &\le g(0)\exp\int_0^t\frac{ds}{1-s} \\
        &= \frac{g(0)}{1-t} \\
        &< \infty \qquad (t<1),
    \end{align*}
    and $\tau=1$.

    (c) For $\varphi\in C_c^\infty(\R^d)$ and $\mu_t=(\Phi_t)_\#\mu_0$,
    \begin{align*}
        \frac{d}{dt}\int\varphi\,d\mu_t &= \int\nabla\varphi(\Phi_t)\cdot v(\Phi_t,t)\,d\mu_0 \\
        &= \int\nabla\varphi\cdot v\,d\mu_t,
    \end{align*}
    the weak form of $\partial_t\mu_t+\nabla\!\cdot(v\mu_t)=0$. For uniqueness, let $(\mu_t)_{t\le T}$, $T<1$, be a narrowly continuous solution and $\psi(x,s)=\varphi(\Phi_{s,T}(x))$, which is $C^1$, compactly supported in $x$, and satisfies
    \begin{equation*}
        \partial_s\psi+v\cdot\nabla\psi=0,\qquad\psi(\cdot,T)=\varphi .
    \end{equation*}
    The weak form extends to such time-dependent test functions by mollification in $s$, since $v$ is bounded on compacts \citep[Ch.~8]{ambrosio2008gradient}. Then
    \begin{align*}
        \frac{d}{ds}\int\psi(\cdot,s)\,d\mu_s &= \int\big(\partial_s\psi+v\cdot\nabla\psi\big)\,d\mu_s \\
        &= 0,
    \end{align*}
    so
    \begin{align*}
        \int\varphi\,d\mu_T &= \int\psi(\cdot,0)\,d\mu_0 \\
        &= \int\varphi\circ\Phi_{0,T}\,d\mu_0 \\
        &= \int\varphi\,d\big((\Phi_T)_\#\mu_0\big),
    \end{align*}
    i.e.\ $\mu_T=(\Phi_T)_\#\mu_0$ \citep[see also][Ch.~8]{ambrosio2008gradient}.
\end{proof}
...
\begin{proof}[Proof of Lemma~\ref{lem:mixture-coupling}]
    By Bayes, Lemma~\ref{lem:cov} and $\pih{i}(y)\propto \KK{i}$,
    \begin{align*}
        \Pr(I=i\mid X_t=x,t) &\propto \pih{i}(y)\,(1-t)^{-d}\,\src{i} \\
        &\propto \src{i}\KK{i},
    \end{align*}
    the numerator of $\wih{i}$ in \eqref{eq:kernel-field}. Both are normalised over $i$, so $\Pr(I=i\mid X_t=x,t)=\wih{i}$. As $X_1-X_0=\fat{i}$ given $I=i$,
    \begin{align*}
        \E[X_1-X_0\mid X_t=x,t] &= \sum_i\wih{i}\,\fat{i} \\
        &= \vstar(x,t,y).\qedhere
    \end{align*}
\end{proof}

\subsection{Proof of Proposition~\ref{prop:projection}}
\label{apx:pf-projection}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:projection}} ($L^2$ projection).
For every square-integrable $v$,
$L_h(v)=\E[\Var(U\mid X_t,t,\widetilde Y)]+\E\,|v-\E[U\mid X_t,t,\widetilde Y]|^2$,
so the a.e.-unique minimiser is $\vstar(x,t,y)=\E[U\mid X_t=x,t,\widetilde Y=y]$ with
...
\begin{proof}[Proof of Proposition~\ref{prop:projection}]
    Let $\mathcal G=\sigma(X_t,t,\widetilde Y)$ and $m=\E[U\mid\mathcal G]$. Then
    \begin{align*}
        L_h(v) &= \E|U-m|^2+\E|m-v|^2+2\,\E\langle U-m,\,m-v\rangle \\
        &= \E[\Var(U\mid\mathcal G)]+\E|v-m|^2,
    \end{align*}
    since
    \begin{align*}
        ...
        &= 0.
    \end{align*}
    The last term vanishes iff $v=m$ a.e., so $\vstar=m$ and $\inf_vL_h=\E[\Var(U\mid\mathcal G)]$.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:collapse}}
\label{apx:pf-collapse}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:collapse}} (Exact conditional collapse; $h=0$).
Let $h=0$. Then
(a) $v_0^\star(x,t,y^i)=(x^i-x)/(1-t)$ for $t<1$ $(\star)$;
(b) $\inf_v L_0(v)=0$;
(c) the ODE $\dot x_t=(x^i-x_t)/(1-t)$ has the explicit solution
...
\begin{proof}[Proof of Proposition~\ref{prop:collapse}]
    By Assumption~\ref{as:standing}(iii), $Y=y^i$ gives $I=i$ and $X_1=x^i$; by Lemma~\ref{lem:cov}, $X_0=\frac{x-tx^i}{1-t}$ given $X_t=x$. Hence
    \begin{equation}
        U = x^i-X_0 = \fat{i},
        \label{eq:pf:collapse:2}
    \end{equation}
    a function of the conditioning variables. By Proposition~\ref{prop:projection},
    \begin{align*}
        v_0^\star(x,t,y^i) &= \E[U\mid X_t=x,t,Y=y^i] \\
        &= \fat{i},
    \end{align*}
    which is (a). By \eqref{eq:pf:collapse:2},
    \begin{align*}
        U-v_0^\star(X_t,t,Y) &= 0 \quad\text{a.s.}, \\
        L_0(v_0^\star) &= 0 = \inf_vL_0,
    \end{align*}
    which is (b). Along $\dot x_t=\frac{x^i-x_t}{1-t}$,
    \begin{align*}
    ...
    \begin{align*}
        x_t &= (1-t)x_0+tx^i \\
        &\to x^i \qquad (t\uparrow1),
    \end{align*}
    and $p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$ with $\Cov=0$, which is (c) and (d).
\end{proof}

\subsection{Proof of Corollary~\ref{cor:repeated}}
\label{apx:pf-repeated}

\begin{corollary}[Repeated labels]
\label{cor:repeated}
Drop the distinctness in Assumption~\ref{as:standing}(iii), let $h=0$, and for a query label $y$ let
$\mathcal I_y=\{i:y^i=y\}\ne\emptyset$. Then
\begin{align*}
    \vstarz(x,t,y) &= \sum_{i\in\mathcal I_y}q_i^y(x,t)\,\fat{i}, \qquad
    q_i^y\propto\src{i}\ \text{ on }\mathcal I_y, \\
    p_1^{\mathrm{cond}}(\cdot\mid y) &= \frac1{|\mathcal I_y|}\sum_{i\in\mathcal I_y}\delta_{x^i},
\end{align*}
whose covariance is the within-label scatter $|\mathcal I_y|^{-1}\sum_{i\in\mathcal I_y}(x^i-\bar x_y)(x^i-\bar x_y)^\top$,
$\bar x_y=|\mathcal I_y|^{-1}\sum_{i\in\mathcal I_y}x^i$.
\end{corollary}

\begin{proof}[Proof of Corollary~\ref{cor:repeated}]
    Given $Y=y$, $I$ is uniform on $\mathcal I_y$ and $X_0\sim\pi_0$ is independent of $I$. By Bayes and Lemma~\ref{lem:cov},
    \begin{align*}
        \Pr(I=i\mid X_t=x,t,Y=y) &= \frac{\src{i}}{\sum_{j\in\mathcal I_y}\src{j}} \\
        &= q_i^y(x,t),
    \end{align*}
    and given $I=i$, $U=\fat{i}$ by \eqref{eq:pf:collapse:2}, so by Proposition~\ref{prop:projection}
    \begin{align*}
        \vstarz(x,t,y) &= \E[U\mid X_t=x,t,Y=y] \\
        &= \sum_{i\in\mathcal I_y}q_i^y\,\fat{i}.
    \end{align*}
    The weights $q_i^y$ are smooth, positive and sum to one. As in the proof of Theorem~\ref{thm:endpoint}, with $I$ uniform on $\mathcal I_y$, the flow is well posed, $(\Phi_t)_\#\pi_0=\law(X_t)$, and $X_t\to x^I$ a.s., so
    \begin{align*}
        p_1^{\mathrm{cond}}(\cdot\mid y) &= \law(x^I) \\
        &= \frac1{|\mathcal I_y|}\sum_{i\in\mathcal I_y}\delta_{x^i},\qedhere
    \end{align*}
\end{proof}

\subsection{Proof of Proposition~\ref{prop:uncond}}
\label{apx:pf-uncond}
...
\begin{proof}[Proof of Proposition~\ref{prop:uncond}]
    By Bayes and Lemma~\ref{lem:cov} ($\frac1N(1-t)^{-d}$ cancels),
    \begin{align*}
        q_i(x,t) &= \frac{\src{i}}{\sum_j\src{j}} \\
        &> 0 .
    \end{align*}
    Given $I=i$, $U=\fat{i}$ by \eqref{eq:pf:collapse:2}, so by Proposition~\ref{prop:projection}
    \begin{align*}
        v_{\mathrm{unc}}^\star(x,t) &= \E[U\mid X_t=x,t] \\
        &= \sum_iq_i(x,t)\,\fat{i}.
    \end{align*}
    This has the form of Lemma~\ref{lem:wellposed}. As in the proof of Theorem~\ref{thm:endpoint}, with the uniform law on $I$, $p_t=\law(X_t)$, and $X_t\to X_1\sim\rhohat_X$ a.s., so $p_1^{\mathrm{unc}}=\rhohat_X$.

    With $u_i=\fat{i}$ and $\bar u=\sum_iq_iu_i$,
    \begin{align*}
        \Var(U\mid X_t=x,t) &= \sum_iq_i\,|u_i-\bar u|^2 \\
        &> 0
    \end{align*}
    whenever $x^i\ne x^j$ for some $i,j$ ($q_i>0$ and $u_i\ne u_j$). Hence $\inf_vL_{\mathrm{unc}}=\E[\Var(U\mid X_t,t)]>0$.
\end{proof}

\subsection{Proof of Corollary~\ref{cor:distinction}}
\label{apx:pf-distinction}

\begin{quote}\small
\noindent\textbf{Corollary~\ref{cor:distinction}} (The correct conditional/unconditional distinction).
Both exact population flows are supported on the training set:
$p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$ and
$p_1^{\mathrm{unc}}=\frac1N\sum_i\delta_{x^i}$. The difference is between
single-example and full-empirical-measure memorisation, not between memorising and not,
...
\begin{proof}[Proof of Corollary~\ref{cor:distinction}]
    By Proposition~\ref{prop:collapse}(d), $p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$ and $\Cov=0$. By Proposition~\ref{prop:uncond},
    \begin{align*}
        \Cov[p_1^{\mathrm{unc}}] &= \frac1N\sum_i(x^i-\bar x)(x^i-\bar x)^\top \\
        &= \Xhat .
    \end{align*}
    Both laws are mixtures of point masses at training examples.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:kernel-field}}
\label{apx:pf-kernel-field}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:kernel-field}} (Kernel-weighted population minimiser).
For $h>0$, $t<1$,
\begin{equation*}
\vstar(x,t,y)=\sum_i w_i^{(h)}(x,t,y)\,\frac{x^i-x}{1-t},\quad
w_i^{(h)}=\frac{\pi_0\!\big(\tfrac{x-tx^i}{1-t}\big)K_h(y-y^i)}
...
\begin{proof}[Proof of Proposition~\ref{prop:kernel-field}]
    It suffices to compute the posterior of the index $I$ given $(X_t,\widetilde Y)$.
    Given $I=i$, $X_t=(1-t)X_0+tx^i$ and $\widetilde Y=y^i+h\eps$ with $X_0\perp\eps$, so $X_t$ and $\widetilde Y$ are conditionally independent and their joint density factorises. By Bayes and Lemma~\ref{lem:cov},
    \begin{align*}
        \Pr(I=i\mid X_t=x,t,\widetilde Y=y) &= \frac{\src{i}\KK{i}}{\sum_j\src{j}\KK{j}} \\
        &= \wih{i},
    \end{align*}
    since $\frac1N(1-t)^{-d}$ cancels. Given $I=i$ and $X_t=x$, $U=\fat{i}$ by \eqref{eq:pf:collapse:2}, so by Proposition~\ref{prop:projection}
    \begin{align*}
        \vstar(x,t,y) &= \E[U\mid X_t=x,t,\widetilde Y=y] \\
        &= \sum_i\wih{i}(x,t,y)\,\fat{i}.\qedhere
    \end{align*}
\end{proof}

\subsection{Proof of Theorem~\ref{thm:endpoint}}
\label{apx:pf-endpoint}

\begin{quote}\small
\noindent\textbf{Theorem~\ref{thm:endpoint}} (Endpoint law under label smoothing).
For every $h>0$ and query $y$,
\begin{equation*}
p_h^{\mathrm{gen}}(\cdot\mid y)=\sum_{i=1}^N p_i^{(h)}(y)\,\delta_{x^i}.
\end{equation*}
...
\begin{proof}[Proof of Theorem~\ref{thm:endpoint}]
    The marginals of the coupling below solve the continuity equation of the flow, so they equal the flow's marginals, and they converge to $\law(x^I)$. Here $p_h^{\mathrm{gen}}(\cdot\mid y):=\lim_{t\uparrow1}(\Phi_t)_\#\pi_0$ for the flow $\Phi$ of $\vstar(\cdot,\cdot,y)$.
    By Lemma~\ref{lem:mixture-coupling}, $\vstar(x,t,y)=\E[X_1-X_0\mid X_t=x,t]$ for the coupling $I\sim p^{(h)}(y)$, $X_1=x^I$. The weights $\wih{i}$ are smooth, positive and sum to one, so by Lemma~\ref{lem:wellposed} the flow $(\Phi_t)_{t<1}$ exists and
    \begin{equation}
        \partial_tp_t+\nabla\!\cdot\big(p_t\vstar\big)=0,\qquad p_0=\pi_0,
        \label{eq:pf:endpoint:2}
    \end{equation}
    has a unique solution. $(\Phi_t)_\#\pi_0$ solves it, and so does $\law(X_t)$: for $\varphi\in C_c^\infty$,
    \begin{align*}
        \frac{d}{dt}\E\,\varphi(X_t) &= \E\langle\nabla\varphi(X_t),\,X_1-X_0\rangle \\
        &= \E\langle\nabla\varphi(X_t),\,\vstar(X_t,t,y)\rangle,
    \end{align*}
    by conditioning on $X_t$ (Lemma~\ref{lem:mixture-coupling}). Hence
    \begin{align*}
        (\Phi_t)_\#\pi_0 &= \law(X_t) =: p_t .
    \end{align*}
    Since $X_t=(1-t)X_0+tx^I\to x^I$ a.s.,
    \begin{align*}
        p_1 &= \lim_{t\uparrow1}p_t \\
        &= \law(x^I) \\
        &= \sum_i\pih{i}(y)\,\delta_{x^i}.\qedhere
    \end{align*}
\end{proof}

This is convergence in law of the marginals of the flow, which is what $p_1$ denotes. Convergence of
individual trajectories is not claimed; Proposition~\ref{prop:cfgatom} assumes it for guided flows.

\subsection{Proof of Corollary~\ref{cor:zerobw}}
\label{apx:pf-zerobw}

\begin{quote}\small
\noindent\textbf{Corollary~\ref{cor:zerobw}} (Zero bandwidth at fixed $N$).
If $y$ has a unique nearest label $i^\star=\arg\min_i|y-y^i|$, then
$p_i^{(h)}(y)\to\mathbf 1\{i=i^\star\}$ as $h\to0$, so
$p_h^{\mathrm{gen}}(\cdot\mid y)\Rightarrow\delta_{x^{i^\star}}$; at $y=y^i$ this
...
\begin{proof}[Proof of Corollary~\ref{cor:zerobw}]
    For $i\ne i^\star$,
    \begin{align*}
        \frac{\KK{i}}{\KK{i^\star}} &= \exp\Big[-\frac{|y-y^i|^2-|y-y^{i^\star}|^2}{2h^2}\Big] \\
        &\xrightarrow[h\to0]{} 0,
    \end{align*}
    so $\pih{i}(y)\to\ind\{i=i^\star\}$, and $p_h^{\mathrm{gen}}\Rightarrow\delta_{x^{i^\star}}$ by Theorem~\ref{thm:endpoint}.
\end{proof}

\subsection{Proof of Corollary~\ref{cor:infbw}}
\label{apx:pf-infbw}

\begin{quote}\small
\noindent\textbf{Corollary~\ref{cor:infbw}} (Infinite bandwidth at fixed $N$).
$p_i^{(h)}(y)\to1/N$ for every $i$ as $h\to\infty$, so
$p_h^{\mathrm{gen}}(\cdot\mid y)\Rightarrow p_1^{\mathrm{unc}}$.
\end{quote}

\begin{proof}[Proof of Corollary~\ref{cor:infbw}]
    Uniformly in $i$, $\KK{i}=(2\pi h^2)^{-k/2}\big[1+O(h^{-2})\big]$. The prefactor cancels:
    \begin{align*}
        \pih{i}(y) &= \frac{1+O(h^{-2})}{N+O(h^{-2})} \\
        &\longrightarrow \frac1N .
    \end{align*}
    By Theorem~\ref{thm:endpoint}, $p_1\to\frac1N\sum_i\delta_{x^i}=\rhohat_X$.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:moments}}
\label{apx:pf-moments}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:moments}} (Exact finite-$N$ moments).
$\bar x_h(y)=\sum_i p_i^{(h)}(y)\,x^i$ and
$\Covh(y)=\sum_i p_i^{(h)}(y)\,(x^i-\bar x_h(y))(x^i-\bar x_h(y))^\top$, both computable
from the training set for any $h$; they are the reference for generated moments.
$\tr\Covh(y)$ need not be monotone in $h$.
\end{quote}

\begin{proof}[Proof of Proposition~\ref{prop:moments}]
    By Theorem~\ref{thm:endpoint} the endpoint law is $\sum_i\pih{i}\delta_{x^i}$, so
    \begin{align*}
        \bar x_h(y) &= \sum_i\pih{i}x^i, \\
        \Covh(y) &= \sum_i\pih{i}(x^i-\bar x_h)(x^i-\bar x_h)^\top .
    \end{align*}
    For non-monotonicity take $k=d=1$, $y=0$ and $(y^i,x^i)=(0,0),(1,10),(10,0)$. Then
    \begin{align*}
        h\to0: &\qquad p^{(h)}\to(1,0,0), && \tr\Covh\to0, \\
        h=2: &\qquad p^{(h)}=(0.531,\,0.469,\,2{\times}10^{-6}), && \tr\Covh=24.90, \\
        h\to\infty: &\qquad p^{(h)}\to(\tfrac13,\tfrac13,\tfrac13), && \tr\Covh\to\tr\Xhat=\tfrac{200}{9}=22.22
    \end{align*}
    (Corollaries~\ref{cor:zerobw}, \ref{cor:infbw}).
\end{proof}

\subsection{Proof of Proposition~\ref{prop:atomicity}}
\label{apx:pf-atomicity}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:atomicity}} (Label smoothing never leaves the training set).
For every $h>0$, and for $h=0$ at $y=y^i$, $\operatorname{supp}(p_h^{\mathrm{gen}}(\cdot\mid y))
\subseteq\{x^1,\dots,x^N\}$. Consequently, if the true posterior $p(\cdot\mid y)$ puts no
mass on $\{x^i\}$,
\begin{equation*}
...
\begin{proof}[Proof of Proposition~\ref{prop:atomicity}]
    Any coupling with an atomic law moves each posterior draw at least to its nearest atom, and that bound involves the atoms alone.
    By Theorem~\ref{thm:endpoint}, $p_h^{\mathrm{gen}}=\sum_i\pih{i}\delta_{x^i}$, so $\supp\,p_h^{\mathrm{gen}}\subseteq\{x^i\}$. For any coupling $(X,X')$ of $p_h^{\mathrm{gen}}$ and $p(\cdot\mid y)$, $X\in\{x^i\}$ a.s., so
    \begin{align*}
        \E|X-X'|^2 &\ge \E\dist(X',\{x^i\})^2 \\
        &= \int\dist(x,\{x^i\})^2\,p(x\mid y)\,dx =: F(y).
    \end{align*}
    Taking the infimum over couplings gives \eqref{eq:w2floor}. $F$ does not involve $h$, and $F(y)>0$ since $\{x^i\}$ is $p(\cdot\mid y)$-null.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:tgtnoise}}
\label{apx:pf-tgtnoise}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:tgtnoise}} (Endpoint smoothing).
Replace the endpoint by $X_1=x^I+\rho\,\xi$ with $\xi\sim\N(0,I_d)$ independent of
$(I,X_0,\varepsilon)$ and redrawn every step, keeping label bandwidth $h>0$. Write
$s_t^2=(1-t)^2+t^2\rho^2$. Then $\vstar$ is again a weighted average of per-atom
fields, now with spatial weights $\varphi_{s_t}(x-tx^i)$ in place of
...
For every $\rho>0$ this law is absolutely continuous, so \eqref{eq:w2floor} does not
apply to it; moreover $s_1=\rho>0$, so $\vstar$ extends continuously to $t=1$ and the
$1/(1-t)$ blow-up of $(\star)$ disappears.
\end{quote}

\begin{proof}[Proof of Proposition~\ref{prop:tgtnoise}]
    Given $I=i$,
    \begin{align*}
        r:=x-tx^i &= (1-t)X_0+t\rho\xi \\
        &\sim \N(0,s_t^2I_d), \qquad s_t^2=(1-t)^2+t^2\rho^2,
    \end{align*}
    so $p(x\mid I=i,t)=\varphi_{s_t}(x-tx^i)$ and, as in Proposition~\ref{prop:kernel-field},
    \begin{align*}
        w_i &= \Pr(I=i\mid X_t=x,t,\widetilde Y=y) \\
        &\propto \varphi_{s_t}(x-tx^i)\,\KK{i}.
    \end{align*}
    Since $(X_0,\xi,r)$ is jointly Gaussian,
    \begin{align*}
        \E[X_0\mid r] &= \frac{1-t}{s_t^2}\,r, \\
        \E[\xi\mid r] &= \frac{t\rho}{s_t^2}\,r,
    \end{align*}
    and with $U=x^i+\rho\xi-X_0$,
    \begin{equation}
        \E[U\mid X_t=x,t,I=i] = x^i+\frac{t\rho^2-(1-t)}{s_t^2}\,(x-tx^i).
        \label{eq:pf:tgtnoise:4}
    \end{equation}
    By Proposition~\ref{prop:projection},
    \begin{equation}
        \vstar(x,t,y) = \sum_iw_i\Big[x^i+\frac{t\rho^2-(1-t)}{s_t^2}\,(x-tx^i)\Big].
        \label{eq:tgtfield}
    \end{equation}
    The field \eqref{eq:tgtfield} is smooth on $\R^d\times[0,1]$ with at most linear growth, so the flow is global and the uniqueness argument of Lemma~\ref{lem:wellposed}(c) applies verbatim.
    For the coupling $I\sim p^{(h)}(y)$, $X_0\sim\pi_0$, $\xi\sim\N(0,I_d)$ independent, $X_1=x^I+\rho\xi$, the index posterior is $\propto \pih{i}\varphi_{s_t}(x-tx^i)\propto w_i$ and \eqref{eq:pf:tgtnoise:4} holds, so its conditional velocity is $\vstar$. As in the proof of Theorem~\ref{thm:endpoint}, and since $X_t\to X_1$ a.s.,
    \begin{align*}
        p_1 &= \law(x^I+\rho\xi) \\
        &= \sum_i\pih{i}(y)\,\N(x^i,\rho^2I_d),
    \end{align*}
    with
    \begin{align*}
        \text{mean} &= \bar x_h(y), \\
        \Cov &= \rho^2I_d+\Covh(y).
    \end{align*}
    It is absolutely continuous, so \eqref{eq:w2floor} does not apply. Since $s_1=\rho>0$, $\frac{t\rho^2-(1-t)}{s_t^2}\to1$, and $\vstar$ is bounded on bounded sets up to $t=1$.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:prop17}}
\label{apx:pf-prop17}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:prop17}} (Closed form and endpoint invariance in $\sigma$).
Let $h=0$ and $v_\sigma^\star=\E[\dot X_t\mid X_t,t,Y]$ for target
\eqref{eq:c2target}. Then for $t\in(0,1)$:
(a) $v_\sigma^\star(x,t,y^i)=x^i+c(t)(x-tx^i)$ with
$c(t)=\frac{-(1-t)+\frac{\sigma^2}2(1-2t)}{s_t^2}=\frac12\frac{d}{dt}\log s_t^2$;
...
\begin{proof}[Proof of Proposition~\ref{prop:prop17}]
    Given $I=i$ (pinned by $Y=y^i$),
    \begin{align*}
        r:=x-tx^i &= (1-t)X_0+\gamma Z \\
        &\sim \N(0,s_t^2I_d), \qquad s_t^2=(1-t)^2+\gamma^2,
    \end{align*}
    with $\Cov(X_0,r)=(1-t)I_d$ and $\Cov(Z,r)=\gamma I_d$, so
    \begin{align*}
        \E[X_0\mid r] &= \frac{1-t}{s_t^2}\,r, \\
        \E[Z\mid r] &= \frac{\gamma}{s_t^2}\,r.
    \end{align*}
    Since $\dot X_t=x^i-X_0+\dot\gamma Z$,
    \begin{align*}
        v_\sigma^\star(x,t,y^i) &= x^i+\frac{-(1-t)+\dot\gamma\gamma}{s_t^2}\,r, \\
        \dot\gamma\gamma &= \tfrac12(\gamma^2)' = \tfrac{\sigma^2}2(1-2t),
    \end{align*}
    which is (a) with $c(t)=\frac{-(1-t)+\frac{\sigma^2}2(1-2t)}{s_t^2}$. Moreover
    \begin{align*}
        (s_t^2)' &= -2(1-t)+2\dot\gamma\gamma \\
        &= 2s_t^2\,c(t),
    \end{align*}
    so $c=\frac12(\log s_t^2)'$.

    (b) $u_t=x_t-tx^i$ satisfies $\dot u_t=c(t)u_t$, so
    \begin{align*}
        u_t &= x_0\exp\!\big(\tfrac12[\log s_t^2-\log s_0^2]\big) \\
        &= s_t\,x_0,
    \end{align*}
    that is, $x_t=tx^i+s_tx_0$.

    (c) $s_t^2=(1-t)\big[(1-t)+\sigma^2t\big]$, so $s_1=0$ and $x_t\to x^i$ for all $x_0$ and $\sigma\ge0$.
\end{proof}

\subsection{Proof of Corollary~\ref{cor:prop18}}
\label{apx:pf-prop18}
...
($\inf_v L_\sigma=+\infty$ for $\sigma>0$, whereas $\inf_v L_0=0$) and halves the Lipschitz
blow-up rate: $\lim_{t\uparrow1}(1-t)|c(t)|=1$ for $\sigma=0$ and $\tfrac12$ for
$\sigma>0$.
\end{corollary}

\begin{proof}[Proof of Corollary~\ref{cor:prop18}]
    The endpoint is Proposition~\ref{prop:prop17}(c). For $\sigma=0$, $\dot X_t=X_1-X_0$ and $\inf_vL_0=0$ (Proposition~\ref{prop:collapse}). For $\sigma>0$, $\inf_vL_\sigma=\E\Var(\dot X_t\mid X_t,t,Y)$. With $u=(-1,\dot\gamma)$, $w=(1-t,\gamma)$, $|w|^2=s_t^2$ and $u\cdot w=c\,s_t^2$,
    \begin{align*}
        \Var(\dot X_t\mid r,I=i) &= |u|^2-\frac{(u\cdot w)^2}{|w|^2} \\
        &= 1+\dot\gamma^2-c(t)^2s_t^2 .
    \end{align*}
    Near $t=0$, $c$ is bounded and $\dot\gamma^2\sim\sigma^2/(4t)$, so $\int_0^1\Var\,dt=+\infty$ and $\inf_vL_\sigma=+\infty$. Then $v_\sigma^\star$ is the conditional mean of the pathwise target, not an $L^2$-minimiser. Finally
    \begin{align*}
...
\label{apx:pf-factors}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:factors}} (The two mechanisms act on different factors).
Let the atoms be not all equal and $y$ have a unique nearest label $i^\star$. Label smoothing
changes the endpoint law: $p_h^{\mathrm{gen}}(\cdot\mid y)$ tends to $\delta_{x^{i^\star}}$ as
$h\to0$ and to $\rhohat_X$ as $h\to\infty$, and these differ. Interpolant noise does not:
although it replaces the spatial factor $\N(tx^i,(1-t)^2I_d)$ of Lemma~\ref{lem:cov} by
$\N(tx^i,s_t^2I_d)$, identically for every $i$, $p_1^{\mathrm{cond}}(\cdot\mid y^i)=\delta_{x^i}$ for
every $\sigma\ge0$ (Proposition~\ref{prop:prop17}). The two remedies are therefore not
interchangeable.
\end{quote}

\begin{proof}[Proof of Proposition~\ref{prop:factors}]
    Label factor: $p_h^{\mathrm{gen}}=\sum_i\pih{i}\delta_{x^i}$ depends on $h$ through $\KK{i}$, and by Corollaries~\ref{cor:zerobw}, \ref{cor:infbw}
    \begin{align*}
        p_h^{\mathrm{gen}} &\Rightarrow \delta_{x^{i^\star}} \quad (h\to0,\ \text{unique nearest label}), \\
        p_h^{\mathrm{gen}} &\Rightarrow \rhohat_X \quad (h\to\infty),
    \end{align*}
    which are distinct because the atoms are not all equal. Spatial factor: for $h=0$ and any $\sigma\ge0$,
    \begin{align*}
        \N\big(tx^i,(1-t)^2I_d\big) &\longrightarrow \N\big(tx^i,s_t^2I_d\big) \quad \forall i, \\
        p_1^{\mathrm{cond}}(\cdot\mid y^i) &= \delta_{x^i} \quad \forall\sigma
    \end{align*}
    (Proposition~\ref{prop:prop17}(c)).
\end{proof}

\subsection{Proof of Proposition~\ref{prop:expansion}}
\label{apx:pf-expansion}
...
assumes, and the fluctuation estimate in the proof of Proposition~\ref{prop:expansion} is the only place it is used.

\begin{proposition}[Bandwidth expansion of the conditional covariance; random design]
\label{prop:expansion}
Under Assumption~\ref{as:design}, pointwise at $y$, as $Nh^k\to\infty$ and $h\to0$ (the $h^2$ term is identifiable only if $Nh^{k+4}\to\infty$),
\begin{equation}
...
\begin{proof}[Proof of Proposition~\ref{prop:expansion}]
    Let $s=\nabla\log\rho_Y(y)$, $q_h\propto K_h(y-\cdot)\rho_Y$, $\E_h[f]=\E[K_h(y-Y)f]/\E[K_h(y-Y)]$ and $u=(Y-y)/h$. Let $\Cov_h^{\mathrm{pop}}:=\Cov_h[X]$ be the population counterpart of $\Covh$; its deviation from $\Covh$ at $h=h_N$ is bounded in the last part. By total covariance
    \begin{equation}
        \Cov_h^{\mathrm{pop}} = \E_h[\Sigma(Y)]+\Cov_h[\mu(Y)].
        \label{eq:pf:expansion:2}
    \end{equation}
    Substituting $y'=y+hu$, $K_h(y-y')=h^{-k}\varphi(u)$, and Taylor expanding $\rho_Y$,
    \begin{equation}
    \begin{split}
        \tilde q_h(u) &= \frac{\varphi(u)\rho_Y(y+hu)}{\int\varphi\,\rho_Y(y+h\cdot)} \\
        &= \varphi(u)\big[1+h\,s^\top u+O(h^2)\big].
    \end{split}
    \label{eq:pf:expansion:3}
    \end{equation}
    With $\E_\varphi[u]=0$, $\E_\varphi[uu^\top]=I_k$, $\E_\varphi[u_au_bu_c]=0$ and $\E_\varphi[u_au_bu_cu_d]=\delta_{ab}\delta_{cd}+\delta_{ac}\delta_{bd}+\delta_{ad}\delta_{bc}$,
    \begin{equation}
    \begin{split}
        \E_{\tilde q_h}[u] &= h\,s+O(h^2), \\
        \E_{\tilde q_h}[uu^\top] &= I_k+O(h^2), \\
        \Cov_{\tilde q_h}[u] &= I_k+O(h^2).
    \end{split}
    \label{eq:pf:expansion:6}
    \end{equation}

    \emph{Within-group.} $\Sigma\equiv\Sigma(y)$ on $B(y,r)$, and outside
    \begin{align*}
        \int_{|u|>r/h}(1+|y+hu|^{2m})\varphi(u)\,du &\le C\,e^{-r^2/4h^2} \\
        &= o(h^p) \quad \forall p,
    \end{align*}
    so $\E_h[\Sigma(Y)]=\Sigma(y)+O(h^4)$. Without homoscedasticity, for $\Sigma\in C^2$, \eqref{eq:pf:expansion:6} gives
    \begin{align*}
        \E_h[\Sigma(Y)] &= \Sigma+h\,\nabla\Sigma\cdot\E[u]+\tfrac{h^2}2\nabla^2\Sigma\cdot\E[uu^\top]+o(h^2) \\
        &= \Sigma+h^2\big[\nabla\Sigma\cdot s+\tfrac12\tr\nabla^2\Sigma\big]+o(h^2),
    \end{align*}
    of the order of $h^2JJ^\top$. For example $k=d=1$, $Y\sim\mathrm{Unif}(-2,2)$, $X\mid Y=y\sim\N(0,1+y^2)$ has $J\equiv0$ and $\Covh(y)=\Sigma(y)+h^2$.

    \emph{Between-group.} Since $\mu\in C^3$,
    \begin{align*}
        \mu(y+hu) &= \mu(y)+hJu+\tfrac{h^2}2Q(u)+R_h(u), \\
        Q(u)_a &= \sum_{b,c}\partial^2_{bc}\mu_a(y)\,u_bu_c, \\
        |R_h(u)| &\le \tfrac{M_3}6\,h^3|u|^3 .
    \end{align*}
    The moments of $\tilde q_h$ are bounded for $h\le1$, so the covariances involving $R_h$ are $O(h^4)$, $O(h^5)$, $O(h^6)$, and
    \begin{align*}
        \Cov_h[\mu(Y)] &= h^2J\,\Cov[u]\,J^\top+h^3\big[J\,\Cov(u,Q)+\mathrm{sym}\big] \\
        &\quad+h^4\Cov[Q]+O(h^4).
    \end{align*}
    By \eqref{eq:pf:expansion:3}, $\E_{\tilde q_h}[u_au_bu_c]=h\,[s_c\delta_{ab}+s_b\delta_{ac}+s_a\delta_{bc}]+O(h^2)$, so $\Cov(u,Q)=O(h)$; with \eqref{eq:pf:expansion:6},
    \begin{equation}
    \begin{split}
        \Cov_h[\mu(Y)] &= h^2JJ^\top+O(h^4), \\
        \Cov_h^{\mathrm{pop}} &= \Sigma(y)+h^2JJ^\top+O(h^4).
    \end{split}
    \label{eq:pf:expansion:1}
    \end{equation}

    \emph{Fluctuation.} With $S_N=\frac1N\sum_j\KK{j}$, $T_N=\frac1N\sum_j\KK{j}x^j$ and $\bar x_h=T_N/S_N$,
    \begin{align*}
        \Var\big(K_h(y-Y)\big) &= O(h^{-k}), \\
        \Var\big(K_h(y-Y)XX^\top\big) &= O(h^{-k}),
    \end{align*}
    the latter using $m_4$ bounded near $y$ (the only use of (iv)). Hence $S_N,T_N=\E[\cdot]+O_P(\sqrt{h^{-k}/N})$, with $\E[K_h(y-Y)]\to\rho_Y(y)>0$, and by the delta method
    \begin{align*}
        \bar x_h &= \E_h[X]+O_P\big((Nh^k)^{-1/2}\big), \\
        \Covh &= \Cov_h^{\mathrm{pop}}+O_P\big((Nh^k)^{-1/2}\big).
    \end{align*}
    With \eqref{eq:pf:expansion:1} this is \eqref{eq:expansion}.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:nwrate}}
\label{apx:pf-nwrate}

\begin{proposition}[Nadaraya--Watson rates for the conditional mean]
\label{prop:nwrate}
Under the assumptions of Proposition~\ref{prop:expansion}, with the Gaussian label kernel
$K$ of the setup, $\bar x_h(y)$ is exactly the Nadaraya--Watson estimator of $\mu(y)$, with
\[
...
\begin{proof}[Proof of Proposition~\ref{prop:nwrate}]
    Under Assumption~\ref{as:design}, with $K$ bounded and symmetric, $\int K=1$, $h\to0$, $Nh^k\to\infty$ \citep[\S1.5--1.6]{tsybakov2009nonparametric}, \citep[Ch.~5]{gyorfi2002distribution}, and $\mu_2(K)=\int u_1^2K$, $R(K)=\int K^2$,
    \begin{align*}
        \mathrm{Bias} &= \tfrac{h^2}2\,\mu_2(K)\,b(y)+o(h^2), \\
        \Var &= \frac{R(K)\,\Sigma(y)}{Nh^k\rho_Y(y)}+o\big((Nh^k)^{-1}\big).
    \end{align*}
    Hence $\mathrm{MSE}=c_1h^4+c_2(Nh^k)^{-1}+o(\cdot)$ with $c_1=\frac14\mu_2(K)^2|b(y)|^2$, and
    \begin{align*}
        \frac{d}{dh}\mathrm{MSE} &= 4c_1h^3-c_2\,k\,N^{-1}h^{-k-1} = 0 \\
        &\iff h^{k+4}=\frac{c_2\,k}{4c_1N},
    \end{align*}
    a minimum, so $h^\star\asymp N^{-1/(k+4)}$ ($c_1,c_2>0$).

    Linear-Gaussian instance: $\mu(y')=By'$ and, in $y'$, $K_h(y-y')\rho_Y(y')\propto\N\big(\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}y,\,\cdot\big)$. Therefore
    \begin{align*}
        \bar x_h(y) &= \frac{\E[\mu(Y)K_h(y-Y)]}{\E[K_h(y-Y)]} \\
        &= B\,\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}y, \\
        \bar x_h(y)-By &= -B\,h^2(\Sigma_Y+h^2I_k)^{-1}y \\
        &= -h^2B\Sigma_Y^{-1}y+O(h^4),
    \end{align*}
    which is \eqref{eq:lgbias}. With $\Delta\mu=0$ and $\nabla\log\rho_Y=-\Sigma_Y^{-1}y$, $b(y)=-2B\Sigma_Y^{-1}y$ (checked in \texttt{scripts/verify\_nw\_bias\_affine.py}).

    Design points, with $\rho_Y(y^i)$ bounded away from $0$: at $y=y^i$ the weight of $j=i$ is
    \begin{align*}
        \frac{K_h(0)}{\sum_jK_h(y^i-y^j)} &= \Theta\big((Nh^k)^{-1}\big) \\
        &\to 0 .
    \end{align*}
    If $Nh^k\not\to\infty$ it does not vanish, and it tends to $1$ when $Nh^k\to0$ (Corollary~\ref{cor:zerobw}).
\end{proof}

\subsection{Proof of Proposition~\ref{prop:liplb}}
\label{apx:pf-liplb}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:liplb}} (Lipschitz lower bound).
...
\begin{proof}[Proof of Proposition~\ref{prop:liplb}]
    Let $X,X'$ be i.i.d.\ $\N(tx^i,(1-t)^2I_d)$ and $e=f_i-v$. Then
    \begin{align*}
        \E|f_i(X)-f_i(X')|^2 &= \frac{\E|X-X'|^2}{(1-t)^2} = 2d, \\
        \E|v(X)-v(X')|^2 &\le L^2\,\E|X-X'|^2 = 2dL^2(1-t)^2, \\
        \E|e(X)-e(X')|^2 &= 2\E|e(X)|^2-2\big|\E e(X)\big|^2 \\
        &\le 2\,\E|e(X)|^2 .
    \end{align*}
    The triangle inequality in $L^2(\Omega)$ for $f_i(X)-f_i(X')=[e(X)-e(X')]+[v(X)-v(X')]$ gives
    \begin{align*}
        \sqrt{2d} &\le \sqrt{2\,\E|e(X)|^2}+\sqrt{2d}\,L(1-t), \\
        \E|e(X)|^2 &\ge d\,[1-L(1-t)]_+^2 .
    \end{align*}
    (The identity for $\E|e(X)-e(X')|^2$, rather than the bound $4\E|e|^2$, keeps the constant $d$.)
\end{proof}
...
\begin{quote}\small
\noindent\textbf{Corollary~\ref{cor:floor}} (Loss floor under a uniform Lipschitz constraint).
...
\begin{proof}[Proof of Corollary~\ref{cor:floor}]
    ...
    \begin{align*}
        ...
        &= \frac1{3L}.
    \end{align*}
    Proposition~\ref{prop:liplb} holds for every $i$, every $v\in\mathcal F_L$ and $t\ge1-1/L$; averaging over $I$ and $t$,
    \begin{align*}
...
\subsection{Proof of Proposition~\ref{prop:cfg}}
\label{apx:pf-cfg}
...
\label{eq:cfgfield}
\end{equation}
where $q_i$ are the unconditional index weights of Proposition~\ref{prop:uncond}. The coefficients sum to $1$ but are in general \emph{not} nonnegative; hence
...
\label{eq:cfgorth}
\end{equation}
so $q_t\to0$ and every weak limit of the guided flow's marginals is supported in $\mathcal A$,
for every $w$ and every initial law. In particular, if $N\le d$ then $\dim\mathcal A\le
...
\begin{proof}[Proof of Proposition~\ref{prop:cfg}]
    By Propositions~\ref{prop:kernel-field} and~\ref{prop:uncond},
    \begin{align*}
        \vstar &= \sum_i\wih{i}\,\fat{i}, \\
        v_{\mathrm{unc}}^\star &= \sum_iq_i\,\fat{i}.
    \end{align*}
    In $v_w=(1+w)\vstar-w\,v_{\mathrm{unc}}^\star$ the $x$-terms give $-(1+w)x+wx=-x$, so $v_w=\frac{m_w-x}{1-t}$ with coefficients $(1+w)\wih{i}-wq_i$ summing to $1$; thus $m_w\in\mathcal A$. For the orthogonal projection $\Pi$ onto $\mathcal A$ (affine, $\Pi m_w=m_w$),
    \begin{align*}
        \dot q_t &= -\frac{q_t}{1-t} \\
        \Longrightarrow\ q_t &= (1-t)\,q_0 ,
    \end{align*}
    so $\dist(x_t,\mathcal A)\to0$ and, $\mathcal A$ being closed, the endpoint law is supported in $\mathcal A$. Well-posedness: $|m_w|\le(1+2w)\max_i|x^i|$, so \eqref{eq:vgrowth} holds with $M\to(1+2w)M$. For a fixed $p_0\in\mathcal A$ and $R=(1+2w)M+|p_0|$, integrating $\frac d{dt}\frac{\Pi x_t-p_0}{1-t}=\frac{m_w-p_0}{(1-t)^2}$ gives $|\Pi x_t-p_0|\le(1-t)|\Pi x_0-p_0|+Rt$, so the trajectories are bounded, the marginals are tight, weak limits exist along subsequences, and each is supported in $\mathcal A$.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:nearduplicate}}
\label{apx:pf-nearduplicate}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:nearduplicate}} (Near-duplicate labels).
...
\begin{proof}[Proof of Proposition~\ref{prop:nearduplicate}]
    Let $K_i=c\,e^{-|y-y^i|^2/2h^2}$.

    (a)
    \begin{align*}
        \sum_{j\notin C}\pih{j} &\le \frac{\sum_{j\notin C}K_j}{\sum_{i\in C}K_i} \\
        &\le \frac{(N-|C|)\,e^{-\Delta^2/2h^2}}{|C|\,e^{-\eps^2/2h^2}} = \eta .
    \end{align*}

    (b) For $i,i'\in C$,
    \begin{align*}
        \frac{\pih{i}}{\pih{i'}} &= \frac{K_i}{K_{i'}} = \exp\frac{|y-y^{i'}|^2-|y-y^i|^2}{2h^2} \\
        &\le e^{\eps^2/2h^2} = \kappa .
    \end{align*}

    (c) Let $S=\sum_{i\in C}\pih{i}$. By (b), $|C|\,\pih{i}\le\kappa S$, so
    \begin{align*}
        \sum_{i\in C}(\pih{i})^2 &\le S\max_{i\in C}\pih{i} \le \frac\kappa{|C|}, \\
        \sum_{j\notin C}(\pih{j})^2 &\le \eta^2,
    \end{align*}
    and
    \begin{align*}
        n_{\mathrm{eff}} &= \Big(\sum_i(\pih{i})^2\Big)^{-1} \\
        &\ge \frac{|C|}{\kappa+|C|\eta^2}.
    \end{align*}
    As $\eps/h\to0$ and $\Delta/h\to\infty$: $\kappa\to1$, $\eta\to0$, $S\ge1-\eta\to1$ and $p_i/p_{i'}\in[\kappa^{-1},\kappa]$, so the posterior tends to the uniform law on $C$ and, by Theorem~\ref{thm:endpoint}, the endpoint law to $|C|^{-1}\sum_{i\in C}\delta_{x^i}$. Also $\sum_i(\pih{i})^2\ge S^2/|C|$ gives $n_{\mathrm{eff}}\le|C|/S^2\to|C|$.
\end{proof}

\subsection{Proof of Proposition~\ref{prop:cfgatom}}
\label{apx:pf-cfgatom}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:cfgatom}} (Guidance lands on a training atom).
...
\begin{proof}[Proof of Proposition~\ref{prop:cfgatom}]
    Suppose $m_w(x_t,t)\to m^*\ne x^*$ and put $u=\frac{m^*-x^*}{|m^*-x^*|}$, $c=\frac{|m^*-x^*|}2$. For $t\ge t_0$, $\langle m_w-x_t,u\rangle\ge c$, so
    \begin{align*}
        \frac d{dt}\langle x_t,u\rangle &= \frac{\langle m_w-x_t,u\rangle}{1-t} \\
        &\ge \frac c{1-t}, \\
        \langle x_t,u\rangle &\ge \langle x_{t_0},u\rangle+c\log\frac{1-t_0}{1-t} \\
        &\to +\infty,
    \end{align*}
    contradicting $x_t\to x^*$. Hence
    \begin{equation}
        m_w(x_t,t)\to m^* \ \Longrightarrow\ m^*=x^* .
        \label{eq:pf:cfgatom:1}
    \end{equation}
    Both $\wih{j}$ and $q_j$ have the form (Lemma~\ref{lem:cov})
    \begin{equation*}
        \alpha_j = \frac{c_j\,e^{-|x-tx^j|^2/2(1-t)^2}}{\sum_lc_l\,e^{-|x-tx^l|^2/2(1-t)^2}},
        \qquad c_j=\KK{j}\ \text{resp.}\ \tfrac1N .
    \end{equation*}
    As $x_t\to x^*$ and the nearest atom $x^{i^*}$ is unique,
    \begin{align*}
        |x_t-tx^j|^2-|x_t-tx^{i^*}|^2 &\to 2\kappa_j>0, \\
        \frac{\alpha_j}{\alpha_{i^*}} &\le \frac{c_j}{c_{i^*}}\,e^{-\kappa_j/2(1-t)^2} \\
        &\to 0,
    \end{align*}
    so $\alpha_{i^*}\to1$ and $\sum_j\alpha_jx^j\to x^{i^*}$, for both families with the same $i^*$.

    (a) $h>0$:
    \begin{align*}
        m_w &= (1+w)\,\bar x_h-w\,\bar x_{\mathrm{unc}} \\
        &\to (1+w)\,x^{i^*}-w\,x^{i^*} = x^{i^*},
    \end{align*}
    so $x^*=x^{i^*}$ by \eqref{eq:pf:cfgatom:1}.

    (b) $h=0$: the conditional mean is $x^i$ (Proposition~\ref{prop:collapse}(a)) and $\bar x_{\mathrm{unc}}\to x^{i^*}$, so by \eqref{eq:pf:cfgatom:1}
    \begin{equation}
        x^* = (1+w)\,x^i-w\,x^{i^*}.
        \label{eq:cfgfixed}
    \end{equation}
    Let $i^*\ne i$. If $w=0$, $x^*=x^i$ and $i^*=i$ by uniqueness, a contradiction. If $w>0$, $x^*-x^i=w\,(x^i-x^{i^*})$ and $x^*-x^{i^*}=(1+w)(x^i-x^{i^*})$, so
    \begin{align*}
        |x^*-x^i| &= w\,|x^i-x^{i^*}| \\
        &< (1+w)\,|x^i-x^{i^*}| \\
        &= |x^*-x^{i^*}|,
    \end{align*}
    contradicting the definition of $i^*$. Hence $i^*=i$ and $x^*=x^i$.
\end{proof}

\begin{remark}
The uniqueness hypothesis excludes only $x^*$ equidistant from two atoms. When it
fails, the weights concentrate on the set of nearest atoms and the argument yields
$x^*$ in their affine hull (in their convex hull for $w=0$), which is all that survives. Note also what
...
\subsection{Proof of Proposition~\ref{prop:survival}}
\label{apx:pf-survival}

\begin{quote}\small
\noindent\textbf{Proposition~\ref{prop:survival}} (Error survival).
...
collapsed field, let $x^v_t$ solve $\dot x_t=v(x_t,t,y^i)$ from $x_0$ (we assume it exists on $[0,t]$), and let
...
\item \emph{(the bound is attained, so the threshold is real)} fix a unit vector $u$
  ...
  for $a>1$: the trichotomy is realised and not merely permitted. These fields have
  $\mathcal L_i=\infty$ for $a\ge\frac12$, so the trichotomy concerns pathwise errors and
  not models with finite loss;
\item \emph{(what a surviving deviation requires)} write
  ...
  and there is no endpoint; and if $e_t=(1-t)I(t)$ has no limit in $\R^d\cup\{\infty\}$,
  bounded or not, there is no endpoint either. In particular a nonzero endpoint deviation \emph{requires} the
...
\begin{proof}[Proof of Proposition~\ref{prop:survival}]
    We have $e_0=0$, and $\vstarz$ is affine in $x$ with linear part $-\frac1{1-t}I_d$, so
    \begin{align*}
        \dot e_t &= \Delta(x^v_t,t)+\big[\vstarz(x^v_t,t,y^i)-\vstarz(x^\star_t,t,y^i)\big] \\
    ...
    \end{align*}
    and integrating with $e_0=0$ gives \eqref{eq:survival-id}; the triangle inequality gives \eqref{eq:survival}.

    (a) By Cauchy--Schwarz,
    \begin{align*}
        |e_t| &\le (1-t)\Big(\int_0^t|\Delta|^2\Big)^{1/2}\Big(\int_0^t\frac{ds}{(1-s)^2}\Big)^{1/2} \\
        &\le (1-t)\,\eps\sqrt{\tfrac t{1-t}} \\
        &= \eps\sqrt{t(1-t)} \\
        &\le \eps\sqrt{1-t}.
    \end{align*}

    (b) Since $\int_0^t(1-s)^{-1-a}ds=\frac1a\big((1-t)^{-a}-1\big)$,
    \begin{align*}
        |e_t| &\le \frac ca\big((1-t)^{1-a}-(1-t)\big) \\
        &= c\,t \le c \qquad (a=1),
    \end{align*}
    and the bound tends to $0$, $c$, $+\infty$ for $a<1$, $a=1$, $a>1$.

    (c) For $\Delta=c(1-t)^{-a}u$, $|u|=1$, the integrand is trajectory-free:
    \begin{equation*}
        e_t =
        \begin{cases}
            \dfrac ca\big((1-t)^{1-a}-(1-t)\big)\,u, & a>0, \\[6pt]
            -c\,(1-t)\log(1-t)\,u, & a=0,
        \end{cases}
    \end{equation*}
    which is \eqref{eq:survival-sharp}. The direction is fixed, so \eqref{eq:survival} is an equality, and with $\epsilon=1-t$,
    \begin{align*}
        |e_t| &= \frac ca\big(\epsilon^{1-a}-\epsilon\big) \\
        &\to 0,\ c,\ \infty \qquad (a<1,\ a=1,\ a>1).
    \end{align*}
    Also $v=\vstarz+\Delta$ is locally Lipschitz with $L_\Delta=0$, so Assumption~\ref{as:lipschitz} holds.

    (d) With $I(t)=\int_0^t\frac{\Delta(x^v_s,s)}{1-s}\,ds$,
    \begin{align*}
        e_t &= (1-t)\,I(t), \\
        e_t-e_\star &= (1-t)\Big[I(t)-\frac{e_\star}{1-t}\Big],
    \end{align*}
    so $e_t\to e_\star$ iff $I(t)=\frac{e_\star}{1-t}+o\big((1-t)^{-1}\big)$, which is \eqref{eq:survival-conv}. The four cases are the behaviours of $(1-t)I(t)$.
\end{proof}

\subsection{Proof of Corollary~\ref{cor:lossform}}
\label{apx:pf-lossform}

\begin{quote}\small
\noindent\textbf{Corollary~\ref{cor:lossform}} (Loss form).
...
deviation over initial conditions. Then for $t\le T$, and for $\delta$ with $1-\delta\le T$,
\begin{equation*}
...
\begin{proof}[Proof of Corollary~\ref{cor:lossform}]
    $U=\vstarz(X_t,t,y^i)$ a.s.\ (Proposition~\ref{prop:collapse}(a)), so
    \begin{align*}
        \mathcal L_i(v) &= \int_0^1\ell(s)^2\,ds, \\
        \ell(s)^2 &= \E_{X_0}|\Delta(x^\star_s,s)|^2 .
    \end{align*}
    Assume that $x^v$ exists on $[0,T]$ for $\pi_0$-a.e.\ $X_0$, with $T\ge1-\delta$. A pathwise Gr\"onwall gives $|e_t|\le e^{L_\Delta t}\int_0^t|\Delta(x^\star_s,s)|\,ds$, which is in $L^2(X_0)$ because $\ell\in L^2(0,1)$, so $\mathcal E(t)<\infty$.
    Taking $L^2(X_0)$ norms in \eqref{eq:survival} (Minkowski) and using Assumption~\ref{as:lipschitz},
    \begin{align*}
        \mathcal E(t) &\le (1-t)\int_0^t\frac{\|\Delta(x^v_s,s)\|_{L^2}}{1-s}\,ds, \\
        \|\Delta(x^v_s,s)\|_{L^2} &\le \ell(s)+L_\Delta\,\mathcal E(s).
    \end{align*}
    With $F=\frac{\mathcal E}{1-t}$ and $G=\int_0^t\frac{\ell(s)}{1-s}\,ds$,
    \begin{align*}
        F(t) &\le G(t)+L_\Delta\int_0^tF(s)\,ds \\
        \Longrightarrow\ F(t) &\le G(t)\,e^{L_\Delta t},
    \end{align*}
    by Gr\"onwall ($G$ is nondecreasing). By Cauchy--Schwarz,
    \begin{align*}
        G(t) &\le \Big(\int_0^t\ell^2\Big)^{1/2}\Big(\int_0^t\frac{ds}{(1-s)^2}\Big)^{1/2} \\
        &\le \sqrt{\mathcal L_i(v)}\,\sqrt{\tfrac t{1-t}},
    \end{align*}
    so
    \begin{align*}
        \mathcal E(t) &= (1-t)\,F(t) \\
        &\le e^{L_\Delta t}\sqrt{\mathcal L_i(v)}\,\sqrt{t(1-t)},
    \end{align*}
    which is \eqref{eq:lossform} ($t=1-\delta$). Finally $|x^\star_{1-\delta}-x^i|=\delta\,|X_0-x^i|$, and the triangle inequality in $L^2(X_0)$ gives the last display.
\end{proof}
...
\begin{proof}[Proof of Lemma~\ref{lem:nonintersect}]
    (a) We have $\ell_i(\hat t)=\ell_j(\hat t)$ iff $z^j=z^i+\frac{1-\hat t}{\hat t}(x_0^i-x_0^j)$. With $s=\frac{1-t}t\in(0,\infty)$,
    \begin{align*}
        \{\exists\,t:\ell_i(t)=\ell_j(t)\} &= \{z^j\in R\}, \\
        R &= \{z^i+s\,(x_0^i-x_0^j):s>0\}.
    \end{align*}
    $R$ is a half-line (a point if $x_0^i=x_0^j$), hence Lebesgue-null for $d\ge2$; $z^j\perp(x_0^i,x_0^j,z^i)$ has an absolutely continuous law, so the probability is $0$. A union bound over pairs finishes (a).

    (b) Here
    \begin{equation*}
        z^j\in\Big\{\big[(1-t_i)x_0^i+t_iz^i-(1-t_j)x_0^j\big]/t_j:\ (t_i,t_j)\in(0,1)^2\Big\},
    \end{equation*}
    a countable union of Lipschitz images of bounded planar sets, of Hausdorff dimension $\le2<d$, hence null.
\end{proof}
...
$t^{(i,j)}\sim\mathrm{Unif}(0,1)$, $j=1,\dots,m$, independently across $(i,j)$. Let
...
\begin{proof}[Proof of Proposition~\ref{prop:memorize}]
    By Lemma~\ref{lem:nonintersect}(a)--(b) and a union bound over $\binom N2$ pairs, a.s.\ the segments $\{\ell_i(t)\}$ are pairwise disjoint, so $(X_t^{(i,j)},t^{(i,j)})$ never coincide across $i$ and $i(x,t)$ is defined on the sample. Set
    \begin{align*}
        v(x,t) &= z^{i(x,t)}-x_0^{i(x,t)} \quad\text{on the sample}, \\
        v(x,t) &= 0 \quad\text{elsewhere}.
    \end{align*}
    Then
    \begin{align*}
        v\big(X_t^{(i,j)},t^{(i,j)}\big) &= z^i-x_0^i \quad\forall(i,j), \\
        L_{\mathrm{MC}}^{\mathrm{unc}}(v) &= 0 .\qedhere
    \end{align*}
\end{proof}
...
\begin{proof}[Proof of Remark~\ref{lem:norepr}]
    $f_\theta$ is a composition of finitely many affine maps and Lipschitz activations, hence $K$-Lipschitz with $K<\infty$. For $t_0\in[0,1)$,
    \begin{align*}
        |f_\theta(t)| &\le |f_\theta(t_0)|+K\,|t-t_0| \\
        &\le |f_\theta(t_0)|+K \qquad (t\in[0,1)),
    \end{align*}
    so $f_\theta$ is bounded on $[0,1)$, whereas $g(t)=\frac1{1-t}\to\infty$. Hence $\sup_{t\in[0,1)}|f_\theta(t)-g(t)|=\infty$.
\end{proof}
...
\paragraph{When does the flow commit?} Theorem~\ref{thm:endpoint} gives the
destination and not the speed. Along exact trajectories the effective number of atoms
$n_{\mathrm{eff}}(t)=1/\sum_iw_i^2$ of the index posterior falls from the size of the
label window, which $h$ sets, to one. Figure~\ref{fig:collapsetime}(a) shows this on the
EXP-1 instance: the label factor sets where each curve starts and the spatial factor
brings every curve to one by $1-t\approx0.03$. Panel~(b) computes, for standard Gaussian
atoms, the time $t_c$ at which the spatial factor alone has selected one atom (median
$n_{\mathrm{eff}}\le2$). It falls from $0.86$--$0.97$ at $d=2$ to $0.06$ at $d=3072$, so
at image dimensions the flow commits within the first tenth of its clock. The dotted
curve $t_c/(1-t_c)=2\sqrt{\log N/d}$ comes from a Gaussian-fluctuation argument that we do
not prove; it is a heuristic that tracks the computed values, not a result of this paper.
This is the flow-matching counterpart of the collapse time of \citet{biroli2024dynamical},
computed for the exact field.

\begin{figure}[htb]
\centering
\includegraphics[width=0.98\textwidth]{fig_collapse_time.png}
\caption{When the index posterior commits to one atom, from exact trajectories of the
population flow (no training). (a)~$n_{\mathrm{eff}}$ against $1-t$ on the EXP-1 instance
($d{=}2$, $N{=}200$) for three bandwidths and the unconditional field; lines are medians
over $160$ source draws, bands the $10$--$90\%$ range. The label factor sets the start,
the spatial factor the end. (b)~Collapse time $t_c$ of the spatial factor against
dimension for Gaussian atoms and three dataset sizes; dotted, a heuristic scaling.}
\label{fig:collapsetime}
\end{figure}

\paragraph{The optimal field, drawn.} Figure~\ref{fig:velocityfield} plots
$\vstar(x,t,y)=\sum_iw_i(x^i-x)/(1-t)$ on the same instance. At $h=0$ every arrow points
at the one atom the label identifies, at every time. At $h=h^\star$, where
$\tr\Covh=\tr\Sigmapost$, the field points at many atoms at $t=0.5$ and commits to the
nearest of them by $t=0.9$, so the smoothed flow is still a partition of the plane into
basins of attraction of the atoms.

\begin{figure}[htb]
\centering
\includegraphics[width=0.98\textwidth]{fig_velocity_field.png}
\caption{The population velocity field on the EXP-1 instance at $t=0.5$ and $t=0.9$. Arrows
have unit length and are coloured by the atom of largest weight, with opacity given by
that weight; black dots are the atoms, with area proportional to the label weight
$p_i^{(h)}(y)$. (a,b)~$h=0$: one atom attracts everywhere. (c,d)~$h=h^\star\approx0.70$:
the field commits atom by atom as $t\to1$.}
...
induces\\
\citet{gao2024flow}; \citet{bertrand2025closed} & unconditional & the closed-form
optimal velocity is a weighted average of the directions $x^i-x$ and its flow reaches only
training points; generalisation appears when the network fails to approximate it & the
\emph{conditional} closed form, and what conditioning noise does to the endpoint law
rather than to the field\\
\citet{pfrommer2025conditional} & conditional & trained conditional models are smooth in
the conditioning variable and deviate from the denoising path (``self-guidance'') &
the exact population counterpart: noise on the label gives a kernel over the atoms, with
a moment reference and an atomicity floor\\
...
rare without it; randomising captions mitigates & the conditional side of this
asymmetry, Eq.~\eqref{eq:distinction}, and that caption randomisation is label smoothing in the
...
minimiser agrees with it a.e.\ in $(x,t,y)$. A query $y$ is a null slice, so at fixed
$h>0$ it is the choice of the version continuous in $(x,t,y)$ that fixes the field at
$y$, and we use it throughout. At $h=0$ and $y=y^i$ the label has positive mass and
there is no such choice. \emph{(ii) Endpoints.}
...
reproduced to six decimal places for $h\in\{0,0.7\}$ and $w\in\{0,3,12\}$). Proposition~\ref{prop:cfgatom} proves it pathwise for every convergent trajectory whose
limit has a unique nearest atom; that almost every trajectory converges, and avoids the
non-generic ties, is what the numerics support and we do not prove.
...
at once exactly when $\varepsilon\ll h\ll\Delta$, so a usable $h$ exists when
...
hypothesis, and it is a strong one (see below).
...
field, whose constant $1/(1-t)$ diverges by construction. An error $c\,(1-t)^{-1}u$, the
kind part~(b) identifies as the only kind that survives, is constant in $x$ and has
$L_\Delta=0$. The hypothesis is nonetheless strong. It is equivalent to
$\|\nabla_xv+(1-t)^{-1}I\|\le L_\Delta$, so a model with $\operatorname{Lip}_xv\le K$
satisfies it on $[0,T]$ only if $L_\Delta\ge(1-T)^{-1}-K$. For a bounded-Lipschitz
network and $T=1-\delta$ the bound is of order $e^{1/\delta}$ and says nothing, and the
corollary is a statement about fields that reproduce the linear part of $(\star)$. For
such fields, \eqref{eq:lossform} says that the conditional standard deviation retained
at truncation $1-\delta$ is at most $e^{L_\Delta}\sqrt{\mathcal L_i\delta}$: loss and
truncation enter symmetrically, both as square roots. Appendix~\ref{app:survival} reports the bound's measured
tightness over sixteen configurations and reports the $\sqrt{\mathcal L_i}$ scaling on
EXP-1's own checkpoints, where the slope is $0.429$ ($R^2=0.99$) over six checkpoints
and $0.485$ ($R^2=0.998$) over the last three, against $\tfrac12$. That is an empirical
scaling and not a test of the bound, whose hypothesis these networks do not meet with a
moderate constant; the fit implies a prefactor of about $25$, which we do not use. One limit remains: the bound
...
appendix) and stays far below the posterior. Equation~\eqref{eq:invariance} covers the
corrected target, whose endpoint law is $\delta_{x^i}$ for every $\sigma$. The
mis-specified target ends at $x^i+\sigma^{2/(1-\sigma^2)}X_0$, a spread of about $0.01$ at
$\sigma=0.1$, so the two rows agree here because both are dominated by the gap to the
training data and not because \eqref{eq:invariance} applies to the old target.
...
truncations, with $\mathcal L_i$ estimated on a finite sample, since it is infinite for
$a\ge\frac12$) it holds everywhere, with the measured deviation between $1.9$ and $27$
...
describes. The implied prefactor is $e^{L_\Delta}\approx25$ and is near-constant across the
collapse ($23.9\to26.0$). We do not identify the implied $L_\Delta\ge3.2$ with a measured
Lipschitz constant. Since $\nabla_xv^\star=-I_d/(1-t)$, a network with
$\operatorname{Lip}_xv\le K$ has $L_\Delta\ge(1-T)^{-1}-K$, which at $\delta=10^{-3}$ is
of order $10^3$, so Assumption~\ref{as:lipschitz} does not hold for these networks with a
moderate constant and the slope below is an empirical scaling, not a verification of the
bound. We put it no more strongly than that: the observed scaling is \emph{consistent} with the
$\tfrac12$ exponent the bound predicts, on one problem and one training trajectory,
which is not a demonstration that optimisation must follow it. The fitted prefactor
of $25$ is not used anywhere. This is also where ``optimisation-paced''
...
contraction whose strength diverges, so a deviation reaches the endpoint only if the
accumulated error does: $a<1$ is driven to zero, $a=1$ is the separatrix and
flattens at the error's own coefficient $c$, and $a>1$ grows without bound (for these
explicit errors, where the bound is an equality; the bound alone constrains nothing). The
...
$e^{L_\Delta}\approx25$; the bound is loose, and the corollary's hypothesis does not
hold for this network with a moderate $L_\Delta$ (Appendix~\ref{app:survival}), so the
slope is an empirical scaling consistent with $\tfrac12$ and not a test of the bound. The
...
avoid) and, empirically, the loss tracks how far it has stopped failing.}
\label{fig:survival}
\end{figure}
...
within seed noise. Proposition~\ref{prop:prop17}(c) covers the corrected target; the old
target ends at $x^i+\sigma^{2/(1-\sigma^2)}X_0$, a spread of $0.01$ at $\sigma=0.1$ and $0.07$
at $\sigma=0.3$.}
\label{tab:p7i}
\end{table}
...
by \texttt{scripts/make\_image\_bandwidth\_figure.py}. The same sweep for seeds~1 and~2 is
Figures~\ref{fig:imagebandwidth1} and~\ref{fig:imagebandwidth2}, and the $h{=}0$ row as it forms is
Figure~\ref{fig:imagetraining}.}
\label{fig:imagebandwidth}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\textwidth]{fig_image_training.png}
\caption{The $h{=}0$ row of Figure~\ref{fig:imagebandwidth} as it forms (CIFAR-10 DDPM, seed~$0$):
the grid the run saved at each of six iterations, on the two conditions shown there. At $500$ and
$2000$ iterations the six completions of a condition differ from one another and agree with the
observed top half only. From $8000$ iterations on they are one image, the true one, and stay so
to $60000$ (Proposition~\ref{prop:collapse}). Cut from the saved grids by
\texttt{scripts/make\_image\_extra\_figures.py}.}
\label{fig:imagetraining}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\textwidth]{fig_image_bandwidth_seed1.png}
\caption{Figure~\ref{fig:imagebandwidth} for seed~$1$; each seed draws its own training set, so the
conditions differ. $h{=}0$: six identical completions, equal to the true image, on both
conditions. $h{=}4$: one of six completions in the upper row reproduces the true image and none
does in the lower row; $h{=}5$ and $h{=}6$: none does.}
\label{fig:imagebandwidth1}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\textwidth]{fig_image_bandwidth_seed2.png}
\caption{Figure~\ref{fig:imagebandwidth} for seed~$2$. $h{=}0$: six identical completions, equal to
the true image, on both conditions. $h{=}4$: two of six completions in the upper row reproduce
the true image and none does in the lower row; $h{=}5$ and $h{=}6$: none does.}
\label{fig:imagebandwidth2}
\end{figure}

\paragraph{Class labels.} With $N{=}2000$ CIFAR-10 images and the class as the label, about $200$
images share each label, so the population minimiser collapses onto the class and not onto one image
(Corollary~\ref{cor:repeated}), and the conditional variance it predicts is the
within-class scatter rather than zero. Figure~\ref{fig:imageclass} shows two runs of the DDPM, at
$h{=}0$ and $h{=}0.4$ on the one-hot label: the variance is retained, and most of the completions are
nonetheless training images.

\begin{figure}[htb]
\centering
\includegraphics[width=0.74\textwidth]{fig_image_class.png}
\caption{Class-conditional CIFAR-10 DDPM ($N{=}2000$, seed~$0$). \textbf{(a)}~$\mathrm{tr}\,\mathrm{Cov}$
of the completions, averaged over $10$ conditions, at the six checkpoints, for $h{=}0$ and
$h{=}0.4$, against the within-class scatter $717$ (dashed; $719$ at $h{=}0.4$). It does not collapse: it stays at
$0.64$--$0.83$ of that value for $h{=}0$ and $0.62$--$1.30$ for $h{=}0.4$ (the largest at iteration
$500$), and both end at $0.76$ while the training loss falls from $0.22$ to $0.06$.
\textbf{(b,~c)}~Sample grids at $60000$ iterations. The model sees only the class of the observed
image, and its six completions are different images of that class, not one image, and yet
$19$ of the $24$ completions in each run lie on a training image, at both $h$: their distance to
the nearest of the $50000$ CIFAR-10 training images is below $\tfrac13$ of the distance to the
second nearest (the criterion of Section~\ref{sec:cifarddpm}). The cells were recovered from the
saved grids at their 8-bit values (\texttt{scripts/grid\_nearest\_training\_image.py}); against all
$50000$ images the count is a lower bound, and the same test on the DDPM grids of
Figures~\ref{fig:imagebandwidth}, \ref{fig:imagebandwidth1} and~\ref{fig:imagebandwidth2} gives
$1.00$, $0.67$, $0.53$ and $0.54$ on an atom at $h{=}0,4,5,6$ ($72$ completions each), below the
full-sample $1.000$, $0.829$, $0.717$ and $0.620$ of Section~\ref{sec:cifarddpm}, as a lower bound
should. Cut from the saved grids and metrics by
\texttt{scripts/make\_image\_extra\_figures.py}.}
\label{fig:imageclass}
\end{figure}
