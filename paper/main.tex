...
\begin{figure}[t]
\centering
\includegraphics[width=0.70\textwidth]{fig_mechanism_schematic.png}
...
moving the support removes that obstruction (Proposition~\ref{prop:tgtnoise}). The middle
panel is this paper's title; Figure~\ref{fig:mechanismreal} is the same scenes on a real
instance.}
\label{fig:mechanism}
\end{figure}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.68\textwidth]{fig_cifar_ddpm_tracking.png}
...
$\beta=1$; solid: the fitted slope, with the range the regression had. The panels share
...
exact for the population minimiser, does not survive training ($1.000$, $0.829$, $0.717$,
$0.620$ on an atom at $h=0,4,5,6$); $n_{\mathrm{eff}}$ rather than $h$ governs the
discrepancy; and across three fresh instances the aggregate ratio is stable only where
the reference has range.
...
either true or false. Collapse is moreover \emph{optimisation-paced}, and that gap we
bound away from one explanation rather than explain: Corollary~\ref{cor:floor}'s
Lipschitz floor sits $\sim\!50\times$ below the $h{=}0$ plateau (Appendix~\ref{sec:aux}).
...
that scale, and for text-conditional models whose label space carries no natural kernel,
persistence is a claim we do not establish; endpoint smoothing's benefit is likewise
bounded by the obstruction it removes---small in exactly the regime we study most---and
...
training atoms---which neither it nor interpolant noise ever leaves. Only endpoint
smoothing does, and only as far as the floor it removes is large
(Appendix~\ref{app:remedies} places all four).
