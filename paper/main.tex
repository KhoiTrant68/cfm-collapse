...
That the empirical minimiser memorises is \emph{not} new; it is well established in
the diffusion-memorisation literature
...
Closest in spirit is \citet{buchanan2025edge}, who characterise a
...
\item \textbf{Label smoothing $\equiv$ kernel regression.} Label noise makes the
  population endpoint an exact Nadaraya--Watson mixture
...
$\operatorname{supp}(\rho_Y)$ at which $\rho_Y$ is positive and $C^2$, $\Sigma(\cdot)$ is
$C^1$, and $\mu(\cdot)$ is $C^2$ (all standard nonparametric-regression regularity, needed
only in a neighbourhood of this fixed $y$). Then, pointwise at this $y$, as
$Nh^k\to\infty,h\to0$,
\begin{equation}
\Covh(y)=\Sigma(y)+h^2 J(y)J(y)^\top+O(h^4)+O_P\big((Nh^k)^{-1/2}\big),
\label{eq:expansion}
\end{equation}
so $\tr\Covh(y)=\tr\Sigma(y)+h^2\|J(y)\|_F^2+\cdots$. The expansion is not claimed
uniform over $y$; near the boundary of $\operatorname{supp}(\rho_Y)$ the kernel loses mass
asymmetrically and the bias term changes order (the standard nonparametric
boundary-bias phenomenon), which \eqref{eq:expansion} does not cover.
\end{proposition}
...
the two; we flag this conflation rather than disentangle it.
