...
directly shows it is small at our default configuration, and that removing the
obstruction does not buy accuracy there: swept over $(h,\rho)$, the closest law to the
posterior is the unsmoothed one, in both metrics we score with. Two superficially similar remedies are moreover not
...
log--log slope of $0.481$ where the theory predicts $1$, and the atomicity that holds
exactly for the population minimiser is not inherited by the trained model---$38\%$ of
its samples are off-atom at the widest bandwidth we test, under the standard
...
widens to $[0.92,1.18]$---while the across-condition slope is $-0.156$
  ...
  population minimiser is not inherited by the trained model, $38\%$ of whose samples
  are off-atom. The warning
...
\item \textbf{A diagnostic, and the wrong one it replaces.} The methodological
  ...
  evaluation conditions, where the theory predicts $\beta=1$ and $\beta\approx0$ means
  no detectable log--log dependence of the measured variance on the predicted one. On our data the two statistics disagree in the worst
  possible way---at $h{=}6$ the level says $1.15$ and the slope says $-0.156$---and
  the slope is the more informative of the two in that regime. It is not a certificate
  either: $\tr\Cov=10\,\tr\Covh$ has slope exactly $1$ and is calibrated wrong by an
  order of magnitude, so $\beta$ and the level answer different questions and a claim
  of agreement needs both. We also show what the level's error bars hide:
...
The floor's \emph{size} is what decides whether this matters, and it is governed by the
rate factor $N^{-1/d}$: $0.071$ at our $(d,N)=(2,200)$ default but $0.9975$ at
$(3072,2000)$. Zador's theorem \citep[Ch.~6]{graf2000quantization} gives that rate for
the \emph{optimal} $N$-point quantiser and so bounds our floor from below, the training
atoms being one admissible $N$-point set; whether i.i.d.\ atoms attain it is a separate
question, measured in Appendix~\ref{app:deferred}. The obstruction is negligible
...
$\operatorname{supp}(\rho_Y)$ and a ball $B(y,r)$ contained in that interior, on which
we assume: $\rho_Y$ is positive and $C^2$; $\mu$ is $C^3$ with
$M_3:=\sup_{B(y,r)}\|\nabla^3\mu\|<\infty$; and $\Sigma(\cdot)$ is \emph{constant}.
Assume also the global tail conditions that $\rho_Y$ is bounded and that
$|\mu(y')|+\|\Sigma(y')\|\le C(1+|y'|^m)$ for some $C,m$. Then, pointwise at this $y$,
as $Nh^k\to\infty,h\to0$,
\begin{equation}
\Covh(y)=\Sigma(y)+h^2 J(y)J(y)^\top+O(h^4)+O_P\big((Nh^k)^{-1/2}\big),
\label{eq:expansion}
\end{equation}
so $\tr\Covh(y)=\tr\Sigma(y)+h^2\|J(y)\|_F^2+\cdots$. Two remarks on the regularity.
$C^3$ on $\mu$, rather than the $C^2$ that would suffice for the $h^2$ term alone, is
what the $O(h^4)$ remainder costs: $C^2$ gives a Taylor remainder $o(h^2)$ by Peano and
only $o(h^3)$ after it is paired with the $hJu$ term, whereas a bounded third
derivative gives $O(h^3|u|^3)$ and hence $O(h^4)$. The tail conditions are needed
because the kernel is Gaussian, so $u=(Y-y)/h$ ranges over all of $\R^k$ while the
local hypotheses hold only on $B(y,r)$; they make the contribution of $|u|>r/h$
smaller than every power of $h$ (Step~3 of the proof). Both are free in the instance we
test: the linear-Gaussian model of Section~\ref{sec:setup} has $\mu$ affine, so
$\nabla^3\mu\equiv0$ and the expansion is exact. Homoscedasticity is
...
The between-group inflation $h^2\|J\|_F^2$ is positive and survives kernel symmetry,
which kills only the odd-order terms; it produces the right-hand (over-smoothing)
branch of the empirical U-curve. In the linear-Gaussian
...
label smoothing. Of the four interventions considered in this paper, interpolant noise
leaves the population endpoint law exactly where it was, at every noise level
(Proposition~\ref{prop:prop17}). Guidance cannot move the endpoint off the affine hull
of the atoms (Proposition~\ref{prop:cfg}), and under hard conditioning it is fully
inert---the conditioned atom is reached for every $w$; with smoothed labels ($h>0$) it
still lands on a training atom, but it can change \emph{which} one
(Proposition~\ref{prop:cfgatom}). So guidance reweights rather than escapes, as does
label smoothing, which changes the endpoint law only by reweighting the same atoms, so
the atomicity ceiling still binds (Proposition~\ref{prop:atomicity}). Only endpoint
smoothing moves the support off the training set (Proposition~\ref{prop:tgtnoise}), and
it pays $+\rho^2I_d$ of variance to do it. That ordering---inert, atom-preserving,
reweighting, support-moving---is the practical content of the theory.
...
on $[0,1)$; Proposition~\ref{prop:survival}(d) says that failing to reproduce that
rate is the \emph{only} route to nonzero conditional variance under hard conditioning,
and \eqref{eq:survival-conv} says it as an exact asymptotic rather than as a word:
the accumulated weighted error must sit at $e_\star/(1-t)+o((1-t)^{-1})$, and
part~(c) exhibits errors that do. We are deliberate about the form of that converse.
It constrains $I(t)=\int_0^t\Delta/(1-s)\,ds$, not $\Delta$ pointwise: an error can
oscillate, or be large only on a thin set of times, and what survives depends on the
accumulation alone. The fourth case of (d) is not vacuous, which is why we state it:
the spatially constant $\Delta(t)=c(1-t)^{-1}e^{i\theta(t)}$ in the plane, with
...
magnitude with no endpoint at all
(\texttt{scripts/verify\_error\_survival.py}, part~F, which integrates the two flows
and recovers $|e_t|=0.7071$ to five decimals). Read that way, the observed plateau is the size of the weighted
integral the network's error accumulates rather than an unexplained residual, and the
collapse we measure over training (Table~\ref{tab:exp1},
Section~\ref{sec:cifarddpm}) is that quantity shrinking. What the proposition does
\emph{not} give is a pointwise converse of the form ``$e_1\ne0\Rightarrow
\Delta=\Theta((1-t)^{-1})$''; we do not claim one.
...
Lipschitz constants of Figure~\ref{fig:lipschitz}. We put this no more strongly than it
deserves: the observed scaling is \emph{consistent} with the $\tfrac12$ exponent the
bound predicts, on one problem and one training trajectory, which is not a
demonstration that optimisation must follow it. The constant is loose by a factor of
$25$ and we do not use it at all.
...
conditions}. Theorem~\ref{thm:endpoint} predicts $\beta=1$. What $\beta\approx0$
licenses is narrower than it may look: it says the measured variance has no detectable
log--log dependence on the reference \emph{over the conditions evaluated}, which is
consistent with the model ignoring the label but equally with a dependence that is
non-monotone, saturating, or on some other statistic of $y$. It is evidence of absent
tracking in this protocol, not proof that the label is unused.
...
against $h$. And repeating every bandwidth at three seeds---each redrawing the mask, the training
subset and all training randomness---separates the estimators sharply: across the three
instances the average of per-condition ratios spreads by $281\%$ at $h{=}4$ and $263\%$
at $h{=}5$, while the aggregate ratio $\tr\Cov/\tr\Covh$ spreads by $23\%$ and $30\%$.
The per-condition average is unusable; the aggregate ratio beats the raw trace where
the reference has range across conditions ($23\%$ against $41\%$ at $h{=}4$) and not
where it has none.
...
new experiments. \emph{Guidance} cannot escape the training set: it cannot leave the latter's affine
hull at any weight, and it lands on an atom, which under hard conditioning is the
conditioned atom itself for every $w$, so there it is fully inert; with smoothed
labels it can only change which atom is reached (Propositions~\ref{prop:cfg}
and~\ref{prop:cfgatom}). \emph{Caption
...
+\tfrac{h^2}2\big[\nabla^2\Sigma(y)\cdot\E_{\tilde q_h}[uu^\top]\big]+o(h^2)\\
&=\Sigma(y)+h^2\Big[\nabla\Sigma(y)\cdot s(y)
+\tfrac12\operatorname{tr}\nabla^2\Sigma(y)\Big]+o(h^2),
\end{align*}
using $\E_{\tilde q_h}[u]=h\,s(y)+O(h^2)$, the $o(h^2)$ here being the Peano remainder,
which is all that $C^2$ gives and all that this remark needs. The bracket is an $O(1)$ quantity multiplying
...
\emph{Step 3: between-group term.} First the tail. Split
$\E_{\tilde q_h}[\cdot]$ over $\{|u|\le r/h\}$ and its complement. On the complement,
$\tilde q_h(u)\le\varphi(u)\sup\rho_Y/\int\varphi\rho_Y(y+h\cdot)$, whose denominator
is $\rho_Y(y)(1+O(h^2))$ and hence bounded below; with the polynomial growth of $\mu$
and $\Sigma$, the contribution of $\{|u|>r/h\}$ to any of the moments below is at most
\[
C'\!\!\int_{|u|>r/h}\!\!(1+|y+hu|^{2m})\varphi(u)\,du
\;\le\;C''\,e^{-r^2/(4h^2)}
\]
for $h$ small, since a Gaussian tail beyond radius $r/h$ dominates any polynomial
factor. That is $o(h^p)$ for every $p$, so it is absorbed into $O(h^4)$ and we may
work on $\{|u|\le r/h\}$, where $y+hu\in B(y,r)$ and the local hypotheses apply.

There, since $\mu$ is $C^3$ on $B(y,r)$ with $\|\nabla^3\mu\|\le M_3$, Taylor with the
Lagrange remainder gives
\[
\mu(y+hu)=\mu(y)+hJ(y)u+\tfrac{h^2}{2}Q(u)+R_h(u),\qquad
Q(u)_a:=\sum_{b,c}\partial^2_{y_by_c}\mu_a(y)\,u_bu_c,
\]
with $|R_h(u)|\le\tfrac{M_3}{6}h^3|u|^3$. (This is the step that needs $C^3$: under
$C^2$ the remainder is only $o(h^2)$, which is too weak for the $O(h^4)$ claimed
below.) Since $\tilde q_h$ has Gaussian tails its moments of every order are bounded
uniformly in $h\le1$, so $\Cov_{\tilde q_h}(hJu,R_h)=O(h\cdot h^3)=O(h^4)$,
$\Cov_{\tilde q_h}(\tfrac{h^2}2Q,R_h)=O(h^5)$ and $\Cov_{\tilde q_h}[R_h]=O(h^6)$; all
are absorbed into $O(h^4)$ and we may drop $R_h$ from what follows.
and since covariance is invariant to the additive constant $\mu(y)$,
\begin{equation}
\begin{aligned}
\Cov_h[\mu(Y)]&=\Cov_{\tilde q_h}\!\Big[hJ(y)u+\tfrac{h^2}2Q(u)\Big]+O(h^4)\\
...
Only the last two steps are ours, and both are elementary. Writing
...
For \eqref{eq:lgbias}, the population version of $\bar x_h$ is the $\rho_Y$-average of
$\mu$ against the kernel, $\bar x_h(y)=\E[\mu(Y)\,K_h(y-Y)]/\E[K_h(y-Y)]$. With
$K_h=\N(0,h^2I_k)$ and $\rho_Y=\N(0,\Sigma_Y)$ the product of the two densities is,
in $y'$, an unnormalised Gaussian with precision $\Sigma_Y^{-1}+h^{-2}I_k$ and mean
$(\Sigma_Y^{-1}+h^{-2}I_k)^{-1}h^{-2}y=\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}y$. Since
$\mu(y')=By'$ is affine, averaging it against that Gaussian just evaluates $B$ at the
mean, so $\bar x_h(y)=B\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}y$ and
\[
\bar x_h(y)-By=B\big[\Sigma_Y(\Sigma_Y+h^2I_k)^{-1}-I_k\big]y=-B\,h^2(\Sigma_Y+h^2I_k)^{-1}y,
\]
which is \eqref{eq:lgbias}; expanding $(\Sigma_Y+h^2I_k)^{-1}=\Sigma_Y^{-1}+O(h^2)$
gives the stated $-h^2B\Sigma_Y^{-1}y+O(h^4)$, and $b(y)=-2B\Sigma_Y^{-1}y$ follows
from \eqref{eq:nwbiascoef} with $\Delta\mu=0$ and
$\nabla\log\rho_Y(y)=-\Sigma_Y^{-1}y$. Note this is an exact statement about the
population kernel average at every $h$, not only asymptotically. Both the closed form
and its agreement with \eqref{eq:nwbiascoef} are checked numerically in
\texttt{scripts/verify\_nw\_bias\_affine.py}, which also exhibits the U-shaped
finite-$N$ MSE at $y\neq0$ and its monotone counterpart at $y=0$.
...
\label{eq:survival-ode}
\end{equation}
The bracketed term is a \emph{contraction}, which is what makes the estimate go the
way it does. Rather than estimate $|e_t|$ directly we solve \eqref{eq:survival-ode}:
$(1-t)^{-1}$ is an integrating factor, since on $[0,T]$ for any $T<1$ the map
$t\mapsto e_t/(1-t)$ is absolutely continuous and, a.e.,
\[
\frac{d}{dt}\Big[\frac{e_t}{1-t}\Big]
=\frac{\dot e_t}{1-t}+\frac{e_t}{(1-t)^2}
=\frac{\Delta(x^v_t,t)}{1-t}-\frac{e_t}{(1-t)^2}+\frac{e_t}{(1-t)^2}
=\frac{\Delta(x^v_t,t)}{1-t},
\]
the two $e_t$ terms cancelling exactly. Integrating from $0$ to $t$ against $e_0=0$
and multiplying by $(1-t)$ gives the identity \eqref{eq:survival-id}, and
\eqref{eq:survival} follows by the triangle inequality for the integral. Note this is
an equality before norms are taken, which is what parts (c) and (d) use.
...
(b) For $a>0$, $\int_0^t(1-s)^{-1-a}ds=\frac{1}{a}\big((1-t)^{-a}-1\big)$, so
\eqref{eq:survival} gives
$|e_t|\le\frac{c}{a}\big((1-t)^{1-a}-(1-t)\big)$; at $a=1$ this is
$c\big(1-(1-t)\big)=c\,t\le c$. As $t\uparrow1$ the right-hand side tends to $0$ when
$a<1$, to $c$ when $a=1$, and to $+\infty$ when $a>1$; only the first two are
statements about $e_t$.

(c) With $\Delta(x,t)=c(1-t)^{-a}u$ the integrand of \eqref{eq:survival-id} does not
depend on the trajectory, so the identity evaluates directly. For $a>0$,
\[
e_t=(1-t)\,c\,u\int_0^t(1-s)^{-1-a}ds
=(1-t)\,c\,u\,\frac{(1-t)^{-a}-1}{a}
=\frac{c}{a}\big((1-t)^{1-a}-(1-t)\big)u ,
\]
and for $a=0$, $\int_0^t(1-s)^{-1}ds=-\log(1-t)$ gives
$e_t=-c(1-t)\log(1-t)\,u$. Both are \eqref{eq:survival-sharp}, and taking norms shows
\eqref{eq:survival} holds with equality since $|u|=1$ and the integrand keeps a fixed
direction. Writing $\varepsilon=1-t$, $|e_t|=\frac{c}{a}(\varepsilon^{1-a}-\varepsilon)$
has limit $0$ for $a<1$ (both powers vanish), limit $c$ for $a=1$ (the expression is
$c(1-\varepsilon)$ exactly), and limit $+\infty$ for $a>1$ (since
$\varepsilon^{1-a}\to\infty$). The field $v=\vstarz+\Delta$ is locally Lipschitz in
$x$ on $\{t<1\}$ because $\vstarz$ is and $\Delta$ is constant in $x$; the same
constancy gives $L_\Delta=0$, so this family also satisfies the hypothesis of
Corollary~\ref{cor:lossform}.

(d) Immediate from \eqref{eq:survival-id}: the two quantities whose limits are
compared are equal for every $t<1$, so one limit exists and equals $L$ exactly when
the other does.
\end{proof}
