...
CIFAR-10 inpainting, and under adversarially shuffled labels.
...
projection identity underlying everything is standard.
...
the two; we flag this conflation rather than disentangle it. Every across-seed
...
\paragraph{Small scale, and a remedy of limited reach.} EXP-1/EXP-2 are $d\le10$
synthetic problems and EXP-3 uses $32\times32$ images, $N{=}500$ and one small U-Net
(3 MNIST seeds, 1 CIFAR-10 seed); the mechanism is parameterisation-independent by
construction, but persistence \emph{at scale} is a separate claim we do not establish,
and the width/depth/optimiser sweep (Appendix~\ref{app:exp}) covers only $\tr\Cov$ at
one budget. Endpoint smoothing (Proposition~\ref{prop:tgtnoise}) removes the atomicity
...
findings the theory does not determine, the seed controls both problem instance and
training run, and the checkpoint-based tables require retraining the $p7y$ runs.
...
how the population optimum depends on $N$. We have not rerun the EXP-1 sweep under
an exposure control; the EXP-3 evidence here is what supports the reading.
