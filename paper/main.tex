...
\begin{proof}[Proof of Theorem~\ref{thm:endpoint}]
    ...
    \end{equation}
    has a unique solution. Both $(\Phi_t)_\#\pi_0$ and $\law(X_t)$ (by superposition) solve \eqref{eq:pf:endpoint:2}, hence
    \begin{align*}
...
\begin{proof}[Proof of Proposition~\ref{prop:factors}]
    ...
    \end{align*}
    which are distinct. Spatial factor: for $h=0$ and any $\sigma\ge0$,
    \begin{align*}
...
\begin{proof}[Proof of Proposition~\ref{prop:expansion}]
    Let $s=\nabla\log\rho_Y(y)$, $q_h\propto K_h(y-\cdot)\rho_Y$, $\E_h[f]=\E[K_h(y-Y)f]/\E[K_h(y-Y)]$ and $u=(Y-y)/h$. As $N\to\infty$, $\Covh\to\Cov_h^{\mathrm{pop}}:=\Cov_h[X]$ (LLN, Slutsky), and by total covariance
    \begin{equation}
    ...
    \end{align*}
    the latter using $m_4$ bounded near $y$ (the only use of (iv)). Hence $S_N,T_N=\E[\cdot]+O_P(\sqrt{h^{-k}/N})$ and, by Slutsky,
    \begin{align*}
...
(the remark after Proposition~\ref{prop:collapse}), and the conditional variance it predicts is the
